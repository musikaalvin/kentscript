...
class BorrowChecker:
    ...
    def exit_scope(self, scope_id=None):
        ...
        # Fast path: nothing borrowed or moved in this scope — avoid the
        # per-iteration dict/list walk in tight loops.
        if not self.borrows and not self.moved:
            return
    ...
    def check_access(self, var, mutable=False):
        ...
        # Fast path: nothing is borrowed/moved anywhere, so every owned variable
        # is freely accessible. Avoids the per-access lookups in tight loops.
        if not self.borrows and not self.moved:
            return
