...
# ── Canonical integer model ───────────────────────────────────────────────
# KentScript integers are signed 64-bit (i64). The C backends compile them
# to `long long` / `int64_t`, which wrap on overflow via two's complement.
# The interpreter previously used Python's arbitrary-precision `int`, so its
# results diverged from compiled C (e.g. big numbers, overflow behaviour).
# These helpers converge the two backends: every integer arithmetic result
# is wrapped into signed 64-bit range, matching C exactly.

_I64_BITS = 64
_I64_MASK = (1 << _I64_BITS) - 1          # 0xFFFFFFFFFFFFFFFF
_I64_SIGN = 1 << (_I64_BITS - 1)          # 0x8000000000000000


def _to_i64(x):
    """Wrap an arbitrary-precision int into signed 64-bit range.

    Mirrors C `long long` two's-complement wraparound. Non-integers (floats,
    strings, ...) are returned unchanged so float math / string ops keep
    their Python semantics.
    """
    if isinstance(x, bool):
        x = int(x)
    elif not isinstance(x, int):
        return x
    # Fast path: values already inside 62-bit range need no masking, which
    # avoids the bitwise arithmetic on every value in a tight numeric loop.
    if -4611686018427387904 <= x < 4611686018427387904:
        return x
    x &= _I64_MASK
    return x - (1 << _I64_BITS) if (x & _I64_SIGN) else x


def _i64_div(a, b):
    """Integer division truncating toward zero — matches C `a / b` (long long)."""
    a = int(a); b = int(b)
    q = abs(a) // abs(b)
    if (a < 0) != (b < 0):
        q = -q
...
def _i64_mod(a, b):
    """Remainder with the sign of the dividend — matches C `a % b` (long long)."""
    a = int(a); b = int(b)
    m = abs(a) % abs(b)
    if a < 0:
        m = -m
...
# Operator -> magic method name mapping for Instance attribute dispatch. Hoisted
# to module scope so the hot binary-op path never re-allocates it on every call.
_MAGIC_OPS = {
    "+": ("__add__", "__radd__"),
    "-": ("__sub__", "__rsub__"),
    "*": ("__mul__", "__rmul__"),
...
# Node type -> fast evaluator method name. Populated at the bottom of the
# module (after the AST classes are importable); the wrappers delegate any
# non-trivial case to the general evaluator so behaviour is preserved.
_EVAL_FAST = {}


def _register_fast(cls_name, method_name):
    """Register an AST-node class (by name) to a fast evaluator method."""
    try:
        _EVAL_FAST[globals()[cls_name]] = method_name
    except KeyError:
        pass
...
class Environment:
    ...
    def set(self, name: str, value: Any):
        ...
        if name in self.vars:
            ...
            # Reject bare assignment to undeclared variable — use 'let' to declare
            raise NameError(f"Undefined variable '{name}'. Use 'let {name} = ...;' to declare.")
...
class ModuleLoader:
    ...
    def load_module_file(self, name):
        ...
        candidate_paths = [
            os.path.join("stdlib", f"{name}.ks"),
            os.path.join(os.path.expanduser("~"), ".ks_packages", f"{name}.ks"),
        ]
        for ks_path in candidate_paths:
...
class KentScriptException(Exception):
    """A raised KentScript exception instance (a user-defined class instance)."""

    def __init__(self, instance):
        self.instance = instance
        cls_name = getattr(getattr(instance, "class_def", None), "name", "Error")
        msg = instance.attrs.get("message", "") if hasattr(instance, "attrs") else ""
        self.ks_class = cls_name
        self.ks_message = msg
        super().__init__(f"{cls_name}: {msg}")

    def __str__(self):
        return f"{self.ks_class}: {self.ks_message}"
...
# ============================================================================
# OPTIMIZATION ENGINE — Inline Caching
# ============================================================================
...
Standard Library Modules (use help('module_name')):
  ...
  help('time')      Time functions (time, sleep, strftime)
  help('random')    Random functions (random, randint, choice, shuffle)
  help('http')      HTTP client (get, post)
  help('string')    String methods (upper, contains, split, etc.)
  help('crypto')    Encoding (base64, hex)
  help('subprocess') Run shell commands
  help('hardware')  Hardware I/O and CPU info
  help('kpm')       Package manager functions
...
def _init_help_function():
    ...
    def help_builtin(topic=None):
        modules = {
            ...
            "time": "import time; time.time(), time.sleep(), time.strftime(), time.localtime()",
            "os": "import os; os.getpid(), os.mkdir(), os.rmdir(), os.getenv(), os.putenv(), os.exists(), os.remove(), os.rename(), os.getcwd(), os.listdir(), os.write_file(), os.read_file()",
            "random": "import random; random.random(), random.randint(), random.choice(), random.shuffle(), random.seed()",
            "http": "import http; http.get(url), http.post(url, headers, body)",
            "subprocess": "import subprocess; subprocess.run(cmd)",
            "sys": "import sys; sys.getpid(), sys.getenv(), sys.setenv()",
            "string": "s.upper(), s.lower(), s.title(), s.contains(), s.startswith(), s.endswith(), s.replace(), s.split(), s.trim(), s.find()  OR  import string; string.upper(s), string.lower(s), etc.",
            "crypto": "import crypto; crypto.base64_encode(), crypto.base64_decode(), crypto.hex_encode(), crypto.hex_decode()",
            "hardware": "import hardware; hardware.read_port(), hardware.write_port(), hardware.outb(), hardware.inb(), hardware.get_cpu_count()",
            "json": "dumps, loads",
            "list": "append, pop, insert, remove, extend, clear, sort",
            "kpm": "kpm_install(), kpm_list(), kpm_search(), kpm_version(), kpm_uninstall()",
            "malloc": "malloc(size), free(ptr), write_byte, read_byte, memcpy, memset",
            "syscall": "open, close, read, write, stat, fstat, lseek, getpid, exit",
            "asm": "asm(code) - Execute inline assembly",
            ...
            # Stdlib modules
            "tui": "Table(headers, rows).print(), confirm(prompt), choose(prompt, options)",
            "dataframe": "DataFrame(data).filter/sort/groupby/aggregate/select/to_csv/to_json",
            "jwt": "encode(payload, secret), decode(token, secret) - HS256",
            "email": "send(host,port,user,pw,from,to,subj,body), fetch(host,user,pw)",
            "markdown": "to_html(md_text), to_html_file(path) - regex-based converter",
            "scheduler": "Scheduler().every(interval_secs, task).start().stop()",
            "ssh": "run(host,cmd), scp(src,dest), shell(host) - subprocess to ssh",
            "docker": "ps(), pull(img), run(img), stop(cnt), rm(cnt), logs(cnt)",
            "watcher": "FileWatcher(path, callback, interval).start().stop() - polling",
            "graphql": "Client(url).query(query_str, variables).mutate(mutation, vars)",
            "excel": "read(path, sheet), write(path, data) - xlsx via zipfile+XML",
            "image": "resize(in,out,w,h), thumbnail, grayscale, blur, rotate, crop - ImageMagick",
            "cache": "Cache(maxsize, ttl).get/set/delete/has/clear/keys/stats",
            "ratelimit": "RateLimiter(max_calls, period).allow(key) / remaining / reset",
            "fileproc": "read_lines(path), each_line, grep, head, count_lines",
            "web": "App().get/post/put/delete/...listen(port) - web framework",
            "openapi": "generate(app, info) -> OpenAPI 3.0 JSON schema",
            "progress": "ProgressBar, Spinner, Counter, tqdm",
            "color": "ANSI colors - RED, GREEN, BLUE, BOLD, colorize(), gradient()",
            "color (stdlib)": "Terminal colors and styles with full ANSI support",
        }
        if topic is None:
            print("KentScript v3.1.0+ Modules:")
            for m in sorted(modules.keys()):
                print(f"  {m}: {modules[m][:80]}...")
        else:
...
# Statement node types that should trigger a debugger pause check.
_DEBUG_STMT_TYPES = {
    "LetDecl", "IfStmt", "WhileStmt", "ForStmt", "ReturnStmt", "YieldStmt",
    "ImportStmt", "ThreadStmt", "UnsafeStmt", "SafeStmt", "BorrowStmt",
    "ReleaseStmt", "MoveStmt", "BreakStmt", "ContinueStmt", "RaiseStmt",
    "MatchStmt", "WithStmt", "AssertStmt", "DelStmt", "GlobalStmt",
    "NonlocalStmt", "PassStmt", "DoWhileStmt", "SwitchStmt", "GotoStmt",
    "LabelStmt", "InlineAsmStmt", "StaticAssertStmt", "FunctionDef",
    "ClassDef", "ExprStmt", "ExpressionStmt",
}
...
class Interpreter:
    ...
    def setup_builtins(self):
        ...
        def builtin_list(*args):
            if len(args) == 1:
                a = args[0]
                if isinstance(a, (str, bytes, list, tuple, set, frozenset, dict, range)) or hasattr(a, "__iter__"):
                    return list(a)
                return [a]
            return list(args)
        ...
        def builtin_ptr_read(addr, size=8):
            ...
            # Handle mmap objects - read directly
            if isinstance(addr, type(mmap)) or hasattr(addr, "write"):
                result = 0
                for i in range(size):
                    result |= addr[i] << (i * 8)
                return result
        ...
        def builtin_ptr_write(addr, value, size=8):
            ...
            # Handle mmap objects - write directly
            if isinstance(addr, type(mmap)) or hasattr(addr, "write"):
                for i in range(size):
        ...
        def builtin_memcpy(*args):
            """Copy memory — supports both memcpy(dest, src, size) and memcpy(dest, d_off, src, s_off, size)"""
            if len(args) == 3:
                dest_addr, src_addr, size = args
                d_off, s_off = 0, 0
            elif len(args) >= 5:
                dest_addr, d_off, src_addr, s_off, size = args[:5]
            else:
                return
            if (
        ...
        def builtin_syscall(number, *args):
            """Execute syscall — translates x86-64 Linux numbers to current platform"""
            if not self.in_unsafe_block:
            ...
            # --- Linux x86_64 → Linux ARM64 translation ---
            _AARCH64_MAP = {
                0: 63, 1: 64, 2: 56, 3: 57, 4: 4, 5: 56, 6: 57, 7: 59,
                8: 62, 9: 222, 10: 226, 11: 215, 12: 45, 13: 233,
                35: 101, 39: 172, 41: 198, 42: 203, 43: 202, 44: 206,
                45: 207, 48: 134, 49: 204, 50: 205, 51: 201,
                56: 57, 57: 57, 59: 221, 60: 93, 61: 260,
                62: 129, 82: 82, 83: 34, 84: 35, 87: 76,
                88: 90, 90: 92, 92: 94, 96: 169,
                102: 175, 110: 173, 186: 178, 201: 196,
            }

            # --- Linux x86_64 → macOS BSD numbers ---
            # macOS C library's syscall() adds 0x2000000 internally.
            # These are the raw BSD numbers from <sys/syscall.h>.
            _MACOS_MAP = {
                0: 3,   1: 4,   2: 5,   3: 6,   4: 188, 5: 189, 6: 190,
                7: 7,   8: 199, 9: 197, 10: 74,  11: 73,  12: 0,  # brk: no equivalent
                35: 240, 39: 20, 41: 97, 42: 98, 43: 30, 44: 31, 45: 32,
                48: 134, 49: 104, 50: 106, 51: 107, 52: 118, 53: 135,
                56: 2, 57: 2, 58: 66, 59: 59, 60: 1, 61: 7,
                62: 37, 63: 202, 64: 203, 72: 204, 73: 205, 74: 33,
                78: 181, 79: 186, 80: 187, 82: 128, 83: 136, 84: 137,
                87: 10, 88: 117, 89: 118, 90: 15, 91: 127, 92: 123,
                96: 116, 102: 174, 104: 105, 105: 104, 107: 91,
                110: 47, 131: 169, 132: 170, 133: 171, 134: 119,
                156: 54, 157: 55, 158: 56, 160: 58, 161: 59,
                186: 122, 200: 198, 201: 344, 202: 345, 203: 346,
                204: 347, 205: 348, 206: 349, 207: 350,
                210: 438, 211: 439, 212: 440, 213: 441,
                214: 443, 215: 444, 216: 445, 217: 446,
                218: 447, 219: 448, 228: 322, 229: 324, 230: 325,
                231: 326, 232: 327, 233: 328, 234: 329, 235: 330,
                236: 331, 237: 332, 238: 333, 239: 334, 240: 240,
                254: 340, 255: 341, 256: 342, 257: 343,
            }

            # --- Linux x86_64 → Windows Nt* index mapping ---
            # The Windows backend (lowlevel_support.py) expects these indices.
            _WINDOWS_MAP = {
                0: 0,   # READ    → NtReadFile (via read() wrapper)
                1: 1,   # WRITE   → NtWriteFile (via write() wrapper)
                2: 2,   # OPEN    → NtCreateFile (not directly usable; use FFI)
                3: 3,   # CLOSE   → NtClose
                4: 15,  # STAT    → NtQueryFullAttributesFile
                8: 16,  # LSEEK   → NtSetInformationFile
                9: 6,   # MMAP    → NtCreateSection+NtMapViewOfSection (emulated)
                10: 8,  # MPROTECT→ NtProtectVirtualMemory
                11: 7,  # MUNMAP  → NtUnmapViewOfSection
                35: 21, # NANOSLEEP→ NtDelayExecution
                39: 5,  # GETPID  → NtQueryInformationProcess
                41: 9,  # SOCKET  → Winsock (not direct Nt*)
                42: 10, # CONNECT → Winsock
                43: 13, # ACCEPT  → Winsock
                44: 1,  # SENDTO  → write-like
                45: 0,  # RECVFROM→ read-like
                48: 23, # SHUTDOWN→ Winsock
                49: 11, # BIND    → Winsock
                50: 12, # LISTEN  → Winsock
                56: 2,  # CLONE   → no equivalent
                57: 2,  # FORK    → no equivalent (Windows has no fork)
                59: 2,  # EXECVE  → no equivalent (use CreateProcess)
                60: 4,  # EXIT    → NtTerminateProcess
                62: 14, # KILL    → NtTerminateProcess(target)
                82: 20, # RENAME  → NtSetInformationFile
                83: 17, # MKDIR   → NtCreateFile
                84: 18, # RMDIR   → NtSetInformationFile
                87: 19, # UNLINK  → NtSetInformationFile
                88: 17, # STATFS  → no direct, use NtQueryVolumeInformationFile
                89: 17, # FSTATFS → no direct
                90: 24, # CHMOD   → NtSetInformationFile (FileBasicInformation)
                92: 24, # CHOWN   → not POSIX uid/gid on Windows
                96: 22, # GETTIMEOFDAY → NtQuerySystemTime
                134: 23,# SHUTDOWN→ Winsock
            }

            # --- Apply platform translation ---
            if _platform == "darwin":
                if number in _MACOS_MAP:
                    number = _MACOS_MAP[number]
                else:
                    # Unknown number — pass through, macOS may handle it
                    pass
            elif _platform == "win32":
                if number in _WINDOWS_MAP:
                    number = _WINDOWS_MAP[number]
                else:
                    raise RuntimeError(
                        f"Syscall #{number} has no Windows equivalent. "
                        f"Windows does not support raw syscalls. "
                        f"Use FFI or ctypes for Windows API access."
                    )
            elif _platform == "linux":
                if machine in ("aarch64", "arm64") and number in _AARCH64_MAP:
                    number = _AARCH64_MAP[number]
            else:
                raise RuntimeError(f"Syscalls not supported on {_platform}")
        ...
        def builtin_inl(port):
            """Read dword from I/O port"""
            self.require_unsafe("I/O port read")
            if not self.KSHardwareIO:
                return 0
            try:
                return self.KSHardwareIO.inl(port)
            except AttributeError:
                return 0

        def builtin_outl(port, value):
            """Write dword to I/O port"""
            self.require_unsafe("I/O port write")
            if not self.KSHardwareIO:
                return
            try:
                self.KSHardwareIO.outl(port, value)
            except AttributeError:
                pass
        ...
        def _debug_get_frame(depth=0):
            import sys as _sf
            try:
                f = _sf._getframe(depth + 3)
                return {
        ...
        builtins = {
            ...
            "randint": lambda a, b=None: (
                __import__("random").randint(0, a) if b is None
                else __import__("random").randint(a, b)
            ),
            ...
            "asm": builtin_asm,
            "inb": builtin_inb,
            "_builtin_inb": builtin_inb,
            "outb": builtin_outb,
            "_builtin_outb": builtin_outb,
            "inw": builtin_inw,
            "_builtin_inw": builtin_inw,
            "outw": builtin_outw,
            "_builtin_outw": builtin_outw,
            "_builtin_inl": builtin_inl,
            "_builtin_outl": builtin_outl,
            "rdtsc": builtin_rdtsc,
            # ===== TYPE NAMES (formerly keywords, now builtins like Python) =====
            "i8": int, "i16": int, "i32": int, "i64": int,
            "u8": int, "u16": int, "u32": int, "u64": int,
            "f32": float, "f64": float,
            "bool": bool, "str": builtin_str, "char": str, "void": type(None),
            ...
            "debug_get_frame": _debug_get_frame,
            "time_time": lambda: __import__("time").time(),
            "pass": None,
        ...
        _posix_constants = {
            ...
            "system_mmap": lambda addr, length, prot, flags, fd, offset: (
                _mmap_mod.mmap(fd, length, flags=flags, prot=prot, offset=offset)
            ),
            "system_munmap": lambda ptr, size: (
                ptr.close() if hasattr(ptr, "close") else None
            ),
            "mmap": lambda addr, length, prot, flags, fd, offset: (
                _mmap_mod.mmap(fd, length, flags=flags, prot=prot, offset=offset)
            ),
            "munmap": lambda ptr, size: (
                ptr.close() if hasattr(ptr, "close") else None
            ),
        }
        ...
        def system_jwt_encode(payload, secret, algorithm="HS256"):
            import json as _json
            import hmac as _hmac

            header = _json.dumps({"alg": algorithm, "typ": "JWT"}, separators=(",", ":"))
            ...
            sig = _hmac.new(secret_bytes, message.encode(), "sha256").digest()
            sig_b64 = _b64url_encode(sig)
            return f"{header_b64}.{payload_b64}.{sig_b64}"

        def system_jwt_decode(token, secret):
            import json as _json
            import hmac as _hmac
            ...
            secret_bytes = secret.encode() if isinstance(secret, str) else secret
            expected_sig = _hmac.new(secret_bytes, f"{header_b64}.{payload_b64}".encode(), "sha256").digest()
        ...
        def system_smtp_send(host, port, user, password, from_addr, to_addrs, subject, body, use_tls=True):
            try:
                ...
                if use_tls:
                    ...
                    server = _smtplib.SMTP_SSL(host, port) if port == 465 else _smtplib.SMTP(host, port)
        ...
        def system_smtp_send_html(host, port, user, password, from_addr, to_addrs, subject, html_body, use_tls=True):
            try:
                import smtplib as _smtplib
                ...
                msg["From"] = from_addr
                msg["To"] = ", ".join(to_addrs) if isinstance(to_addrs, list) else to_addrs
                msg.attach(_MIMEText("This email requires HTML support.", "plain", "utf-8"))
        ...
        def system_imap_fetch(host, user, password, mailbox="INBOX", limit=10, ssl=True):
            try:
                import imaplib as _imaplib

                if ssl:
                    conn = _imaplib.IMAP4_SSL(host)
                else:
                    conn = _imaplib.IMAP4(host)
        ...
        def system_xlsx_read(path, sheet=0):
            try:
                ...
                if "xl/sharedStrings.xml" in z.namelist():
                    ss_xml = _ET.parse(z.open("xl/sharedStrings.xml"))
                    for i, si in enumerate(ss_xml.getroot()):
                        t = si.find("{http://schemas.openxmlformats.org/spreadsheetml/2006/main}t")
                        shared[i] = t.text if t is not None else ""

                # Find sheet
                workbook = _ET.parse(z.open("xl/workbook.xml"))
                ns = {"s": "http://schemas.openxmlformats.org/spreadsheetml/2006/main",
                      "r": "http://schemas.openxmlformats.org/officeDocument/2006/relationships"}
                sheets = workbook.getroot().findall(".//s:sheet", ns)
                sheet_name = None
                if isinstance(sheet, int):
                    sheet_name = sheets[sheet].get("name") if sheet < len(sheets) else None
                else:
                    for sh in sheets:
                ...
                # Get sheet file
                rels = _ET.parse(z.open("xl/_rels/workbook.xml.rels"))
                sheet_path = None
                ...
                if sheet_path is None or sheet_path not in z.namelist():
                    return {"success": False, "error": "Sheet file not found"}

                sheet_xml = _ET.parse(z.open(sheet_path))
                ...
                z.close()
                return {"success": True, "data": rows_data}
            except Exception as e:
                return {"success": False, "error": str(e)}

        def system_xlsx_write(path, data, sheet_name="Sheet1"):
            try:
                import zipfile as _zipfile
                import xml.etree.ElementTree as _ET
                import io as _io
                ...
                lines.append(f'<worksheet xmlns="{ns}"><sheetData>')
                shared_strings = []
                ss_map = {}
                def get_ss(s):
                    s = str(s)
                ...
                for ri, row in enumerate(data):
                    ...
                    for ci, val in enumerate(row):
                        ref = col_str(ci) + str(ri + 1)
                        if isinstance(val, (int, float)):
                            lines.append(f'<c r="{ref}" t="n"><v>{val}</v></c>')
                        else:
                            si = get_ss(str(val))
                ...
                ss_lines = ['<?xml version="1.0" encoding="UTF-8"?>',
                            f'<sst xmlns="{ns}" count="{len(shared_strings)}" uniqueCount="{len(shared_strings)}">']
                ...
                # Build workbook XML
                wb_xml = f"""<?xml version="1.0" encoding="UTF-8"?>
...
<Relationship Id="rId2" Type="http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties" Target="docProps/core.xml"/>
                ...
                wb_rels_xml = f"""<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="{rns}">
...
<dc:creator>KentScript</dc:creator>
<dcterms:created xsi:type="dcterms:W3CDTF">2025-01-01T00:00:00Z</dcterms:created>
</cp:coreProperties>"""

                z = _zipfile.ZipFile(path, "w", _zipfile.ZIP_DEFLATED)
        ...
        def system_markdown_to_html(text):
            try:
                import re as _re
                ...
                # Images
                html = _re.sub(r"!\[([^\]]*)\]\(([^)]+)\)", r'<img src="\2" alt="\1">', html)
                # Unordered lists
        ...
        # Krypton/XChaCha20-Poly1305 AEAD functions
        def system_kcrypt_xchacha20_encrypt(data, key, nonce=None, aad=None):
            try:
                import base64 as _b64
                from nacl.bindings import crypto_aead_xchacha20poly1305_ietf_encrypt as _encrypt_fn
                ...
                if len(key_bytes) == 32:
                    ...
                    key_bytes = _hashlib.sha256(key_bytes).digest()

                if nonce is None:
                    nonce = _nacl_random(24)
                elif isinstance(nonce, str):
                    nonce = nonce.encode()
                nonce = nonce[:24].ljust(24, b"\x00")

                data_bytes = data.encode() if isinstance(data, str) else data
                aad_bytes = aad.encode() if isinstance(aad, str) and aad else b""

                ct = _encrypt_fn(
                    data_bytes, aad_bytes, nonce, key_bytes
                )
                result = nonce + ct
                ...
                raise RuntimeError(f"kcrypt.encrypt failed: {e}")

        def system_kcrypt_xchacha20_decrypt(data, key, nonce=None, aad=None):
            try:
                import base64 as _b64
                from nacl.bindings import crypto_aead_xchacha20poly1305_ietf_decrypt as _decrypt_fn
                import hashlib as _hashlib
                ...
                data_bytes = _b64.b64decode(data.encode() if isinstance(data, str) else data)
                nonce_from_data = data_bytes[:24]
                ct = data_bytes[24:]
                aad_bytes = aad.encode() if isinstance(aad, str) and aad else b""

                pt = _decrypt_fn(
                    ct, aad_bytes, nonce_from_data, key_bytes
                )
                return pt.decode()
            except ImportError:
                raise RuntimeError(
                ...
                raise RuntimeError(f"kcrypt.decrypt failed: {e}")

        # Argon2 key derivation
        def system_kcrypt_derive_key(password, salt, length=32):
            try:
                import hashlib as _hashlib
                import base64 as _b64
                ...
                dk = _hashlib.scrypt(
                    pw_bytes, salt=salt_bytes, n=16384, r=8, p=1, dklen=length
                )
                return _b64.b64encode(dk).decode()
            except Exception as e:
                raise RuntimeError(f"kcrypt.derive_key failed: {e}")

        # Generate a random key
        def system_kcrypt_random_key(length=32):
            try:
                import base64 as _b64
                from nacl.utils import random as _nacl_random
                return _b64.b64encode(_nacl_random(length)).decode()
            except ImportError:
                try:
                    import secrets as _secrets
                    import base64 as _b64
                    return _b64.b64encode(_secrets.token_bytes(length)).decode()
                except Exception as e:
                    raise RuntimeError(f"kcrypt.random_key failed: {e}")

        # Helper: convert int to 4 big-endian bytes
        def system_kcrypt_int_to_bytes(n):
            return n.to_bytes(4, "big")

        # Helper: convert 4 big-endian bytes to int
        def system_kcrypt_bytes_to_int(b):
            if isinstance(b, str):
                b = b.encode()
            return int.from_bytes(b[:4], "big")

        # Helper: convert string to lowercase
        def system_kcrypt_lower(s):
            return s.lower()

        # --- kcrypt password hashing (Argon2id, branded $kcrypt$ format) ---
        # Format: $kcrypt$<year>$pyLord$<cost>$<salt22>$<payload32>
        #   <year> is the current calendar year (from the system clock)
        #   <salt>    = 16 random bytes -> 22 chars (bcrypt-variant base64)
        #   <payload> = 24 derived bytes -> 32 chars (bcrypt-variant base64)
        _KCRYPT_B64 = "./ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
        _KCRYPT_B64_INV = {c: i for i, c in enumerate(_KCRYPT_B64)}

        def _kcrypt_b64_encode(data):
            out = []
            n = len(data)
            i = 0
            while i < n:
                ...
                t = (b0 << 16) | (b1 << 8) | b2
                out.append(_KCRYPT_B64[(t >> 18) & 0x3F])
                out.append(_KCRYPT_B64[(t >> 12) & 0x3F])
                if i + 1 < n:
                    out.append(_KCRYPT_B64[(t >> 6) & 0x3F])
                if i + 2 < n:
                    out.append(_KCRYPT_B64[t & 0x3F])
                i += 3
            return "".join(out)

        def _kcrypt_b64_decode(s):
            out = bytearray()
            n = len(s)
            i = 0
            while i < n:
                c0 = _KCRYPT_B64_INV[s[i]]
                c1 = _KCRYPT_B64_INV[s[i + 1]] if i + 1 < n else 0
                c2 = _KCRYPT_B64_INV[s[i + 2]] if i + 2 < n else 0
                c3 = _KCRYPT_B64_INV[s[i + 3]] if i + 3 < n else 0
        ...
        def _kcrypt_cost_params(c):
            import nacl.pwhash as _pw
            ml_max = getattr(_pw.argon2id, "MEMLIMIT_MAX", 2147483648)
            op_max = getattr(_pw.argon2id, "OPSLIMIT_MAX", 4294967295)
            # memlimit grows with cost but is hard-capped to 64 MiB to avoid OOM
            # (cost 8 -> 8 MiB, cost >= 11 -> 64 MiB cap).
            memlimit = 1 << min(c + 15, 26)
            if memlimit > ml_max:
                memlimit = ml_max
            # opslimit grows with cost but is hard-capped to 8 passes.
            opslimit = 1 << min(max(c - 7, 1), 3)
            if opslimit > op_max:
                opslimit = op_max
            return opslimit, memlimit

        def _kcrypt_ct_compare(a, b):
            if len(a) != len(b):
                return False
            r = 0
            for x, y in zip(a, b):
        ...
        def system_kcrypt_hash_password(password, cost=8):
            try:
                import os
                import time
                import nacl.pwhash as _pw
                c = _kcrypt_resolve_cost(cost)
                opslimit, memlimit = _kcrypt_cost_params(c)
                pw = password.encode() if isinstance(password, str) else password
                salt = os.urandom(16)
                payload = _pw.argon2id.kdf(24, pw, salt, opslimit=opslimit, memlimit=memlimit)
                salt_b64 = _kcrypt_b64_encode(salt)
                payload_b64 = _kcrypt_b64_encode(payload)
                # Year taken from the real system clock so the branded token
                # ages with time. Verification does not pin the year, so
                # tokens stay valid across years.
                year = time.localtime().tm_year
                return "$kcrypt$%d$pyLord$%02d$%s$%s" % (year, c, salt_b64, payload_b64)
            except ImportError:
                raise RuntimeError("pynacl not installed. Run: pip install pynacl")
            except Exception as e:
                raise RuntimeError("kcrypt.hash_password failed: %s" % e)

        def system_kcrypt_verify_password(hash_str, password):
            try:
                import nacl.pwhash as _pw
                parts = hash_str.split("$")
                if len(parts) != 7 or parts[1] != "kcrypt":
                    return False
                c = _kcrypt_resolve_cost(parts[4])
                opslimit, memlimit = _kcrypt_cost_params(c)
                salt = _kcrypt_b64_decode(parts[5])
                expected = _kcrypt_b64_decode(parts[6])
                pw = password.encode() if isinstance(password, str) else password
                derived = _pw.argon2id.kdf(24, pw, salt, opslimit=opslimit, memlimit=memlimit)
                return _kcrypt_ct_compare(derived, expected)
            except Exception:
                return False
        ...
        # OS functions
        def system_os_getenv(name=None, default=None):
            if name is None:
        ...
        class _KSStdin:
            def read(self, size=-1):
                return _sys.stdin.read() if size < 0 else _sys.stdin.read(size)
            def readline(self, size=-1):
                return _sys.stdin.readline() if size < 0 else _sys.stdin.readline(size)
            def readlines(self):
                return _sys.stdin.readlines()
            def __iter__(self):
                return iter(_sys.stdin)
            def __next__(self):
                return next(_sys.stdin)
            def fileno(self):
                return _sys.stdin.fileno()
            @property
            def closed(self):
                return _sys.stdin.closed
        ...
        class _KSStdout:
            ...
            def flush(self):
                _sys.stdout.flush()
            def fileno(self):
                return _sys.stdout.fileno()
            @property
            def closed(self):
                return _sys.stdout.closed

        class _KSStderr:
            def write(self, data):
                _sys.stderr.write(str(data))
                return len(str(data))
            def flush(self):
                _sys.stderr.flush()
            def fileno(self):
                return _sys.stderr.fileno()
            @property
            def closed(self):
                return _sys.stderr.closed

        system_stdin = _KSStdin()
        system_stdout = _KSStdout()
        system_stderr = _KSStderr()

        def system_stdin_read():
            return _sys.stdin.read()

        def system_stdin_readline():
            return _sys.stdin.readline()

        def system_stdin_readlines():
            return _sys.stdin.readlines()

        def system_which(cmd):
            import shutil as _shutil
            return _shutil.which(cmd)

        def system_glob(pattern):
        ...
        def system_tempfile(suffix=None, prefix=None):
            import tempfile as _tempfile
            import os as _os
        ...
        def system_ffi_call(func_ptr_or_lib, *args):
            import ctypes as _ct
            _TYPE_MAP = {
                "void": None, "int": _ct.c_int, "uint": _ct.c_uint,
                "long": _ct.c_long, "ulong": _ct.c_ulong,
                "float": _ct.c_float, "double": _ct.c_double,
                "char": _ct.c_char, "char*": _ct.c_char_p, "void*": _ct.c_void_p,
                "bool": _ct.c_bool, "int8": _ct.c_int8, "uint8": _ct.c_uint8,
                "int16": _ct.c_int16, "uint16": _ct.c_uint16,
                "int32": _ct.c_int32, "uint32": _ct.c_uint32,
                "int64": _ct.c_int64, "uint64": _ct.c_uint64,
            }
            def _resolve_type(t):
                if t is None: return None
                if isinstance(t, str): return _TYPE_MAP.get(t, getattr(_ct, t, _ct.c_void_p))
                return t

            if callable(func_ptr_or_lib) and not isinstance(func_ptr_or_lib, type):
                fn = func_ptr_or_lib
                if len(args) >= 2 and isinstance(args[0], (list, tuple)) and isinstance(args[1], (list, tuple)):
                    call_args, argtypes, restype = args[0], args[1], args[2] if len(args) > 2 else None
                    fn.argtypes = [_resolve_type(t) for t in argtypes]
                    fn.restype = _resolve_type(restype)
                    converted = []
                    for a, t in zip(call_args, argtypes):
                        if t == "char*" and isinstance(a, str):
                            converted.append(a.encode())
                        else:
                            converted.append(a)
                    return fn(*converted)
                return fn(*args)
        ...
        # FFI builtins used by stdlib/ffi.ks
        def system_ffi_get_symbol(lib_handle, name):
            return getattr(lib_handle, name, None)

        def system_ffi_close(lib_handle):
            pass

        def system_ffi_string_at(addr, size):
            import ctypes as _ct
            if size is not None:
                return _ct.string_at(addr, size)
            return _ct.string_at(addr)

        def system_ffi_memmove(dst, src, count):
            import ctypes as _ct
            _ct.memmove(dst, src, count)

        def system_ffi_memset(dst, value, count):
            import ctypes as _ct
            _ct.memset(dst, value, count)

        def system_ffi_pack_struct(obj, fields):
            import ctypes as _ct
            return _ct.string_at(_ct.addressof(obj))

        def system_ffi_unpack_struct(data, fields):
            return data

        def system_ffi_pack_union(obj, fields):
            import ctypes as _ct
            return _ct.string_at(_ct.addressof(obj))

        def system_ffi_unpack_union(data, fields):
            return data

        def system_ffi_create_buffer(size, init):
            import ctypes as _ct
            if isinstance(init, str):
                buf = _ct.create_string_buffer(init.encode(), size)
            else:
                    ...
                    if handler:
                        ...
                        if qs:
                            for _p in qs.split("&"):
                                if "=" in _p:
                                    _k, _v = _p.split("=", 1)
                                    query[_up.unquote(_k)] = _up.unquote(_v)
                                else:
                                    query[_up.unquote(_p)] = ""
                        req = {
                            "method": self.command,
                            "path": path,
                            "query": query,
        ...
        def system_webserver_route(srv, method, path, handler):
            if isinstance(handler, Function):
                ...
                _handler = handler
                def _wrapper(req):
                    _env = Environment(_handler.closure)
                    if _handler.params:
                        _env.define(_handler.params[0], req)
                    try:
                        for _s in _handler.body:
                            _interp.eval(_s, _env)
                    except ReturnException as _e:
                        return _e.value
                    return None
                srv["routes"][(method.upper(), path)] = _wrapper
            else:
        ...
        # --- PostgreSQL full interface ---
        def system_database_postgres_execute(conn, query, params=()):
            cur = conn.cursor()
            cur.execute(query, params)
            return cur

        def system_database_postgres_fetchone(cur):
            return cur.fetchone()

        def system_database_postgres_fetchall(cur):
            return cur.fetchall()

        def system_database_postgres_fetchmany(cur, size=10):
            return cur.fetchmany(size)

        def system_database_postgres_commit(conn):
            conn.commit()

        def system_database_postgres_rollback(conn):
            conn.rollback()

        def system_database_postgres_close(conn):
            conn.close()

        def system_database_postgres_cursor(conn):
            return conn.cursor()

        def system_database_postgres_description(cur):
            return cur.description

        def system_database_postgres_rowcount(cur):
            return cur.rowcount

        def system_database_postgres_executemany(conn, query, params_list):
            cur = conn.cursor()
            cur.executemany(query, params_list)
            return cur

        def system_database_postgres_copy_from(conn, table, data, sep="\t"):
            cur = conn.cursor()
            import io as _io
            buf = _io.StringIO(data)
            cur.copy_expert("COPY " + table + " FROM STDIN WITH (FORMAT CSV, DELIMITER '" + sep + "')", buf)
            return cur.rowcount

        # --- MySQL full interface ---
        def system_database_mysql_execute(conn, query, params=()):
            cur = conn.cursor()
            cur.execute(query, params)
            return cur

        def system_database_mysql_fetchone(cur):
            return cur.fetchone()

        def system_database_mysql_fetchall(cur):
            return cur.fetchall()

        def system_database_mysql_fetchmany(cur, size=10):
            return cur.fetchmany(size)

        def system_database_mysql_commit(conn):
            conn.commit()

        def system_database_mysql_rollback(conn):
            conn.rollback()

        def system_database_mysql_close(conn):
            conn.close()

        def system_database_mysql_cursor(conn):
            return conn.cursor()

        def system_database_mysql_description(cur):
            return cur.description

        def system_database_mysql_rowcount(cur):
            return cur.rowcount

        def system_database_mysql_executemany(conn, query, params_list):
            cur = conn.cursor()
            cur.executemany(query, params_list)
            return cur

        def system_database_mysql_lastrowid(cur):
            return cur.lastrowid

        # --- MariaDB full interface (same driver as MySQL) ---
        def system_database_mariadb_connect(host, port, database, user, password):
            try:
                ...
                try:
                    ...
                    return {"error": "mariadb or mysql-connector not installed. Run: pip install mariadb"}

        def system_database_mariadb_execute(conn, query, params=()):
            cur = conn.cursor()
            cur.execute(query, params)
            return cur

        def system_database_mariadb_fetchone(cur):
            return cur.fetchone()

        def system_database_mariadb_fetchall(cur):
            return cur.fetchall()

        def system_database_mariadb_fetchmany(cur, size=10):
            return cur.fetchmany(size)

        def system_database_mariadb_commit(conn):
            conn.commit()

        def system_database_mariadb_rollback(conn):
            conn.rollback()

        def system_database_mariadb_close(conn):
            conn.close()

        def system_database_mariadb_cursor(conn):
            return conn.cursor()

        def system_database_mariadb_description(cur):
            return cur.description

        def system_database_mariadb_rowcount(cur):
            return cur.rowcount

        def system_database_mariadb_executemany(conn, query, params_list):
            cur = conn.cursor()
            cur.executemany(query, params_list)
            return cur

        def system_database_mariadb_lastrowid(cur):
            return cur.lastrowid
        ...
        # SIMD vector operations  [KS-SIMD-001] REAL: NumPy-backed data-parallel
        def _ks_np2(a, b, dtype, op):
            try:
                import numpy as np

                xa = np.array(a, dtype=dtype)
                xb = np.array(b, dtype=dtype)
                if op == "add":
                    r = xa + xb
                elif op == "sub":
                    r = xa - xb
        ...
        def _ks_np1(v, dtype, op):
            try:
                ...
                if op == "sqrt":
                    return [math.sqrt(t) for t in v]

        def system_simd_add_i32(a, b):
            return _ks_np2(a, b, "int32", "add")

        def system_simd_sub_i32(a, b):
            return _ks_np2(a, b, "int32", "sub")

        def system_simd_mul_i32(a, b):
            return _ks_np2(a, b, "int32", "mul")

        def system_simd_add_f32(a, b):
            return _ks_np2(a, b, "float32", "add")

        def system_simd_sub_f32(a, b):
            return _ks_np2(a, b, "float32", "sub")

        def system_simd_mul_f32(a, b):
            return _ks_np2(a, b, "float32", "mul")

        def system_simd_div_f32(a, b):
            return _ks_np2(a, b, "float32", "div")

        def system_simd_sqrt_f32(v):
            return _ks_np1(v, "float32", "sqrt")

        def system_simd_hadd_f32(a, b):
            return [a[0] + a[1], a[2] + a[3], b[0] + b[1], b[2] + b[3]]

        def system_simd_max_f32(a, b):
            return _ks_np2(a, b, "float32", "max")

        def system_simd_min_f32(a, b):
            return _ks_np2(a, b, "float32", "min")
        ...
        # AVX2 256-bit operations  [KS-SIMD-002] REAL: NumPy data-parallel
        def system_simd256_add_f32(a, b):
            return _ks_np2(a, b, "float32", "add")

        def system_simd256_add_f64(a, b):
            return _ks_np2(a, b, "float64", "add")
        ...
        # AVX-512 512-bit vector operations  [KS-SIMD-002] REAL: NumPy data-parallel
        ...
        def system_simd512_mul_f64(a, b):
            return _ks_np2(a, b, "float64", "mul")

        def system_simd512_sqrt_f32(v):
            return _ks_np1(v, "float32", "sqrt")

        def system_simd512_sqrt_f64(v):
            return _ks_np1(v, "float64", "sqrt")
        ...
        # ARM NEON vector operations  [KS-SIMD-003] REAL: NumPy data-parallel
        def system_neon_add_u8(a, b):
            return _ks_np2(a, b, "uint8", "add")

        def system_neon_add_u16(a, b):
            return _ks_np2(a, b, "uint16", "add")

        def system_neon_add_u32(a, b):
            return _ks_np2(a, b, "uint32", "add")
        ...
        def file_search(path, pattern):
            content = system_file_read_text(path)
            lines = content.split("\n")
            result = []
        ...
        system_funcs = {
            ...
            "system_jwt_encode": system_jwt_encode,
            "system_jwt_decode": system_jwt_decode,
            ...
            "system_kcrypt_xchacha20_encrypt": system_kcrypt_xchacha20_encrypt,
            "system_kcrypt_xchacha20_decrypt": system_kcrypt_xchacha20_decrypt,
            "system_kcrypt_derive_key": system_kcrypt_derive_key,
            "system_kcrypt_random_key": system_kcrypt_random_key,
            "system_kcrypt_int_to_bytes": system_kcrypt_int_to_bytes,
            "system_kcrypt_bytes_to_int": system_kcrypt_bytes_to_int,
            "system_kcrypt_lower": system_kcrypt_lower,
            "system_kcrypt_hash_password": system_kcrypt_hash_password,
            "system_kcrypt_verify_password": system_kcrypt_verify_password,
            ...
            "system_stdin": system_stdin,
            "system_stdout": system_stdout,
            "system_stderr": system_stderr,
            "system_stdin_read": system_stdin_read,
            "system_stdin_readline": system_stdin_readline,
            "system_stdin_readlines": system_stdin_readlines,
            "system_which": system_which,
            "system_glob": system_glob,
            "system_rmtree": system_rmtree,
            "system_copytree": system_copytree,
            "system_tempdir": system_tempdir,
            "system_tempfile": system_tempfile,
            ...
            "system_strings_contains": system_str_contains,
            "system_str_startswith": system_str_startswith,
            "system_strings_startswith": system_str_startswith,
            "system_str_endswith": system_str_endswith,
            "system_strings_endswith": system_str_endswith,
            "system_str_split": system_str_split,
            "system_strings_split": system_str_split,
            "system_str_join": system_str_join,
            "system_strings_join": system_str_join,
            "system_str_strip": system_str_strip,
            "system_strings_strip": system_str_strip,
            "system_str_lstrip": system_str_lstrip,
            "system_str_rstrip": system_str_rstrip,
            "system_str_replace": system_str_replace,
            "system_strings_replace": system_str_replace,
            "system_str_upper": system_str_upper,
            "system_strings_upper": system_str_upper,
            "system_str_lower": system_str_lower,
            "system_strings_lower": system_str_lower,
            "system_str_title": system_str_title,
            "system_strings_title": system_str_title,
            "system_str_capitalize": system_str_capitalize,
            "system_strings_capitalize": system_str_capitalize,
            "system_str_swapcase": system_str_swapcase,
            "system_strings_swapcase": system_str_swapcase,
            ...
            "mprotect": system_memory_mprotect,
            ...
            "system_ffi_get_symbol": system_ffi_get_symbol,
            "system_ffi_close": system_ffi_close,
            "system_ffi_string_at": system_ffi_string_at,
            "system_ffi_memmove": system_ffi_memmove,
            "system_ffi_memset": system_ffi_memset,
            "system_ffi_pack_struct": system_ffi_pack_struct,
            "system_ffi_unpack_struct": system_ffi_unpack_struct,
            "system_ffi_pack_union": system_ffi_pack_union,
            "system_ffi_unpack_union": system_ffi_unpack_union,
            "system_ffi_create_buffer": system_ffi_create_buffer,
            ...
            # Email
            "system_smtp_send": system_smtp_send,
            "system_smtp_send_html": system_smtp_send_html,
            "system_imap_fetch": system_imap_fetch,
            "system_markdown_to_html": system_markdown_to_html,
            "system_xlsx_read": system_xlsx_read,
            "system_xlsx_write": system_xlsx_write,
            # Phase 8 extras
            ...
            "system_database_postgres_execute": system_database_postgres_execute,
            "system_database_postgres_fetchone": system_database_postgres_fetchone,
            "system_database_postgres_fetchall": system_database_postgres_fetchall,
            "system_database_postgres_fetchmany": system_database_postgres_fetchmany,
            "system_database_postgres_commit": system_database_postgres_commit,
            "system_database_postgres_rollback": system_database_postgres_rollback,
            "system_database_postgres_close": system_database_postgres_close,
            "system_database_postgres_cursor": system_database_postgres_cursor,
            "system_database_postgres_description": system_database_postgres_description,
            "system_database_postgres_rowcount": system_database_postgres_rowcount,
            "system_database_postgres_executemany": system_database_postgres_executemany,
            "system_database_postgres_copy_from": system_database_postgres_copy_from,
            "system_database_mysql_connect": system_database_mysql_connect,
            "system_database_mysql_execute": system_database_mysql_execute,
            "system_database_mysql_fetchone": system_database_mysql_fetchone,
            "system_database_mysql_fetchall": system_database_mysql_fetchall,
            "system_database_mysql_fetchmany": system_database_mysql_fetchmany,
            "system_database_mysql_commit": system_database_mysql_commit,
            "system_database_mysql_rollback": system_database_mysql_rollback,
            "system_database_mysql_close": system_database_mysql_close,
            "system_database_mysql_cursor": system_database_mysql_cursor,
            "system_database_mysql_description": system_database_mysql_description,
            "system_database_mysql_rowcount": system_database_mysql_rowcount,
            "system_database_mysql_executemany": system_database_mysql_executemany,
            "system_database_mysql_lastrowid": system_database_mysql_lastrowid,
            "system_database_mariadb_connect": system_database_mariadb_connect,
            "system_database_mariadb_execute": system_database_mariadb_execute,
            "system_database_mariadb_fetchone": system_database_mariadb_fetchone,
            "system_database_mariadb_fetchall": system_database_mariadb_fetchall,
            "system_database_mariadb_fetchmany": system_database_mariadb_fetchmany,
            "system_database_mariadb_commit": system_database_mariadb_commit,
            "system_database_mariadb_rollback": system_database_mariadb_rollback,
            "system_database_mariadb_close": system_database_mariadb_close,
            "system_database_mariadb_cursor": system_database_mariadb_cursor,
            "system_database_mariadb_description": system_database_mariadb_description,
            "system_database_mariadb_rowcount": system_database_mariadb_rowcount,
            "system_database_mariadb_executemany": system_database_mariadb_executemany,
            "system_database_mariadb_lastrowid": system_database_mariadb_lastrowid,
            ...
            # [KS-SIMD] Real SIMD vector builtins (NumPy data-parallel; NEON/AVX/AVX-512 in compiled C)
            "system_simd_add_i32": system_simd_add_i32,
            "system_simd_sub_i32": system_simd_sub_i32,
            "system_simd_mul_i32": system_simd_mul_i32,
            "system_simd_add_f32": system_simd_add_f32,
            "system_simd_sub_f32": system_simd_sub_f32,
            "system_simd_mul_f32": system_simd_mul_f32,
            "system_simd_div_f32": system_simd_div_f32,
            "system_simd_sqrt_f32": system_simd_sqrt_f32,
            "system_simd_hadd_f32": system_simd_hadd_f32,
            "system_simd_max_f32": system_simd_max_f32,
            "system_simd_min_f32": system_simd_min_f32,
            "system_simd_load_f32": system_simd_load_f32,
            "system_simd_store_f32": system_simd_store_f32,
            "system_simd_set1_f32": system_simd_set1_f32,
            "system_simd_zero": system_simd_zero,
            "system_simd256_add_f32": system_simd256_add_f32,
            "system_simd256_add_f64": system_simd256_add_f64,
            "system_simd256_mul_f32": system_simd256_mul_f32,
            "system_simd256_sqrt_f32": system_simd256_sqrt_f32,
            "system_simd512_add_f32": system_simd512_add_f32,
            "system_simd512_add_f64": system_simd512_add_f64,
            "system_simd512_mul_f32": system_simd512_mul_f32,
            "system_simd512_mul_f64": system_simd512_mul_f64,
            "system_simd512_sqrt_f32": system_simd512_sqrt_f32,
            "system_simd512_sqrt_f64": system_simd512_sqrt_f64,
            "system_simd512_max_f32": system_simd512_max_f32,
            "system_simd512_min_f32": system_simd512_min_f32,
            "system_neon_add_u8": system_neon_add_u8,
            "system_neon_add_u16": system_neon_add_u16,
            "system_neon_add_u32": system_neon_add_u32,
            "system_neon_add_f32": system_neon_add_f32,
            "system_neon_mul_f32": system_neon_mul_f32,
            "system_neon_mul_u32": system_neon_mul_u32,
            "system_neon_load_u8": system_neon_load_u8,
            "system_neon_store_u8": system_neon_store_u8,
            "system_vectorize_hint": system_vectorize_hint,
            "system_vectorize_enable": system_vectorize_enable,
            "system_vectorize_disable": system_vectorize_disable,
            # Phase 30 remaining
        ...
        try:
            ...
            for stmt in ast:
                if dbg is not None:
                    ...
                    if line is not None:
                        dbg.should_stop(line, self.global_env)
                if (
                    type(stmt).__name__ == "FunctionCall"
                    and type(getattr(stmt, "func", None)).__name__ == "Identifier"
                    and getattr(stmt.func, "name", None) == "main"
                ):
                    main_called_explicitly = True
                self.eval(stmt, self.global_env)
            # Auto-invoke `func main()` so the interpreter matches the C
            # backend, which uses `ks_user_main()` as the program entry point.
            # Skip when the source already called `main()` explicitly at the
            # top level — the C backend drops that redundant call, so running
            # it again here would double-execute the program.
            if not main_called_explicitly:
                self._auto_run_main()
            return True
    ...
    def _auto_run_main(self):
        """Call a top-level `func main()` if one is defined (C-backend parity)."""
        try:
        ...
        if main_fn is None:
            return
        if not (isinstance(main_fn, Function) or type(main_fn).__name__ == "Function"):
            return
        from compiler.parser.parser import FunctionCall, Identifier

        call = FunctionCall(Identifier("main"), [], {})
        call.line = getattr(getattr(main_fn, "node", None), "line", None)
        self.eval(call, self.global_env)

    # ---- Fast evaluator handlers -------------------------------------------------
    # These replicate the common numeric/simple cases of the general evaluator so
    # tight loops avoid the long type-dispatch isinstance chain. Any non-trivial
    # case (strings, Instances, pointers, destructuring, ...) falls back to the
    # general evaluator to preserve exact semantics.

    def _ev_fast_literal(self, node, env):
        val = node.value
        if isinstance(val, str):
            if val.startswith("0x") or val.startswith("0X"):
                return _to_i64(int(val, 16))
            elif val.startswith("0b") or val.startswith("0B"):
                return _to_i64(int(val, 2))
            elif val.startswith("0o") or val.startswith("0O"):
                return _to_i64(int(val, 8))
            return val
        if isinstance(val, int) and not isinstance(val, bool):
            return _to_i64(val)
        return val

    def _ev_fast_identifier(self, node, env):
        name = node.name
        if name in ("none", "None", "null"):
            return None
        if name not in self.borrow_checker.builtins:
            self.borrow_checker.check_access(name)
        try:
            return env.get(name)
        except NameError:
            KSError.name_error(
    ...
    def _ev_fast_binaryop(self, node, env):
        # Short-circuit 'and'/'or' (matching general evaluator).
        op = node.op
        if op == "and":
            left = self.eval(node.left, env)
            return left if not left else self.eval(node.right, env)
        if op == "or":
            left = self.eval(node.left, env)
            return left if left else self.eval(node.right, env)
        left = self.eval(node.left, env)
        right = self.eval(node.right, env)
        # Delegate anything involving instances/pointers/strings to the general
        # evaluator; only handle plain int/float arithmetic here.
        if (
            isinstance(left, Instance)
            or isinstance(right, Instance)
            or isinstance(left, str)
            or isinstance(right, str)
            or (isinstance(left, int) and isinstance(right, bool))
            or (isinstance(left, bool) and isinstance(right, int))
        ):
            self.current_env = env
            return self._eval_general(node, env)
        if isinstance(left, int) and isinstance(right, int):
            if op == "+":
                return _to_i64(left + right)
            if op == "-":
                return _to_i64(left - right)
            if op == "*":
                return _to_i64(left * right)
            if op == "/":
                return left / right
            if op == "//":
                return _i64_div(left, right)
            if op == "%":
                return _i64_mod(left, right)
            if op == "**":
                return _to_i64(int(left ** right))
            if op == "==":
                return left == right
            if op == "!=":
                return left != right
        ...
        if isinstance(left, (int, float)) and isinstance(right, (int, float)):
            ...
            if op == ">=":
                return left >= right
        # Non-prototyped case (borrow, in, custom) -> general evaluator.
        self.current_env = env
        return self._eval_general(node, env)

    def _ev_fast_letdecl(self, node, env):
        if node.name.startswith(("__destructure__", "__tuple_destructure__")):
            self.current_env = env
            return self._eval_general(node, env)
        value = self.eval(node.value, env)
        self.borrow_checker.declare_ownership(node.name, env.scope_id)
        env.define(node.name, value, is_const=bool(getattr(node, "is_const", False)))
        return value

    def _ev_fast_assignment(self, node, env):
        target = node.target
        if type(target).__name__ != "Identifier" or getattr(node, "op", "=") != "=":
            self.current_env = env
            return self._eval_general(node, env)
        name = target.name
        if name not in self.borrow_checker.builtins:
            self.borrow_checker.check_access(name, mutable=True)
        value = self.eval(node.value, env)
        self.current_env = env
        env.set(name, value)
        return value

    def _ev_fast_forstmt(self, node, env):
        bc = self.borrow_checker
        # Only handle the plain single-variable iteration over a normal
        # iterable (e.g. a python range or list). Delegate if the body needs
        # destructuring, an else block, or the iterable is a class instance.
        if "," in getattr(node, "var", "") or getattr(node, "else_block", None):
            self.current_env = env
            return self._eval_general(node, env)
        iterable_obj = self.eval(node.iterable, env)
        if isinstance(iterable_obj, Instance):
            self.current_env = env
            return self._eval_general(node, env)
        ...
        try:
            for item in iterable_obj:
                local_env = Environment(env)
                bc.enter_scope(id(local_env))
                local_env.define(var, item)
                try:
                    for stmt in node.body:
                        self.eval(stmt, local_env)
    ...
    def eval(self, node: ASTNode, env: Environment) -> Any:
        # Fast path: no debugger attached (the normal, non-debug run). Avoids
        # the getattr + type-name set membership on every single node eval.
        if self.debugger is None:
            try:
                ...
                return self._eval_error(node, e)
        dbg = self.debugger
        if type(node).__name__ in _DEBUG_STMT_TYPES:
            line = getattr(node, "line", None)
            if line is not None:
                dbg.should_stop(line, env)
        try:
            return self._eval_impl(node, env)
        except (KentScriptNameError, KentScriptSyntaxError, KentScriptTypeError):
            raise
    ...
    def _eval_error(self, node, e):
        """Format+raise a runtime error from an unexpected eval exception."""
        line = getattr(node, "line", None)
    ...
    def _eval_impl(self, node: ASTNode, env: Environment) -> Any:
        # Fast dispatch: route the hottest node types through lean handlers and
        # fall back to the full general evaluator for everything else.
        mname = _EVAL_FAST.get(type(node))
        if mname is not None:
            return getattr(self, mname)(node, env)
        self.current_env = env
        return self._eval_general(node, env)

    def _eval_general(self, node: ASTNode, env: Environment) -> Any:
        self.current_env = env
        ...
        # ---------- IDENTIFIERS ----------
            ...
            # `none` / `None` / `null` are the null value (matches the C
            # backend, which transpiles all three to NULL). `none`/`None`
            # normally arrive as Literals; `null` is lexed as an identifier.
        ...
        # ---------- UNARY OPERATIONS ----------
            ...
            if node.op == "&":
                ...
                operand = self.eval(node.operand, env)
                if node.op == "-":
                    return _to_i64(-operand) if isinstance(operand, int) else -operand
                elif node.op == "not":
                    return not operand
                elif node.op == "~":
                    return _to_i64(~operand) if isinstance(operand, int) else ~operand
        ...
        # ---------- FOR LOOP ----------
            ...
            # Standard iteration
            try:
                iterable_for_loop = enumerate(iterable_obj) if "," in node.var else iterable_obj
                for item in iterable_for_loop:
                    local_env = Environment(env)
                    self.borrow_checker.enter_scope(id(local_env))
        ...
        # ---------- FUNCTION CALL ----------
            ...
            # Property accessed via call syntax (e.g. list.length()) — return value directly
            if (
                not callable(func)
                and not isinstance(func, (Function, Class))
                and not node.args
                and not node.kwargs
            ):
                return func
            ...
            if isinstance(func, Class):
                # Prefer the constructor attached to the class (correct `super` wiring)
                constructor = getattr(func, "_ks_constructor", None)
                if constructor and callable(constructor):
                    return constructor(*args, **kwargs)
                # Fallback: instantiate directly and call __init__/init/new
        ...
        # ---------- CLASS DEFINITION ----------
            ...
            # Evaluate static class variables
            if hasattr(node, "statics"):
                for sname, svalue in node.statics.items():
                    val = self.eval(svalue, env)
                    setattr(class_def, sname, val)
            ...
            def constructor(*args, **kwargs):
                ...
                if _init_name:
                        ...
                        class _SuperProxy:
                            def __getattribute__(self_, name):
                                if name in _parent_ref.methods:
                                    ...
                                    return _super_call
                                # Let Python builtins (repr, class, etc.) behave normally
                    ...
                    # Apply defaults (or None) for any parameter not supplied
                    for _p in init_params:
                        if _p not in local_env.vars:
                            if _p in init_method.defaults:
                                local_env.define(_p, self.eval(init_method.defaults[_p], local_env))
                            else:
                                local_env.define(_p, None)
            ...
            # Attach the constructor to the class itself so it is always callable
            # (with proper `super` wiring) regardless of which module invokes it.
            class_def._ks_constructor = constructor
        ...
        # ---------- STRUCT DEFINITION ----------
            ...
            # Store struct definition in environment as a callable constructor.
            # `Vec(1.0, 2.0)` and `Vec(x=1.0, y=2.0)` build a struct instance dict.
            def _struct_constructor(*args, **kwargs):
                inst = {"__type__": struct_name, "__struct__": True}
                for i, val in enumerate(args):
                    if i < len(struct_fields):
                        inst[struct_fields[i]] = val
                for k, v in kwargs.items():
                    inst[k] = v
                return inst

            _struct_constructor.__struct__ = True
            _struct_constructor.__fields__ = struct_fields
            _struct_constructor.__name__ = struct_name
            env.define(struct_name, _struct_constructor)
            return None
        ...
        # ---------- TRY/EXCEPT ----------
            ...
            try:
                ...
                caught = False
                _ks_inst = e.instance if isinstance(e, KentScriptException) else e
                _ks_name = e.ks_class if isinstance(e, KentScriptException) else type(e).__name__
                for exc_type, exc_var, except_body in node.except_blocks:
                    if (
                        exc_type is None
                        or exc_type == _ks_name
                        ...
                        local_env = Environment(env)
                        if exc_var:
                            local_env.define(exc_var, _ks_inst)
                    ...
                    # If exc_type looks like a variable name (lowercase), treat as catch-all binding
                        ...
                        caught = True
                        local_env = Environment(env)
                        local_env.define(exc_type, _ks_inst)
                        for stmt in except_body:
                            self.eval(stmt, local_env)
                        break
                if not caught:
        ...
        # ---------- RAISE ----------
            ...
            if node.exception:
                ...
                if isinstance(exc, type) and issubclass(exc, Exception):
                    ...
                    raise exc
                elif isinstance(exc, Instance):
                    raise KentScriptException(exc)
                else:
        ...
        # ---------- MATCH ----------
            ...
            def _run_match_body(body, bound_env):
                last_val = None
                for stmt in body:
                    last_val = self.eval(stmt, bound_env)
                return last_val
        ...
        # ---------- SWITCH STATEMENT ----------
        elif type(node).__name__ == "SwitchStmt":
            val = self.eval(node.expr, env)
            for pattern, body in node.cases:
                pv = None
                try:
                    pv = self.eval(pattern, env)
                except:
                    pv = pattern.value if hasattr(pattern, 'value') else pattern
                if val == pv:
                    for stmt in body:
                        self.eval(stmt, env)
                    return None
        ...
        # ---------- MOVE ----------
            ...
            target_cls = node.target.__class__.__name__ if node.target else ""
            if target_cls == "Identifier":
                target_name = node.target.name
                env.define(target_name, value)
                ...
                if isinstance(target_env, Environment):
                    ...
                    env.define(node.var, value)
            self.borrow_checker.move_ownership(node.var, id(env), id(env))
            return value
    ...
    def _get_module_patches(self, module_name: str) -> dict:
        ...
        if module_name == "fileio":
            ...
            patches = {
                ...
                "getenv": lambda k=None, d=None: dict(_p_os.environ) if k is None else _p_os.getenv(k, d),
                "getpid": _p_os.getpid,
                "stat": lambda p: {
            ...
            if module_name == "network":
                ...
                def _sk_connect_timeout(s, host, port, timeout):
                    try:
                        old = s.gettimeout()
                ...
                def _sk_send(s, data, flags=0):
                    try:
                        return s.send(data.encode() if isinstance(data, str) else data)
                    except Exception:
                        return -1
                ...
                patches = {
                    ...
                    "socket_create": lambda domain, type_, proto=0: _sk.socket(
                        int(domain), int(type_), int(proto)
                    ),
                    "socket_connect_timeout": _sk_connect_timeout,
                    "socket_send": _sk_send,
                    "socket_close": _sk_close,
                }
    ...
    def import_module(
        ...
        # 'asm' is a keyword — map to the assembly module
        if module_name == "asm":
            module_name = "assembly"

        # FFI is safe to import (unsafe only needed for actual FFI calls)
        ...
        if alias in self.modules and alias != "hardware":
        ...
        ks_file = None
        kpm_file = os_path.join(
            os_module.path.expanduser("~"), ".ks_packages", f"{module_name}.ks"
        )
        for cand in (
            kpm_file,
            os_path.join(_ks_base_dir, "stdlib", f"{module_name}.ks"),
            f"{module_name}.ks",
        ):
            if os_module.path.exists(cand):
                ks_file = cand
                break
        if ks_file and os_module.path.exists(ks_file):
            ...
            _ks_native = {
                ...
                "os_getenv": lambda k=None, d=None: dict(_ks_os.environ) if k is None else _ks_os.getenv(k, d),
            ...
            # Execute the module body with this module's source as the error
            # context (so runtime errors inside the module point at the module),
            # then restore the caller's context so subsequent errors in the
            # caller's code are not mis-attributed to this module.
            _saved_ctx = KSError.get_context()
            KSError.set_context(ks_file, code)
            try:
                for stmt in ast:
        ...
        # Built-in modules
            ...
            module_attrs = {
                ...
                "getenv": lambda k=None, d=None: dict(os_module.environ) if k is None else os_module.getenv(k, d),
        ...
        # Add HardwareAccess extras for hardware module loaded from .ks
        if module_name == "hardware":
            module_attrs.setdefault("write_port", HardwareAccess.write_port)
            module_attrs.setdefault("read_port", HardwareAccess.read_port)
            module_attrs.setdefault("write_mmio", HardwareAccess.write_mmio)
            module_attrs.setdefault("read_mmio", HardwareAccess.read_mmio)
            module_attrs.setdefault("write_memory", HardwareAccess.write_memory)
            module_attrs.setdefault("read_memory", HardwareAccess.read_memory)
            module_attrs.setdefault("request_dma_buffer", HardwareAccess.request_dma_buffer)
            module_attrs.setdefault("init_hardware_perms", HardwareAccess._init_permissions)
            module_attrs.setdefault("mmio_read8", lambda addr: HardwareAccess.read_mmio(addr, 1))
            module_attrs.setdefault("mmio_write8", lambda addr, val: HardwareAccess.write_mmio(addr, val, 1))
            module_attrs.setdefault("mmio_read16", lambda addr: HardwareAccess.read_mmio(addr, 2))
            module_attrs.setdefault("mmio_write16", lambda addr, val: HardwareAccess.write_mmio(addr, val, 2))
            module_attrs.setdefault("mmio_read32", lambda addr: HardwareAccess.read_mmio(addr, 4))
            module_attrs.setdefault("mmio_write32", lambda addr, val: HardwareAccess.write_mmio(addr, val, 4))
            module_attrs.setdefault("mmio_read64", lambda addr: HardwareAccess.read_mmio(addr, 8))
            module_attrs.setdefault("mmio_write64", lambda addr, val: HardwareAccess.write_mmio(addr, val, 8))
            module_attrs.setdefault("get_cpu_count", lambda: str(__import__("os").cpu_count()))
            module_attrs.setdefault("has_io_perms", lambda: HardwareAccess._initialized)
            module_attrs.setdefault("can_access_hardware", lambda: (
                __import__("os").geteuid() == 0
                if hasattr(__import__("os"), "geteuid") else True
            ))
            module_attrs.setdefault("is_linux", lambda: __import__("sys").platform.startswith("linux"))
            module_attrs.setdefault("is_macos", lambda: __import__("sys").platform == "darwin")
            module_attrs.setdefault("is_windows", lambda: __import__("sys").platform == "win32")
            module_attrs.setdefault("is_arm", lambda: "arm" in __import__("platform").machine().lower())
            module_attrs.setdefault("is_x86_64", lambda: "x86_64" in __import__("platform").machine() or "amd64" in __import__("platform").machine())
    ...
    def _init_builtin_modules(self):
        ...
        # ── security ─────────────────────────────────────────────────────
        # Loaded from stdlib/security.ks at import time (path/command/file
        # safety helpers). Native crypto/scan helpers are injected via
        # _get_module_patches("security") so both sets of functions coexist.
        ...
        self.modules["string"] = _ksmod(
            "string",
            {
                ...
                "ord": lambda s: ord(str(s)[0]) if str(s) else 0,
        ...
        try:
            ...
            self.modules["sys"] = _ksmod(
                "sys",
                {
                    ...
                    "getenv": lambda k=None, d=None: dict(__import__("os").environ) if k is None else __import__("os").getenv(k, d),
        ...
        self.modules["json"] = _ksmod(
            "json",
            {
                ...
                "parse": _json_mod.loads,
                "dumps": lambda obj, **kw: _json_mod.dumps(obj, **kw),
                "stringify": lambda obj, **kw: _json_mod.dumps(obj, **kw),
                "load": _json_mod.load,
                "dump": _json_mod.dump,
        ...
        # ── simd (real hardware-native vectorization) ─────────────────────
        import ks_core as _ks_core_simd

        _simd_obj = _ks_core_simd.SIMDAcceleration
        _simd_methods = {}
        for _m in (
            "has_sse2", "has_avx", "has_avx2", "has_avx512",
            "vectorized_add_i32", "vectorized_mul_f32",
            "alloc_f32", "alloc_f64", "alloc_i32", "alloc_i64",
            "free_f32", "free_f64", "free_i32", "free_i64",
            "get_f32", "get_f64", "get_i32", "get_i64",
            "set_f32", "set_f64", "set_i32", "set_i64",
            ...
            "scale_f32", "scale_f64", "scale_i32", "scale_i64",
            "addc_f32", "addc_f64", "addc_i32", "addc_i64",
            "fma_f32", "fma_f64", "fma_i32", "fma_i64",
            "sum_f32", "sum_f64", "sum_i32", "sum_i64",
            "dot_f32", "dot_f64", "dot_i32", "dot_i64",
            "arch", "width",
        ):
            _simd_methods[_m] = getattr(_simd_obj, _m)
        self.modules["simd"] = _ksmod("simd", _simd_methods)

        # ── gpu (real OpenCL acceleration; CPU SIMD fallback) ───────────
        # Mirrors ks_gpu.h: elementwise ops run on an OpenCL device when one
        # is present, otherwise fall back to the real NumPy/SIMD CPU path.
        _gpu_cl = {"ctx": None, "queue": None, "devname": "cpu-fallback", "tried": False}
        _GPU_KERNEL_CACHE = {}
        ...
        def _gpu_cl_init():
            ...
            try:
                ...
                if not devs:
                    return None
                _gpu_cl["ctx"] = cl.Context([devs[0]])
                _gpu_cl["queue"] = cl.CommandQueue(_gpu_cl["ctx"])
                _gpu_cl["devname"] = devs[0].name
                return _gpu_cl["ctx"]
            except Exception:
                return None

        def _gpu_available():
            return _gpu_cl_init() is not None

        def _gpu_name():
            ctx = _gpu_cl_init()
            return _gpu_cl["devname"] if ctx is not None else "cpu-fallback"

        def _gpu_cuda_available():
            try:
            ...
            try:
                ...
                try:
                    ...
                    if numba.cuda.is_available():
                        return numba.cuda.get_current_device().name
                except Exception:
                    pass
            return "cpu-fallback"
        ...
        _GPU_OPS = {"add": "+", "sub": "-", "mul": "*", "div": "/"}

        def _gpu_binop(kind, op, a, b, c, n):
            ctx = _gpu_cl_init()
            if ctx is None:
                # CPU SIMD fallback (real NumPy vectorization)
                getattr(_simd_obj, "%s_%s" % (op, kind))(a, b, c, n)
                return
            import pyopencl as cl
            import numpy as np
            ...
            dt = _GPU_DTYPES[kind]
            key = (dt, op)
            if key not in _GPU_KERNEL_CACHE:
                src = (
                    "__kernel void k(__global %s* a,__global %s* b,"
                    "__global %s* c){int i=get_global_id(0);"
                    "c[i]=a[i]%s b[i];}" % (dt, dt, dt, _GPU_OPS[op])
                )
                _GPU_KERNEL_CACHE[key] = cl.Program(ctx, src).build().k
            kern = _GPU_KERNEL_CACHE[key]
            a_np = np.asarray(a, dtype=dt)
            b_np = np.asarray(b, dtype=dt)
            c_np = np.empty(int(n), dtype=dt)
            mf = cl.mem_flags
            ...
            kern(_gpu_cl["queue"], (int(n),), None, a_buf, b_buf, c_buf)
            cl.enqueue_copy(_gpu_cl["queue"], c_np, c_buf)
            c[:int(n)] = c_np[:int(n)]

        _gpu_methods = {}
        # alloc/free/get/set and reductions keep the NumPy/SIMD CPU path
        for _m in (
            "alloc_f32", "alloc_f64", "alloc_i32", "alloc_i64",
            "free_f32", "free_f64", "free_i32", "free_i64",
            "get_f32", "get_f64", "get_i32", "get_i64",
            ...
            _gpu_methods[_m] = getattr(_simd_obj, _m)
        # elementwise ops -> GPU-aware (OpenCL device, else CPU SIMD)
        def _make_gpu_op(op, kind):
            def _op(a, b, c, n):
                return _gpu_binop(kind, op, a, b, c, n)
            return _op

        for _op in ("add", "sub", "mul", "div"):
            for _kind in ("f32", "f64", "i32", "i64"):
                _gpu_methods["%s_%s" % (_op, _kind)] = _make_gpu_op(_op, _kind)
        _gpu_methods["available"] = _gpu_available
        _gpu_methods["name"] = _gpu_name
        _gpu_methods["cuda_available"] = _gpu_cuda_available
        _gpu_methods["cuda_name"] = _gpu_cuda_name
        self.modules["gpu"] = _ksmod("gpu", _gpu_methods)
...
# Register fast evaluator dispatch for the hottest AST node types. Uses exact
# node type -> method name so the general chain stays authoritative for any
# unregistered or complex node.
_register_fast("Literal", "_ev_fast_literal")
_register_fast("Identifier", "_ev_fast_identifier")
_register_fast("BinaryOp", "_ev_fast_binaryop")
_register_fast("LetDecl", "_ev_fast_letdecl")
_register_fast("Assignment", "_ev_fast_assignment")
_register_fast("ForStmt", "_ev_fast_forstmt")
