...
class BuildPipeline:
    ...
    def __init__(self, source_file, output_name=None, output_dir=None):
        ...
        self.output_llvm_ir = _os.path.join(self.output_dir, f"{self.base_name}.ll")
    ...
    def compile_c_to_binary(self, optimization="O2") -> bool:
        ...
        # Aggressive flags to approach C native speed
        base_flags = (
            f"-{optimization} -march=native -mtune=native "
        ...
        cmd = f"gcc {base_flags} {self.temp_c_file} -o {self.output_binary} -lm -I{include_dir} {runtime_dir}/ks_runtime.a"
    ...
    def build(self, output_format="binary", optimization="O2"):
        ...
        try:
            ...
            if output_format == "c" or output_format == "binary":
                ...
                with open(self.output_llvm_ir, "w") as f:
                    f.write(ir_code)
                print(f"✓ LLVM IR saved to {self.output_llvm_ir}")
                print(f"  Output directory: {self.output_dir}")
                return True
...
""")
    ...
    if getattr(args, "llvm_ir", False) and args.file:
        ...
        if _llvm_backend._llvmlite_available:
            ...
            print(
                "[KS-LLVM] llvmlite not installed (pip install llvmlite) — emitting textual IR"
            )
        ...
        print(
            f"[KS-LLVM]   Compile: llc -filetype=obj {_ll_out} && clang {_ll_out.replace('.ll', '.o')} -o {_ll_out.replace('.ll', '')}"
        )
    ...
    if getattr(args, "opt_pipeline", None) and args.file:
        ...
        for _pass in _passes:
            ...
            if _pass == "dce":
                ...
                print(
                    "[KS-OPT]   ssa: SSA renaming tracked (full SSA requires IR lowering via --llvm-ir)"
                )
                _stats["ssa"] = "tracked"
            elif _pass == "escape":
                _escaped = sum(
                ...
                print(
                    "[KS-OPT]   unroll: loop unroll hints injected (LLVM applies on --llvm-ir)"
                )
    ...
    if getattr(args, "jit", False) and args.file:
        ...
        print(
            f"[KS-JIT] JIT mode — threshold={_threshold} calls before native promotion"
        )
        ...
        def _probe_call(func_name: str):
            ...
            if _call_counts[func_name] == _threshold and func_name not in _jit_compiled:
        ...
        def _jit_promote(func_name: str):
            ...
            if _llvm_b_avail:
                try:
                    ...
                    _mod = _llvmir.Module()
                    _fnty = _llvmir.FunctionType(
                        _llvmir.IntType(64), [_llvmir.IntType(64), _llvmir.IntType(64)]
                    )
                    ...
                    print(
                        f"[KS-JIT] ✓ llvmlite JIT: {func_name} promoted to native "
                        f"(addr={_addr:#x})"
                    )
                    return
            ...
            try:
                ...
                if _cfunc(3, 4) == 7:
                    _jit_compiled[func_name] = _cfunc
                    print(
                        f"[KS-JIT] ✓ mmap JIT: {func_name} promoted to native "
        ...
        print(
            f"[KS-JIT] Found {len(_funcs)} function(s) — simulating {_threshold + 10} calls each to trigger JIT"
        )
        ...
        print(
            f"[KS-JIT]   Backend:               {'ARM64' if _is_arm64 else 'x86-64'} native + llvmlite"
        )
        print(
            f"[KS-JIT]   JIT functions:         {list(_jit_compiled.keys()) or 'none promoted yet'}"
        )
        _llvmlite_avail_check = False
        try:
            import llvmlite.binding as _lchk

            _llvmlite_avail_check = True
        except ImportError:
            pass
