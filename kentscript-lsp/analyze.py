...
# Actual keywords from compiler/lexer/lexer.py KEYWORDS dict (65 keywords)
KEYWORDS = {
...
# Actual types from codegen/c_transpiler.py and the language spec
TYPES = {
...
# Unsafe builtins from ks/interpreter.py (require unsafe {} block)
UNSAFE_BUILTINS = {
    'malloc', 'realloc', 'calloc', 'alloca',
    'ptr_read', 'ptr_write', 'ptr_add', 'ptr_sub', 'ptr_cast',
    ...
    'dma_transfer', 'call_ptr', 'ptr',
    'mmio_write', 'mmio_read', 'write_mmio', 'read_mmio',
    'enable_interrupts', 'disable_interrupts',
    'volatile_read', 'volatile_write',
    'memory_barrier', 'dmb', 'dsb', 'isb',
    'write_port', 'read_port',
}

# Safe builtins from ks/interpreter.py (usable without unsafe {})
SAFE_BUILTINS = {
    ...
    'contains', 'startswith', 'endswith', 'type_of', 'sizeof', 'copy',
    'panic', 'assert', 'assert_eq', 'assert_ne', 'assert_true', 'assert_false',
    'unwrap', 'dbg', 'new',
    'exit', 'sleep', 'system', 'env', 'getcwd', 'spawn',
    ...
    'reversed', 'sorted', 'sum', 'min', 'max',
    'list', 'dict', 'set', 'tuple', 'str', 'int', 'float', 'bool', 'type',
    'format_value', 'reduce', 'ternary', 'borrow', 'release', 'move',
    'makedirs', 'remove', 'rename', 'rmtree', 'copytree',
    'getenv', 'uuid', 'to_string',
    'Lock', 'RLock', 'Event', 'Semaphore', 'ThreadPool',
    'thread', 'channel',
    'range_new',
    'free',
    'system_time', 'system_time_now', 'system_time_monotonic', 'system_time_perf_counter',
    'system_time_clock_gettime', 'system_time_sleep', 'system_time_time', 'system_time_utc',
    'alloc_i8', 'alloc_i16', 'alloc_i32', 'alloc_i64', 'alloc_i128',
    'alloc_u8', 'alloc_u16', 'alloc_u32', 'alloc_u64', 'alloc_u128',
    'alloc_f32', 'alloc_f64', 'alloc_f16', 'alloc_f128',
    'free_i8', 'free_i16', 'free_i32', 'free_i64', 'free_f32', 'free_f64',
    'get_i8', 'get_i16', 'get_i32', 'get_i64', 'get_f32', 'get_f64',
    'set_i8', 'set_i16', 'set_i32', 'set_i64', 'set_f32', 'set_f64',
}
...
def analyze(source):
    ...
    def check_stmt(node):
        ...
        if t == 'FunctionDef':
            ...
            if target and type(target).__name__ == 'Identifier':
                name = getattr(target, 'name', None)
                if name and lookup(name) is None:
                    err(node_line(node), 0, f"Undefined variable '{name}'. Use 'let {name} = ...;' to declare.")
            check_expr(getattr(node, 'value', None))
