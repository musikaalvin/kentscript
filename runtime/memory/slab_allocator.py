...
def _build_so() -> bool:
    """Compile ks_slab.c → ks_slab.{so,dylib,dll} if needed."""
    if not _SRC.exists():
        return False
    ...
    _SO_ext = _SO.with_suffix(lib_ext) if _SO.suffix != lib_ext else _SO

    cmd = [compiler_path, "-O3", "-shared", "-fPIC",
           "-o", str(_SO_ext), str(_SRC), "-lpthread"]
