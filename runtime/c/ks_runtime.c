...
/* ════════════════════════════════════════════════════════════════════════════
 * PRINT / OUTPUT — KentScript print/println builtins
 * ════════════════════════════════════════════════════════════════════════════ */

KS_EXPORT void print(const char *s) {
    if (s) printf("%s", s);
}

KS_EXPORT void println(const char *s) {
    if (s) printf("%s\n", s);
    else   printf("\n");
}
...
/* ════════════════════════════════════════════════════════════════════════════
 * kcrypt — native crypto backend (libsodium)
 *
 * These mirror the Python native bindings in ks/interpreter.py so that the
 * C-transpiled (build) backend produces identical results to the interpreter.
 * The branded password-hash string format is:
 *     $kcrypt$<year>$pyLord$<cost>$<salt22>$<payload32>
...
/* ── standard base64 (matches Python base64.b64encode) ─────────────────────── */
static const char KS_B64[] =
    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

static char ks_b64_idx(char c) {
    const char *p = strchr(KS_B64, c);
    return (char)(p ? (p - KS_B64) : 0);
}

static char *ks_b64_encode(const unsigned char *in, size_t len) {
    ...
    for (i = 0; i < len; i += 3) {
        ...
        if (i + 2 < len) n |= ((unsigned int)in[i + 2]);
        out[j++] = KS_B64[(n >> 18) & 0x3f];
        out[j++] = KS_B64[(n >> 12) & 0x3f];
        out[j++] = (i + 1 < len) ? KS_B64[(n >> 6) & 0x3f] : '=';
        out[j++] = (i + 2 < len) ? KS_B64[n & 0x3f] : '=';
    }
    out[j] = '\0';
    return out;
}

static unsigned char *ks_b64_decode(const char *in, size_t *outlen) {
    size_t len = in ? strlen(in) : 0;
    unsigned char *out = (unsigned char *)malloc((len / 4) * 3 + 3);
    if (!out) { *outlen = 0; return NULL; }
    size_t i, j = 0;
    unsigned int buf = 0;
    int bits = 0;
    for (i = 0; i < len; i++) {
        int v = (unsigned char)ks_b64_idx(in[i]);
        if (in[i] == '=' || in[i] == '\0') continue;
...
/* ── bcrypt-variant base64 (matches Python _kcrypt_b64_encode/decode) ──────── */
static const char KS_B64B[] =
    "./ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";

static char ks_b64b_idx(char c) {
    const char *p = strchr(KS_B64B, c);
    return (char)(p ? (p - KS_B64B) : 0);
}

static char *ks_b64b_encode(const unsigned char *in, size_t len) {
    size_t olen = (len * 8 + 5) / 6 + 1;
    char *out = (char *)malloc(olen + 1);
    if (!out) return NULL;
    size_t i, j = 0;
    for (i = 0; i < len; i += 3) {
        ...
        if (i + 1 < len) n |= ((unsigned int)in[i + 1]) << 8;
        if (i + 2 < len) n |= ((unsigned int)in[i + 2]);
        out[j++] = KS_B64B[(n >> 18) & 0x3f];
        out[j++] = KS_B64B[(n >> 12) & 0x3f];
        if (i + 1 < len) out[j++] = KS_B64B[(n >> 6) & 0x3f];
        if (i + 2 < len) out[j++] = KS_B64B[n & 0x3f];
    }
    out[j] = '\0';
    return out;
}

static unsigned char *ks_b64b_decode(const char *in, size_t *outlen) {
    size_t len = in ? strlen(in) : 0;
    unsigned char *out = (unsigned char *)malloc((len * 3) / 4 + 3);
    if (!out) { *outlen = 0; return NULL; }
    size_t i, j = 0;
    for (i = 0; i < len; i += 4) {
        unsigned int n = 0;
        n |= ((unsigned int)ks_b64b_idx(in[i])) << 18;
        if (i + 1 < len) n |= ((unsigned int)ks_b64b_idx(in[i + 1])) << 12;
        if (i + 2 < len) n |= ((unsigned int)ks_b64b_idx(in[i + 2])) << 6;
        if (i + 3 < len) n |= ((unsigned int)ks_b64b_idx(in[i + 3]));
        out[j++] = (unsigned char)((n >> 16) & 0xff);
        if (i + 2 < len) out[j++] = (unsigned char)((n >> 8) & 0xff);
        if (i + 3 < len) out[j++] = (unsigned char)(n & 0xff);
    }
    *outlen = j;
    return out;
}

/* ── cost tier → Argon2id params (mirrors Python _kcrypt_cost_params) ─────── */
static int ks_kcrypt_resolve_cost(long long cost) {
    int c = (int)cost;
    if (c < 3) c = 3;
    if (c > 24) c = 24;
    return c;
...
static void ks_kcrypt_cost_params(int c, unsigned long long *opslimit,
                                  size_t *memlimit) {
    int mshift = (c + 15 > 26) ? 26 : (c + 15);
    *memlimit = ((size_t)1) << mshift;
    int oshift = c - 7;
    if (oshift < 1) oshift = 1;
    if (oshift > 3) oshift = 3;
    *opslimit = ((unsigned long long)1) << oshift;
}

/* ── XChaCha20-Poly1305 AEAD ───────────────────────────────────────────────── */
KS_EXPORT char *system_kcrypt_xchacha20_encrypt(const char *data, const char *key,
                                                const char *nonce, const char *aad) {
    if (sodium_init() < 0) return NULL;
    unsigned char keybuf[32];
    size_t kl = key ? strlen(key) : 0;
    if (kl == 32) memcpy(keybuf, key, 32);
    else if (kl < 32) { memset(keybuf, 0, 32); memcpy(keybuf, key, kl); }
    else crypto_hash_sha256(keybuf, (const unsigned char *)key, kl);

    unsigned char noncebuf[24];
    if (!nonce) randombytes_buf(noncebuf, 24);
    else { memset(noncebuf, 0, 24); size_t nl = strlen(nonce); memcpy(noncebuf, nonce, nl > 24 ? 24 : nl); }

    const unsigned char *msg = (const unsigned char *)(data ? data : "");
    size_t mlen = data ? strlen(data) : 0;
    const unsigned char *aadp = (const unsigned char *)(aad ? aad : "");
    size_t alen = aad ? strlen(aad) : 0;

    unsigned char *ct = (unsigned char *)malloc(24 + mlen + crypto_aead_xchacha20poly1305_ietf_ABYTES);
    unsigned long long ctlen = 0;
    if (crypto_aead_xchacha20poly1305_ietf_encrypt(ct, &ctlen, msg, mlen, aadp, alen,
                                                  NULL, noncebuf, keybuf) != 0) {
        free(ct);
        return NULL;
    }
    unsigned char *combined = (unsigned char *)malloc(24 + ctlen);
    memcpy(combined, noncebuf, 24);
    memcpy(combined + 24, ct, ctlen);
    free(ct);
    char *out = ks_b64_encode(combined, 24 + ctlen);
    free(combined);
    return out;
}

KS_EXPORT char *system_kcrypt_xchacha20_decrypt(const char *data, const char *key,
                                                const char *nonce, const char *aad) {
    if (sodium_init() < 0) return NULL;
    size_t clen = 0;
    unsigned char *combined = ks_b64_decode(data ? data : "", &clen);
    if (!combined || clen < 24) { free(combined); return NULL; }
    ...
    const unsigned char *noncebuf = combined;
    const unsigned char *ct = combined + 24;
    size_t ctlen = clen - 24;
    const unsigned char *aadp = (const unsigned char *)(aad ? aad : "");
    size_t alen = aad ? strlen(aad) : 0;

    unsigned char *pt = (unsigned char *)malloc(ctlen + 1);
    unsigned long long ptlen = 0;
    int rc = crypto_aead_xchacha20poly1305_ietf_decrypt(pt, &ptlen, NULL, ct, ctlen,
                                                       aadp, alen, noncebuf, keybuf);
    free(combined);
    if (rc != 0) { free(pt); return NULL; }
    pt[ptlen] = '\0';
    char *out = (char *)malloc(ptlen + 1);
    memcpy(out, pt, ptlen + 1);
    free(pt);
    return out;
}

/* ── scrypt key derivation (n=16384, r=8, p=1) ────────────────────────────── */
KS_EXPORT char *system_kcrypt_derive_key(const char *password, const char *salt,
                                        long long length) {
    if (sodium_init() < 0) return NULL;
    long long len = length;
    if (len <= 0) len = 32;
    const char *pw = password ? password : "";
    const char *sl = salt ? salt : "";
    unsigned char *dk = (unsigned char *)malloc((size_t)len);
    if (crypto_pwhash_scryptsalsa208sha256_ll(
            (const unsigned char *)pw, strlen(pw),
            (const unsigned char *)sl, strlen(sl),
            16384, 8, 1, dk, (size_t)len) != 0) {
        free(dk);
        return NULL;
    }
    char *out = ks_b64_encode(dk, (size_t)len);
    free(dk);
    return out;
}

/* ── random key (base64 of random bytes) ──────────────────────────────────── */
KS_EXPORT char *system_kcrypt_random_key(long long length) {
    if (sodium_init() < 0) return NULL;
    long long len = length;
    if (len <= 0) len = 32;
    unsigned char *buf = (unsigned char *)malloc((size_t)len);
    randombytes_buf(buf, (size_t)len);
    char *out = ks_b64_encode(buf, (size_t)len);
    free(buf);
    return out;
}

/* ── misc helpers ─────────────────────────────────────────────────────────── */
KS_EXPORT char *system_kcrypt_int_to_bytes(long long n) {
...
KS_EXPORT long long system_kcrypt_bytes_to_int(const char *b) {
    if (!b) return 0;
    unsigned char bb[4] = {0, 0, 0, 0};
    int i;
    for (i = 0; i < 4 && b[i]; i++) bb[i] = (unsigned char)b[i];
    return ((long long)bb[0] << 24) | ((long long)bb[1] << 16) |
           ((long long)bb[2] << 8) | (long long)bb[3];
}

KS_EXPORT char *system_kcrypt_lower(const char *s) {
    if (!s) return NULL;
    char *out = strdup(s);
...
/* ── Argon2id password hashing (branded $kcrypt$ format) ──────────────────── */
KS_EXPORT char *system_kcrypt_hash_password(const char *password, long long cost) {
    if (sodium_init() < 0) return NULL;
    int c = ks_kcrypt_resolve_cost(cost);
    unsigned long long opslimit;
    size_t memlimit;
    ks_kcrypt_cost_params(c, &opslimit, &memlimit);

    const char *pw = password ? password : "";
    unsigned char salt[16];
    randombytes_buf(salt, 16);
    unsigned char payload[24];
    if (crypto_pwhash(payload, 24, (const unsigned char *)pw, strlen(pw), salt,
                       opslimit, memlimit, crypto_pwhash_ALG_ARGON2ID13) != 0) {
        return NULL;
    }
    char *salt_b64 = ks_b64b_encode(salt, 16);
    char *payload_b64 = ks_b64b_encode(payload, 24);
    char *result = (char *)malloc(128);
    /* Year is taken from the real system clock so the branded token ages
     * with time (e.g. $kcrypt$2026$... this year, $kcrypt$2027$... next).
     * Verification does not pin the year, so tokens stay valid across years. */
    time_t _now = time(NULL);
    struct tm *_tm = localtime(&_now);
    int _year = _tm ? (_tm->tm_year + 1900) : 2026;
    snprintf(result, 128, "$kcrypt$%d$pyLord$%02d$%s$%s", _year, c, salt_b64, payload_b64);
    free(salt_b64);
    free(payload_b64);
    return result;
}

KS_EXPORT long long system_kcrypt_verify_password(const char *hash, const char *password) {
    if (!hash) return 0;
    if (sodium_init() < 0) return 0;

    char *hcopy = strdup(hash);
    if (!hcopy) return 0;
    char *parts[8];
    int np = 0;
    char *save = NULL;
    char *tok = strtok_r(hcopy, "$", &save);
    while (tok && np < 8) {
        parts[np++] = tok;
        tok = strtok_r(NULL, "$", &save);
    }
    /* strtok drops the leading delimiter, so we get 6 tokens:
       [0]=kcrypt [1]=<year> [2]=pyLord [3]=cost [4]=salt [5]=payload */
    if (np != 6 || strcmp(parts[0], "kcrypt") != 0) {
        free(hcopy);
        return 0;
    }
    int c = ks_kcrypt_resolve_cost((long long)atol(parts[3]));
    unsigned long long opslimit;
    size_t memlimit;
    ks_kcrypt_cost_params(c, &opslimit, &memlimit);

    size_t slen = 0, plen = 0;
    unsigned char *salt = ks_b64b_decode(parts[4], &slen);
    unsigned char *expected = ks_b64b_decode(parts[5], &plen);
    if (!salt || !expected || slen != 16 || plen != 24) {
        free(salt); free(expected); free(hcopy);
        return 0;
    }
    const char *pw = password ? password : "";
    unsigned char derived[24];
    if (crypto_pwhash(derived, 24, (const unsigned char *)pw, strlen(pw), salt,
                      opslimit, memlimit, crypto_pwhash_ALG_ARGON2ID13) != 0) {
        free(salt); free(expected); free(hcopy);
        return 0;
    }
    unsigned char diff = 0;
    for (int i = 0; i < 24; i++) diff |= (unsigned char)(derived[i] ^ expected[i]);
    free(salt); free(expected); free(hcopy);
    return (diff == 0) ? 1 : 0;
}
