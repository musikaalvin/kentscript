...
func _execute(cmd, shell, capture_output) {
    if capture_output {
        let result = system_subprocess_run(cmd, shell, true);
        return _make_completed(cmd, result.returncode, result.stdout, result.stderr);
    } else {
        let result = system_subprocess_run(cmd, shell, false);
...
func call(cmd, shell) {
    if shell == none { shell = true; }
    let result = run_command(cmd, false, false, shell);
    return result.returncode;
}

func check_call(cmd, shell) {
    if shell == none { shell = true; }
    let result = run_command(cmd, false, true, shell);
    return result.returncode;
}

func check_output(cmd) {
    let result = run_command(cmd, true, true, true);
    return result.stdout;
}

func getoutput(cmd) {
    let result = run_command(cmd, true, false, true);
    return result.stdout;
}
