...
ks_val_t multiply(ks_val_t a, ks_val_t b);
void describe_number(ks_val_t n);

#line 9 "examples/demo.ks"
ks_val_t add(ks_val_t a, ks_val_t b) {
    return ks_v_add(a, b);
    return ks_none();
}

#line 13 "examples/demo.ks"
ks_val_t multiply(ks_val_t a, ks_val_t b) {
    return ks_v_mul(a, b);
    return ks_none();
}

#line 33 "examples/demo.ks"
void describe_number(ks_val_t n) {
    { /* match statement */
        ks_val_t _match_v = n;
        if (ks_v_cmp(_match_v, KS_INT(90LL)) == 0) {
            ks_val_print(ks_str("Grade 90: A")); printf("\n");
        } else if (ks_v_cmp(_match_v, KS_INT(80LL)) == 0) {
            ks_val_print(ks_str("Grade 80: B")); printf("\n");
        } else if (ks_v_cmp(_match_v, KS_INT(70LL)) == 0) {
            ks_val_print(ks_str("Grade 70: C")); printf("\n");
        } else {
...
#line 4 "examples/demo.ks"
    ks_val_print(ks_str("=== KentScript Basics ===")); printf("\n");
#line 5 "examples/demo.ks"
    ks_val_print(ks_str("Pattern matching, functions, control flow, and unsafe operations")); printf("\n");
#line 7 "examples/demo.ks"
    ks_val_print(ks_str("\n--- Functions ---")); printf("\n");
#line 17 "examples/demo.ks"
    ks_val_print(ks_str(_ks_concat("add(5, 3) = ", ks_val_to_str(add(KS_INT(5LL), KS_INT(3LL)))))); printf("\n");
#line 18 "examples/demo.ks"
    ks_val_print(ks_str(_ks_concat("multiply(4, 7) = ", ks_val_to_str(multiply(KS_INT(4LL), KS_INT(7LL)))))); printf("\n");
#line 20 "examples/demo.ks"
    ks_val_print(ks_str("\n--- Control Flow ---")); printf("\n");
#line 22 "examples/demo.ks"
    ks_val_t x = KS_INT(10LL);
    if (ks_v_bool(ks_bool(ks_v_cmp(x, KS_INT(0LL)) > 0))) {
        ks_val_print(ks_str("x is positive")); printf("\n");
    } else if (ks_v_bool(ks_bool(ks_v_cmp(x, KS_INT(0LL)) < 0))) {
        ks_val_print(ks_str("x is negative")); printf("\n");
    } else {
        ks_val_print(ks_str("x is zero")); printf("\n");
    }
#line 31 "examples/demo.ks"
    ks_val_print(ks_str("\n--- Pattern Matching ---")); printf("\n");
#line 50 "examples/demo.ks"
    describe_number(KS_INT(90LL));
#line 51 "examples/demo.ks"
    describe_number(KS_INT(85LL));
#line 52 "examples/demo.ks"
    describe_number(KS_INT(72LL));
#line 53 "examples/demo.ks"
    describe_number(KS_INT(50LL));
#line 55 "examples/demo.ks"
    ks_val_print(ks_str("\n--- Unsafe/Memory Operations ---")); printf("\n");
    /* unsafe block */
    {
        void* ptr = ks_malloc(KS_INT(64LL));
        ks_val_print(ks_str(_ks_concat("Allocated 64 bytes at: ", _ks_str_hex((long long)(uintptr_t)ptr)))); printf("\n");
        (ks_ptr_write((void*)ptr, 3735928559LL, 1), 0);
        ks_val_t val = ks_int((long long)(ks_ptr_read((void*)ptr, 8LL)));
        ks_val_print(ks_str(_ks_concat("Read value: 0x", _ks_str_hex(_ks_as_i(val))))); printf("\n");
        free((void*)ptr);
        ks_val_print(ks_str("Memory freed")); printf("\n");
    }
#line 67 "examples/demo.ks"
    ks_val_print(ks_str("\n=== Demo Complete ===")); printf("\n");
    return 0;
}