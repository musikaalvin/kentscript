...
#line 11 "examples/networking.ks"
    ks_val_print(ks_str("=== HIGH-LEVEL HTTP ===")); printf("\n");
#line 13 "examples/networking.ks"
    _ks_http_response_t resp = _ks_http_get("http://httpbin.org/get");
#line 14 "examples/networking.ks"
    ks_val_print(ks_str(_ks_concat("GET status: ", _ks_str_int(resp.status)))); printf("\n");
#line 16 "examples/networking.ks"
    _ks_http_response_t resp2 = _ks_http_post("http://httpbin.org/post", "{\"key\": \"value\"}");
#line 17 "examples/networking.ks"
    ks_val_print(ks_str(_ks_concat("POST status: ", _ks_str_int(resp2.status)))); printf("\n");
#line 19 "examples/networking.ks"
    _ks_http_response_t resp3 = _ks_http_get("http://httpbin.org/json");
    if (ks_v_bool(ks_bool(ks_v_cmp(resp3.status, KS_INT(200LL)) == 0))) {
        ks_val_t data = _ks_json_loads(resp3.body);
        ks_val_print(ks_str("JSON response parsed OK")); printf("\n");
    }
#line 26 "examples/networking.ks"
    ks_val_print(ks_str("\n=== LOW-LEVEL SOCKET ===")); printf("\n");
    /* unsafe block */
    {
        ks_val_t sock = system_socket_create(KS_INT(2LL), KS_INT(1LL), KS_INT(0LL));
        ks_val_print(ks_str(_ks_concat("Socket created: ", ks_val_to_str(sock)))); printf("\n");
        system_socket_settimeout(sock, KS_FLT(5.0));
        long long err = system_socket_connect(sock, ks_str(("93.184.216.34")), KS_INT(80LL));
        if (ks_v_bool(ks_bool(ks_v_cmp(ks_int(err), ks_none()) == 0))) {
            system_socket_send(sock, ks_str(("HEAD / HTTP/1.0\r\nHost: example.com\r\n\r\n")));
            data = system_socket_recv(sock, KS_INT(256LL));
            ks_val_t first_line = ks_array_get(_ks_str_split(ks_val_to_str(data), "\r\n"), KS_INT(0LL));
            ks_val_print(ks_str(_ks_concat("Raw socket response: ", ks_val_to_str(first_line)))); printf("\n");
        } else {
            ks_val_print(ks_str(_ks_concat("Connect error (expected in restricted env): ", _ks_str_int(err)))); printf("\n");
        }
        system_socket_close(sock);
    }
    /* unsafe block */
    {
        ks_val_t udp = system_socket_create(KS_INT(2LL), KS_INT(2LL), KS_INT(0LL));
        ks_val_print(ks_str(_ks_concat("UDP socket created: ", ks_val_to_str(udp)))); printf("\n");
        system_socket_close(udp);
    }
#line 54 "examples/networking.ks"
    ks_val_print(ks_str("\n=== NETWORKING VERIFIED ===")); printf("\n");
    return 0;
}