...
static File_t* __new_File__(void) {
...
static char* __get_File_attr__(File_t* self, const char* name) {
...
static long long __get_File_attr_num__(File_t* self, const char* name) {
    for (int i = 0; i < self->__attr_count__; i++) {
        if (strcmp(self->__attr_names__[i], name) == 0) {
...
static void __set_File_attr__(File_t* self, const char* name, long long value) {
    for (int i = 0; i < self->__attr_count__; i++) {
        if (strcmp(self->__attr_names__[i], name) == 0) {
...
static void __method_File_open__(File_t* self, long long filename) {
    char* content = system_file_read_text(ks_val_to_str(ks_int(filename)));
...
int main(int argc, char** argv) {
    ...
    _ks_init_globals();
    { /* try */
        __method_File_open__(f, (long long)(uintptr_t)"/home/pylord/Documents/apikeys.txt");
    } /* end try */
    if (0) { /* except */
        ks_val_t e = ks_none(); /* except var */
        ks_val_print(ks_str(_ks_concat("Error: ", ks_val_to_str(e)))); printf("\n");
    }
    return 0;
}