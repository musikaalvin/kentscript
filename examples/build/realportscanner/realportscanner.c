...
void scan(ks_val_t target, ks_val_t port, ks_val_t timeout);

#line 6 "examples/realportscanner.ks"
void scan(ks_val_t target, ks_val_t port, ks_val_t timeout) {
    { /* try */
        long long sock = 0;
...
int main(int argc, char** argv) {
    ...
    char* host = "127.0.0.1";
    static ks_val_t _arr_common_ports[] = {(ks_val_t){.tag=KS_T_INT,.as.i=21LL}, (ks_val_t){.tag=KS_T_INT,.as.i=22LL}, (ks_val_t){.tag=KS_T_INT,.as.i=80LL}, (ks_val_t){.tag=KS_T_INT,.as.i=443LL}, (ks_val_t){.tag=KS_T_INT,.as.i=3306LL}, (ks_val_t){.tag=KS_T_INT,.as.i=8080LL}};
    ks_array common_ports = { .data = _arr_common_ports, .length = 6 };
#line 26 "examples/realportscanner.ks"
    ks_val_print(ks_str(_ks_concat(_ks_concat("--- Starting Scan on ", host), " ---"))); printf("\n");
    ...
    for (long long _idx_port = 0; _idx_port < _iter__idx_port.length; _idx_port++) {
        ...
        if (ks_v_bool(scan(ks_str((host)), port, KS_FLT(1.0)))) {
            ks_val_print(ks_str(_ks_concat(_ks_concat("[+] SUCCESS: Port ", ks_val_to_str(port)), " is OPEN"))); printf("\n");
        } else {
            ks_val_print(ks_str(_ks_concat(_ks_concat("[-] FAILED: Port ", ks_val_to_str(port)), " is CLOSED"))); printf("\n");
        }
    }
    return 0;
}