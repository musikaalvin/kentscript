...
static ExploitManager_t* __new_ExploitManager__(void) {
    ExploitManager_t* self = (ExploitManager_t*)malloc(sizeof(ExploitManager_t));
    self->__class__ = "ExploitManager";
    self->__attr_count__ = 0;
    memset(self->__attr_is_str__, 0, sizeof(self->__attr_is_str__));
    return self;
}

static char* __get_ExploitManager_attr__(ExploitManager_t* self, const char* name) {
...
static long long __get_ExploitManager_attr_num__(ExploitManager_t* self, const char* name) {
    for (int i = 0; i < self->__attr_count__; i++) {
        if (strcmp(self->__attr_names__[i], name) == 0) {
...
static void __set_ExploitManager_attr__(ExploitManager_t* self, const char* name, long long value) {
    for (int i = 0; i < self->__attr_count__; i++) {
        if (strcmp(self->__attr_names__[i], name) == 0) {
...
static void __method_ExploitManager_use__(ExploitManager_t* self, long long module_path) {
    __set_ExploitManager_attr__(self, "current_module", module_path);
    ks_val_print(ks_str(_ks_concat("Selected module: ", ks_val_to_str(ks_int(module_path))))); printf("\n");
}

static void __method_ExploitManager_set__(ExploitManager_t* self, long long key, long long value) {
    (__get_ExploitManager_attr_num__(self, "options"))[ks_int(key)] = value;
    ks_val_print(ks_str(_ks_concat(_ks_concat(ks_val_to_str(ks_int(key)), " => "), ks_val_to_str(ks_int(value))))); printf("\n");
}

static void __method_ExploitManager_run__(ExploitManager_t* self) {
    if (__builtin_expect((ks_v_bool(ks_bool(ks_v_cmp(ks_int(__get_ExploitManager_attr_num__(self, "current_module")), ks_none()) == 0))), 0)) {
        ks_val_print(ks_str("Error: No module selected.")); printf("\n");
    } else {
        ks_val_print(ks_str(_ks_concat("Launching exploit: ", ks_val_to_str(ks_int(__get_ExploitManager_attr_num__(self, "current_module")))))); printf("\n");
    }
}
...
int main(int argc, char** argv) {
    ...
    ExploitManager_t* msf = __new_ExploitManager__();
#line 29 "examples/msf.ks"
    __method_ExploitManager_use__(msf, (long long)(uintptr_t)"exploit/multi/handler");
#line 30 "examples/msf.ks"
    __method_ExploitManager_set__(msf, (long long)(uintptr_t)"LHOST", (long long)(uintptr_t)"192.168.1.10");
#line 31 "examples/msf.ks"
    __method_ExploitManager_run__(msf);
    return 0;
}