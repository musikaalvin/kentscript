...
void _ks_init_globals(void) {
    fd = _ks_syscall_open("/dev/mem", _ks_as_i(KS_INT(2LL)));
}

int main(int argc, char** argv) {
    ks_argc = argc; ks_argv = argv;
    _ks_init_globals();
#line 5 "examples/syscall1.ks"
    _ks_syscall_write(fd, ks_val_to_str(data));
#line 6 "examples/syscall1.ks"
    0;
#line 7 "examples/syscall1.ks"
    _ks_syscall_close(fd);
#line 10 "examples/syscall1.ks"
    long long pid = 0;
    if (__builtin_expect((ks_v_bool(ks_bool(ks_v_cmp(ks_int(pid), KS_INT(0LL)) == 0))), 0)) {
        0;
    } else {
        0;
    }
#line 16 "examples/syscall1.ks"
    0;
    return 0;
}