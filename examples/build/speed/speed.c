...
ks_val_t sum = {.tag = KS_T_INT, .as.i = 0LL};
ks_val_t i = {.tag = KS_T_INT, .as.i = 0LL};
ks_val_t iterations = {.tag = KS_T_INT, .as.i = 1000000LL};
double start;

void _ks_init_globals(void) {
    start = ks_time_seconds();
}

int main(int argc, char** argv) {
    ks_argc = argc; ks_argv = argv;
    _ks_init_globals();
#line 7 "examples/speed.ks"
    ks_val_print(ks_str("Starting Real VM Stress Test...")); printf("\n");
    while (ks_v_bool(ks_bool(ks_v_cmp(i, iterations) < 0))) {
        sum = ks_v_add(sum, i);
        i = ks_v_add(i, KS_INT(1LL));
    }
#line 14 "examples/speed.ks"
    double end = ks_time_seconds();
#line 15 "examples/speed.ks"
    ks_val_print(ks_str(_ks_concat("Real Sum: ", ks_val_to_str(sum)))); printf("\n");
#line 16 "examples/speed.ks"
    ks_val_print(ks_str(_ks_concat("Execution Time: ", ks_val_to_str(ks_v_sub(ks_flt(end), ks_flt(start)))))); printf("\n");
    return 0;
}