...
void run_exploit(void);

ks_val_t win;

#line 10 "examples/msf_controlgui.ks"
void run_exploit(void) {
    ks_val_print(ks_str("Executing payload...")); printf("\n");
    _ks_set_text(status, "Status: EXPLOIT SENT");
}

void _ks_init_globals(void) {
    win = _ks_Window(gui, "pyMetasploit Control v5.0");
}

int main(int argc, char** argv) {
    ks_argc = argc; ks_argv = argv;
    _ks_init_globals();
#line 5 "examples/msf_controlgui.ks"
    _ks_set_theme(win, "dark");
#line 7 "examples/msf_controlgui.ks"
    long long title = _ks_Label(gui, "Exploit Manager Active");
#line 8 "examples/msf_controlgui.ks"
    long long status = _ks_Label(gui, "Status: Connected to Framework");
#line 15 "examples/msf_controlgui.ks"
    long long exec_btn = _ks_Button(gui, "Execute Payload", run_exploit);
#line 17 "examples/msf_controlgui.ks"
    _ks_add(win, title);
#line 18 "examples/msf_controlgui.ks"
    _ks_add(win, status);
#line 19 "examples/msf_controlgui.ks"
    _ks_add(win, exec_btn);
#line 20 "examples/msf_controlgui.ks"
    _ks_show(win);
    return 0;
}