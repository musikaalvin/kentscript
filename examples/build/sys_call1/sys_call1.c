...
void _ks_init_globals(void) {
    pid = _ks_syscall_getpid();
}

int main(int argc, char** argv) {
    ks_argc = argc; ks_argv = argv;
    _ks_init_globals();
#line 3 "examples/sys_call1.ks"
    ks_val_print(ks_str("PID:")); printf(" "); ks_val_print(ks_int(pid)); printf("\n");
    /* unsafe block */
    {
        void* ptr = ks_malloc(KS_INT(256LL));
        write_byte((void*)ptr, ks_v_i(KS_INT(0LL)), ks_v_i(KS_INT(65LL)));
        ks_val_print(ks_str("Memory:")); printf(" "); ks_val_print(ks_int(read_byte((void*)ptr, ks_v_i(KS_INT(0LL))))); printf("\n");
        free((void*)ptr);
    }
    return 0;
}