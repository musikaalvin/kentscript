...
void encrypt_file_interactive(void);
void decrypt_file_interactive(void);
void generate_key_interactive(void);

#line 12 "examples/kcrypt_menu.ks"
void ks_user_main(void) {
    while (ks_v_bool(KS_BOOL(1))) {
        print_menu();
        char* choice = _ks_input("Enter choice (1-5): ");
        if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((choice)), ks_str(("1"))) == 0))) {
            encrypt_file_interactive();
        } else if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((choice)), ks_str(("2"))) == 0))) {
            decrypt_file_interactive();
        } else if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((choice)), ks_str(("3"))) == 0))) {
            generate_key_interactive();
        } else if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((choice)), ks_str(("4"))) == 0))) {
            print_banner();
        } else if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((choice)), ks_str(("5"))) == 0))) {
            ks_val_print(ks_str(_ks_concat(_ks_concat("\033[32m", "Goodbye!"), "\033[0m"))); printf("\n");
            break;
        } else {
            ks_val_print(ks_str(_ks_concat(_ks_concat("\033[31m", "Invalid choice. Please enter 1-5."), "\033[0m"))); printf("\n");
        }
        ks_val_print(ks_str("")); printf("\n");
    }
}

#line 35 "examples/kcrypt_menu.ks"
void print_menu(void) {
    ks_val_print(ks_str(_ks_concat(_ks_concat("\033[1m", "=== kcrypt File Encryption Menu ==="), "\033[0m"))); printf("\n");
    ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat(_ks_concat("1. ", "\033[32m"), "Encrypt"), "\033[0m"), " a file (.kcrypt extension)"))); printf("\n");
    ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat(_ks_concat("2. ", "\033[34m"), "Decrypt"), "\033[0m"), " a .kcrypt file"))); printf("\n");
    ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat(_ks_concat("3. ", "\033[33m"), "Generate"), "\033[0m"), " a new random key"))); printf("\n");
    ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat(_ks_concat("4. ", "\033[36m"), "View"), "\033[0m"), " banner"))); printf("\n");
    ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat("5. ", "\033[31m"), "Exit"), "\033[0m"))); printf("\n");
}

#line 44 "examples/kcrypt_menu.ks"
void print_banner(void) {
    ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat("\033[1m", "\033[32m"), "\n  __  __     _     _    ___     ___\n |  \\/  |___| |__ | |  | _ )___| _ \\\n | |\\/| / -_) '_ \\| |__| _ / -_)   /\n |_|  |\\___|_.__/|____|___\\___|_|_\\\n  XChaCha20-Poly1305 AEAD Encryption\n"), "\033[0m"))); printf("\n");
}

#line 56 "examples/kcrypt_menu.ks"
void encrypt_file_interactive(void) {
    char* filepath = _ks_input("Enter file path to encrypt: ");
    if (ks_v_bool(ks_bool(!ks_v_bool(system_file_exists(filepath))))) {
        ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat("\033[31m", "Error: File not found: "), filepath), "\033[0m"))); printf("\n");
        return;
    }
    char* key_input = _ks_input("Enter your encryption key (or type 'gen' for new key): ");
    ks_val_t key = ks_none();
    if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((key_input)), ks_str(("gen"))) == 0))) {
        key = system_kcrypt_random_key(KS_INT(32LL));
        char* key_path = _ks_concat(filepath, ".key");
        (system_file_write_text(key_path, ks_val_to_str(key)), 0);
        ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat("\033[32m", "New key saved to: "), key_path), "\033[0m"))); printf("\n");
    } else {
        key = key_input;
    }
    long long enc_path = 0;
    ks_val_print(ks_str(_ks_concat(_ks_concat("\033[32m", "File encrypted successfully!"), "\033[0m"))); printf("\n");
    ks_val_print(ks_str(_ks_concat("  Output: ", ks_val_to_str(ks_int(enc_path))))); printf("\n");
    ks_val_print(ks_str(_ks_concat(_ks_concat("  Encrypted size: ", _ks_str_int(_ks_len(enc_path))), " (path length)"))); printf("\n");
}

#line 85 "examples/kcrypt_menu.ks"
void decrypt_file_interactive(void) {
    char* filepath = _ks_input("Enter .kcrypt file path to decrypt: ");
    if (ks_v_bool(ks_bool(!ks_v_bool(system_file_exists(filepath))))) {
        ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat("\033[31m", "Error: File not found: "), filepath), "\033[0m"))); printf("\n");
        return;
    }
    char* key = _ks_input("Enter your decryption key: ");
    long long dec_data = 0;
    if (ks_v_bool(ks_bool(ks_v_cmp(ks_int(dec_data), ks_str((""))) != 0))) {
        ks_val_print(ks_str(_ks_concat(_ks_concat("\033[32m", "File decrypted successfully!"), "\033[0m"))); printf("\n");
        ks_val_print(ks_str(_ks_concat(_ks_concat("  Decrypted ", _ks_str_int(_ks_len(dec_data))), " bytes"))); printf("\n");
        if (ks_v_bool(ks_bool(ks_v_cmp(ks_int(_ks_len(dec_data)), KS_INT(200LL)) <= 0))) {
            ks_val_print(ks_str("  --- Content preview ---")); printf("\n");
            ks_val_print(ks_int(dec_data)); printf("\n");
            ks_val_print(ks_str("  --- End preview ---")); printf("\n");
        } else {
            ks_array _slice_tmp_0 = _ks_slice(dec_data, KS_INT(0LL), KS_INT(200LL), KS_INT(1LL));
            ks_array _slice_tmp_1 = _ks_slice(dec_data, KS_INT(0LL), KS_INT(200LL), KS_INT(1LL));
            ks_array _slice_tmp_2 = _ks_slice(dec_data, KS_INT(0LL), KS_INT(200LL), KS_INT(1LL));
            ks_array _slice_tmp_3 = _ks_slice(dec_data, KS_INT(0LL), KS_INT(200LL), KS_INT(1LL));
            ks_array _slice_tmp_4 = _ks_slice(dec_data, KS_INT(0LL), KS_INT(200LL), KS_INT(1LL));
            ks_array _slice_tmp_5 = _ks_slice(dec_data, KS_INT(0LL), KS_INT(200LL), KS_INT(1LL));
            ks_val_print(ks_str(_ks_concat(_ks_concat("  Preview: ", ks_val_to_str(_slice_tmp_5)), "..."))); printf("\n");
        }
    }
}

#line 115 "examples/kcrypt_menu.ks"
void generate_key_interactive(void) {
    char* length_str = _ks_input("Key length in bytes (default 32): ");
    ks_val_t length = KS_INT(32LL);
    if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((length_str)), ks_str((""))) != 0))) {
        ks_val_t ok = KS_BOOL(0);
        ks_val_t parsed = KS_INT(0LL);
        if (ks_v_bool(ks_bool(ks_v_cmp(ks_int(_ks_len(length_str)), KS_INT(0LL)) > 0))) {
            ks_val_t i = KS_INT(0LL);
            ks_val_t is_num = KS_BOOL(1);
            while (ks_v_bool(ks_bool(ks_v_cmp(i, ks_int(_ks_len(length_str))) < 0))) {
                char* c = _ks_str_at(length_str, _ks_as_i(i));
                if (ks_v_bool(ks_bool(ks_v_bool(ks_bool(ks_v_cmp(ks_str((c)), ks_str(("0"))) < 0)) || ks_v_bool(ks_bool(ks_v_cmp(ks_str((c)), ks_str(("9"))) > 0))))) {
                    is_num = KS_BOOL(0);
                    break;
                }
            ...
            if (ks_v_bool(is_num)) {
                parsed = ks_int((long long)(atoll(length_str)));
                if (ks_v_bool(ks_bool(ks_v_cmp(parsed, KS_INT(16LL)) >= 0))) {
                    length = parsed;
                }
            }
        }
    }
    char* key = system_kcrypt_random_key(length);
    ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat("\033[32m", "Generated key: "), key), "\033[0m"))); printf("\n");
    char* save = _ks_input("Save to key file? (y/n): ");
    if (ks_v_bool(ks_bool(ks_v_bool(ks_bool(ks_v_cmp(ks_str((save)), ks_str(("y"))) == 0)) || ks_v_bool(ks_bool(ks_v_cmp(ks_str((save)), ks_str(("Y"))) == 0))))) {
        char* path = _ks_input("File path (default: key.kcrypt.key): ");
        if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((path)), ks_str((""))) == 0))) {
            path = "key.kcrypt.key";
        }
        (system_file_write_text(path, key), 0);
        ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat("\033[32m", "Key saved to: "), path), "\033[0m"))); printf("\n");
    }
}
