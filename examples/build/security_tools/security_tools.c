...
/* ===== END ks_val_t ===== */
void* _ks_fileio_mod = 0;
void* _ks_subprocess_mod = 0;
...
ks_val_t scan_port(ks_val_t host, ks_val_t port);
ks_val_t hash_file(ks_val_t path);

#line 15 "examples/security_tools.ks"
ks_val_t scan_port(ks_val_t host, ks_val_t port) {
    /* unsafe block */
    {
        ks_val_t sock = system_socket_create(KS_INT(2LL), KS_INT(1LL), KS_INT(0LL));
        system_socket_settimeout(sock, KS_FLT(0.5));
        long long err = system_socket_connect(sock, host, port);
        system_socket_close(sock);
        return ks_bool(ks_v_cmp(ks_int(err), ks_none()) == 0);
    }
    return ks_none();
}

#line 38 "examples/security_tools.ks"
ks_val_t hash_file(ks_val_t path) {
    char* content = system_file_read_text(ks_val_to_str(path));
    return ks_str(system_crypto_sha256(content));
    return ks_none();
}

int main(int argc, char** argv) {
    ks_argc = argc; ks_argv = argv;
#line 13 "examples/security_tools.ks"
    ks_val_print(ks_str("=== PORT SCANNER ===")); printf("\n");
#line 25 "examples/security_tools.ks"
    char* target = "127.0.0.1";
#line 26 "examples/security_tools.ks"
    static ks_val_t _arr_ports[] = {(ks_val_t){.tag=KS_T_INT,.as.i=22LL}, (ks_val_t){.tag=KS_T_INT,.as.i=80LL}, (ks_val_t){.tag=KS_T_INT,.as.i=443LL}, (ks_val_t){.tag=KS_T_INT,.as.i=3306LL}, (ks_val_t){.tag=KS_T_INT,.as.i=5432LL}, (ks_val_t){.tag=KS_T_INT,.as.i=6379LL}, (ks_val_t){.tag=KS_T_INT,.as.i=8080LL}};
    ks_array ports = { .data = _arr_ports, .length = 7 };
#line 27 "examples/security_tools.ks"
    ks_val_print(ks_str(_ks_concat(_ks_concat("Scanning ", target), "..."))); printf("\n");
    ...
    for (long long _idx_port = 0; _idx_port < _iter__idx_port.length; _idx_port++) {
        ...
        if (ks_v_bool(scan_port(ks_str((target)), port))) {
            ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat("  [OPEN] ", target), ":"), ks_val_to_str(port)))); printf("\n");
        }
    }
#line 33 "examples/security_tools.ks"
    ks_val_print(ks_str("Scan done.")); printf("\n");
#line 36 "examples/security_tools.ks"
    ks_val_print(ks_str("\n=== FILE INTEGRITY CHECKER ===")); printf("\n");
#line 44 "examples/security_tools.ks"
    (system_file_write_text("/tmp/ks_file1.txt", "original content"), 0);
#line 45 "examples/security_tools.ks"
    (system_file_write_text("/tmp/ks_file2.txt", "original content"), 0);
#line 46 "examples/security_tools.ks"
    (system_file_write_text("/tmp/ks_file3.txt", "tampered content!"), 0);
#line 48 "examples/security_tools.ks"
    static ks_val_t _arr_files[] = {(ks_val_t){.tag=KS_T_STR,.as.s=(char*)"/tmp/ks_file1.txt"}, (ks_val_t){.tag=KS_T_STR,.as.s=(char*)"/tmp/ks_file2.txt"}, (ks_val_t){.tag=KS_T_STR,.as.s=(char*)"/tmp/ks_file3.txt"}};
    ks_array files = { .data = _arr_files, .length = 3 };
#line 49 "examples/security_tools.ks"
    ks_val_t baseline = hash_file(ks_str(("/tmp/ks_file1.txt")));
#line 50 "examples/security_tools.ks"
    ks_val_print(ks_str(_ks_concat("Baseline hash: ", ks_val_to_str(baseline)))); printf("\n");
    ks_array _iter__idx_f = files;
    for (long long _idx_f = 0; _idx_f < _iter__idx_f.length; _idx_f++) {
        ks_val_t f = ks_array_get(_iter__idx_f, ks_int((long long)(_idx_f)));
        ks_val_t h = hash_file(f);
        if (ks_v_bool(ks_bool(ks_v_cmp(h, baseline) == 0))) {
            ks_val_print(ks_str(_ks_concat("  [OK]      ", ks_val_to_str(f)))); printf("\n");
        } else {
            ks_val_print(ks_str(_ks_concat("  [TAMPERED] ", ks_val_to_str(f)))); printf("\n");
        }
    }
#line 61 "examples/security_tools.ks"
    0;
#line 62 "examples/security_tools.ks"
    0;
#line 63 "examples/security_tools.ks"
    0;
#line 66 "examples/security_tools.ks"
    ks_val_print(ks_str("\n=== PROCESS INSPECTOR ===")); printf("\n");
#line 68 "examples/security_tools.ks"
    ks_val_t ps = ks_subprocess_run(strdup(_ks_concat(_ks_concat(_ks_concat(_ks_concat("ps", " "), "aux"), " "), "--no-headers")), ks_int(1LL), ks_int(1LL));
#line 69 "examples/security_tools.ks"
    ks_array lines = _ks_str_split(ks_val_to_str(ks_val_array_get(ps, ks_int((long long)1))), "\n");
#line 70 "examples/security_tools.ks"
    ks_val_print(ks_str(_ks_concat("Total processes: ", _ks_str_int(lines.length)))); printf("\n");
#line 73 "examples/security_tools.ks"
    ks_val_t result = ks_subprocess_run(strdup(_ks_concat(_ks_concat(_ks_concat(_ks_concat("pgrep", " "), "-l"), " "), "python")), ks_int(1LL), ks_int(1LL));
    if (ks_v_bool(ks_bool(ks_v_cmp(ks_val_array_get(result, ks_int((long long)0)), KS_INT(0LL)) == 0))) {
        ks_val_print(ks_str(_ks_concat("Python processes:\n", ks_val_to_str(ks_val_array_get(result, ks_int((long long)1)))))); printf("\n");
    }
#line 79 "examples/security_tools.ks"
    ks_val_print(ks_str("\n=== CRYPTO OPERATIONS ===")); printf("\n");
#line 81 "examples/security_tools.ks"
    char* password = "mysecretpassword";
#line 82 "examples/security_tools.ks"
    char* salt = "randomsalt123";
#line 84 "examples/security_tools.ks"
    char* sha256_hash = system_crypto_sha256(password);
#line 85 "examples/security_tools.ks"
    ks_val_print(ks_str(_ks_concat("SHA256: ", sha256_hash))); printf("\n");
#line 87 "examples/security_tools.ks"
    char* sha512_hash = system_crypto_sha512(password);
#line 88 "examples/security_tools.ks"
    ks_val_print(ks_str(_ks_concat("SHA512: ", sha512_hash))); printf("\n");
#line 90 "examples/security_tools.ks"
    char* hmac = system_crypto_hmac(password, salt);
#line 91 "examples/security_tools.ks"
    ks_val_print(ks_str(_ks_concat("HMAC-SHA256: ", hmac))); printf("\n");
#line 93 "examples/security_tools.ks"
    char* pbkdf2 = system_crypto_pbkdf2(password, salt, 10000LL);
#line 94 "examples/security_tools.ks"
    ks_val_print(ks_str(_ks_concat("PBKDF2: ", pbkdf2))); printf("\n");
#line 96 "examples/security_tools.ks"
    char* token = system_crypto_generate_token(32LL);
#line 97 "examples/security_tools.ks"
    ks_val_print(ks_str(_ks_concat("Secure token: ", token))); printf("\n");
#line 100 "examples/security_tools.ks"
    ks_val_print(ks_str("\n=== SYSCALL INSPECTION ===")); printf("\n");
    /* unsafe block */
    {
        ks_val_t pid = ks_int((long long)(ks_system_syscall(39LL, 0, 0, 0, 0, 0, 0)));
        ks_val_t ppid = ks_int((long long)(ks_system_syscall(110LL, 0, 0, 0, 0, 0, 0)));
        ks_val_t uid = ks_int((long long)(ks_system_syscall(102LL, 0, 0, 0, 0, 0, 0)));
        ks_val_t gid = ks_int((long long)(ks_system_syscall(104LL, 0, 0, 0, 0, 0, 0)));
        ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat(_ks_concat(_ks_concat(_ks_concat(_ks_concat("PID=", ks_val_to_str(pid)), " PPID="), ks_val_to_str(ppid)), " UID="), ks_val_to_str(uid)), " GID="), ks_val_to_str(gid)))); printf("\n");
    }
#line 110 "examples/security_tools.ks"
    ks_val_print(ks_str("\n=== SECURITY TOOLS VERIFIED ===")); printf("\n");
    return 0;
}