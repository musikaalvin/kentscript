...
ks_val_t kmalloc(ks_val_t size);
ks_val_t kstrlen(ks_val_t s);
void uart_puts(ks_val_t msg);
void show_task_info(ks_val_t pid, ks_val_t name, ks_val_t ticks);
void minios_main(void);

#line 4 "examples/minios_component.ks"
ks_val_t kmalloc(ks_val_t size) {
    void* ptr = ks_malloc(size);
    return ptr;
    return ks_none();
}

#line 9 "examples/minios_component.ks"
ks_val_t kstrlen(ks_val_t s) {
    ks_val_t i = KS_INT(0LL);
    while (ks_v_bool(ks_bool(ks_v_cmp(i, ks_int(ks_array_len(s))) < 0))) {
        if (__builtin_expect((ks_v_bool(ks_bool(ks_v_cmp(ks_val_array_get(s, i), KS_INT(0LL)) == 0))), 0)) {
            return i;
        }
...
#line 20 "examples/minios_component.ks"
void uart_puts(ks_val_t msg) {
    ks_val_t i = KS_INT(0LL);
    while (ks_v_bool(ks_bool(ks_v_cmp(i, ks_int(ks_array_len(msg))) < 0))) {
        ks_val_print(ks_val_array_get(msg, i)); printf("\n");
        i = ks_v_add(i, KS_INT(1LL));
    }
}

#line 28 "examples/minios_component.ks"
void show_task_info(ks_val_t pid, ks_val_t name, ks_val_t ticks) {
    ks_val_print(ks_str("Task: ")); printf("\n");
    ks_val_print(name); printf("\n");
    ks_val_print(ks_str(" PID:")); printf("\n");
    ks_val_print(pid); printf("\n");
    ks_val_print(ks_str(" ticks:")); printf("\n");
    ks_val_print(ticks); printf("\n");
    ks_val_print(ks_str("\n")); printf("\n");
}

#line 38 "examples/minios_component.ks"
void minios_main(void) {
    ks_val_print(ks_str("=== MiniOS KentScript Component ===\n")); printf("\n");
    ks_val_print(ks_str("Written in KentScript syntax!\n")); printf("\n");
    ks_val_print(ks_str("Compiles to C -> builds to ELF\n")); printf("\n");
    ks_val_print(ks_str("================================\n")); printf("\n");
    ks_val_t buf = kmalloc(KS_INT(64LL));
    if (ks_v_bool(ks_bool(ks_v_cmp(buf, KS_INT(0LL)) != 0))) {
        ks_val_print(ks_str("[OK] kmalloc working\n")); printf("\n");
    }
    show_task_info(KS_INT(1LL), ks_str(("init")), KS_INT(100LL));
    show_task_info(KS_INT(2LL), ks_str(("shell")), KS_INT(50LL));
    ks_val_print(ks_str("\nKentScript OS ready!\n")); printf("\n");
}

int main(int argc, char** argv) {
    ks_argc = argc; ks_argv = argv;
#line 55 "examples/minios_component.ks"
    minios_main();
    return 0;
}