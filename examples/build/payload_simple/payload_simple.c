...
#line 19 "examples/payload_simple.ks"
void ks_user_main(void) {
    ks_val_print(ks_str("=== KentScript Low-Level Payload Demo ===")); printf("\n");
    ks_val_print(ks_str("")); printf("\n");
    ks_val_print(ks_str("[1] Shellcode Bytes:")); printf("\n");
    ks_val_print(ks_str("    execve('/bin/sh') - x86_64")); printf("\n");
    ks_val_print(ks_str(_ks_concat(_ks_concat("    Size: ", _ks_str_int(ks_array_len(SHELLCODE))), " bytes"))); printf("\n");
    ks_val_print(ks_str("")); printf("\n");
    ks_val_print(ks_str("[2] Memory Operations:")); printf("\n");
    /* unsafe block */
    {
        void* buf = ks_malloc(KS_INT(256LL));
        ks_val_print(ks_str(_ks_concat("    Allocated at: ", _ks_str_hex((long long)(uintptr_t)buf)))); printf("\n");
        (ks_ptr_write((void*)buf, 3735928559LL, 1), 0);
        (ks_ptr_write((void*)((void*)((uintptr_t)(buf) + (uintptr_t)(4LL))), 3405691582LL, 1), 0);
        ks_val_t val1 = ks_int((long long)(ks_ptr_read((void*)buf, 4LL)));
        ks_val_t val2 = ks_int((long long)(ks_ptr_read((void*)((void*)((uintptr_t)(buf) + (uintptr_t)(4LL))), 4LL)));
        ks_val_print(ks_str("    Written: 0xDEADBEEF 0xCAFEBABE")); printf("\n");
        ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat("    Read: ", ks_val_to_str(val1)), " "), ks_val_to_str(val2)))); printf("\n");
        free((void*)buf);
        ks_val_print(ks_str("    Freed")); printf("\n");
    }
    ks_val_print(ks_str("")); printf("\n");
    ks_val_print(ks_str("[3] Inline Assembly:")); printf("\n");
    /* unsafe block */
    {
        __asm__("nop");
        __asm__("mov rax, 42");
        ks_val_print(ks_str("    Executed: nop, mov rax, 42")); printf("\n");
    }
    ks_val_print(ks_str("")); printf("\n");
    ks_val_print(ks_str("[4] Pointer Operations:")); printf("\n");
    ks_val_t x = KS_INT(3735928559LL);
    /* unsafe block */
    {
        void* addr = ((void*)(&x));
        ks_val_print(ks_str(_ks_concat("    Variable x = ", ks_val_to_str(x)))); printf("\n");
        ks_val_print(ks_str(_ks_concat("    Address: ", _ks_str_hex((long long)(uintptr_t)addr)))); printf("\n");
        long long value = (*(long long*)(addr));
        ks_val_print(ks_str(_ks_concat("    Dereferenced: ", _ks_str_int(value)))); printf("\n");
    }
    ks_val_print(ks_str("")); printf("\n");
