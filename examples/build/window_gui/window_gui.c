...
void on_click(void);
void on_get_text(void);
void on_message(void);

ks_val_t win;
ks_val_t label;

#line 14 "examples/window_gui.ks"
void on_click(void) {
    ks_val_t count = ks_v_add(count, KS_INT(1LL));
    _ks_set_text(gui, label, _ks_concat("Clicked: ", ks_val_to_str(count)));
}

#line 28 "examples/window_gui.ks"
void on_get_text(void) {
    long long text = _ks_get_text(gui, entry);
    _ks_set_text(gui, label, _ks_concat("You typed: ", ks_val_to_str(ks_int(text))));
}

#line 37 "examples/window_gui.ks"
void on_message(void) {
    _ks_message_box(gui, "Info", "This is KentScript!", "info");
}

void _ks_init_globals(void) {
    win = _ks_create_window(gui, "My KentScript App", KS_INT(400LL), KS_INT(300LL));
    label = _ks_create_label(gui, win, "Hello KentScript!");
}

int main(int argc, char** argv) {
    ks_argc = argc; ks_argv = argv;
    _ks_init_globals();
#line 8 "examples/window_gui.ks"
    _ks_pack(gui, label);
#line 11 "examples/window_gui.ks"
    ks_val_t count = KS_INT(0LL);
#line 20 "examples/window_gui.ks"
    long long btn = _ks_create_button(gui, win, "Click Me!", on_click);
#line 21 "examples/window_gui.ks"
    _ks_pack(gui, btn);
#line 24 "examples/window_gui.ks"
    long long entry = _ks_create_entry(gui, win);
#line 25 "examples/window_gui.ks"
    _ks_pack(gui, entry);
#line 33 "examples/window_gui.ks"
    long long get_btn = _ks_create_button(gui, win, "Get Text", on_get_text);
#line 34 "examples/window_gui.ks"
    _ks_pack(gui, get_btn);
#line 41 "examples/window_gui.ks"
    long long msg_btn = _ks_create_button(gui, win, "Show Message", on_message);
#line 42 "examples/window_gui.ks"
    _ks_pack(gui, msg_btn);
#line 45 "examples/window_gui.ks"
