...
ks_val_t x = {.tag = KS_T_INT, .as.i = 1000000LL};
double start_time;
ks_val_t result = {.tag = KS_T_INT, .as.i = 0LL};
ks_val_t i = {.tag = KS_T_INT, .as.i = 0LL};

void _ks_init_globals(void) {
    start_time = ks_time_seconds();
}

int main(int argc, char** argv) {
    ks_argc = argc; ks_argv = argv;
    _ks_init_globals();
    while (ks_v_bool(ks_bool(ks_v_cmp(i, x) < 0))) {
        result = ks_v_add(ks_v_sub(ks_v_add(result, ks_v_mul(i, i)), ks_v_div(i, KS_INT(2LL))), ks_v_mod(i, KS_INT(3LL)));
        i = ks_v_add(i, KS_INT(1LL));
    }
#line 15 "examples/cpu_intensive_benchmark.ks"
    double end_time = ks_time_seconds();
#line 16 "examples/cpu_intensive_benchmark.ks"
    ks_val_t execution_time = ks_v_sub(ks_flt(end_time), ks_flt(start_time));
#line 18 "examples/cpu_intensive_benchmark.ks"
    ks_val_print(ks_str(_ks_concat("Result: ", ks_val_to_str(result)))); printf("\n");
#line 19 "examples/cpu_intensive_benchmark.ks"
    ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat(_ks_concat("Time for ", ks_val_to_str(x)), " iterations: "), ks_val_to_str(execution_time)), " seconds"))); printf("\n");
    return 0;
}