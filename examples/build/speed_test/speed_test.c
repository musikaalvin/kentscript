...
ks_val_t now_ms(void);

ks_val_t CPU_ITERS = {.tag = KS_T_INT, .as.i = 10000000LL};
ks_val_t MEM_ITERS = {.tag = KS_T_INT, .as.i = 1000000LL};
ks_val_t FLOAT_ITERS = {.tag = KS_T_INT, .as.i = 10000000LL};

#line 5 "examples/speed_test.ks"
ks_val_t now_ms(void) {
...
#line 9 "examples/speed_test.ks"
    ks_val_print(ks_str("==============================")); printf("\n");
#line 10 "examples/speed_test.ks"
    ks_val_print(ks_str("⚡ KENTSCRIPT HONEST BENCHMARK")); printf("\n");
#line 11 "examples/speed_test.ks"
    ks_val_print(ks_str("==============================")); printf("\n");
#line 14 "examples/speed_test.ks"
    ks_val_t start = now_ms();
#line 15 "examples/speed_test.ks"
    ks_val_t cpu_result = KS_INT(0LL);
#line 16 "examples/speed_test.ks"
    ks_val_t i = KS_INT(0LL);
    while (ks_v_bool(ks_bool(ks_v_cmp(i, CPU_ITERS) < 0))) {
        cpu_result = ks_v_add(cpu_result, ks_v_mul(i, i));
        i = ks_v_add(i, KS_INT(1LL));
    }
#line 21 "examples/speed_test.ks"
    ks_val_t cpu_time = ks_v_sub(now_ms(), start);
#line 22 "examples/speed_test.ks"
    ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat(_ks_concat("CPU:      ", ks_val_to_str(cpu_time)), " ms ("), ks_val_to_str(cpu_result)), ")"))); printf("\n");
    start = now_ms();
#line 26 "examples/speed_test.ks"
    long long* arr = (long long*)ks_alloc_i64(_ks_as_i(MEM_ITERS));
#line 27 "examples/speed_test.ks"
    ks_val_t j = KS_INT(0LL);
    while (ks_v_bool(ks_bool(ks_v_cmp(j, MEM_ITERS) < 0))) {
        ((long long*)(arr))[_ks_as_i(j)] = _ks_as_i(j);
        j = ks_v_add(j, KS_INT(1LL));
    }
    j = KS_INT(0LL);
    while (ks_v_bool(ks_bool(ks_v_cmp(j, MEM_ITERS) < 0))) {
        ((long long*)(arr))[_ks_as_i(j)] = _ks_as_i(ks_v_mul(ks_int(((long long*)(arr))[_ks_as_i(j)]), KS_INT(2LL)));
        j = ks_v_add(j, KS_INT(1LL));
    }
#line 37 "examples/speed_test.ks"
    ks_val_t mem_result = ks_int(((long long*)(arr))[_ks_as_i(ks_v_sub(MEM_ITERS, KS_INT(1LL)))]);
#line 38 "examples/speed_test.ks"
    free((void*)arr);
#line 39 "examples/speed_test.ks"
    ks_val_t mem_time = ks_v_sub(now_ms(), start);
#line 40 "examples/speed_test.ks"
    ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat(_ks_concat("Memory:   ", ks_val_to_str(mem_time)), " ms ("), ks_val_to_str(mem_result)), ")"))); printf("\n");
    start = now_ms();
#line 44 "examples/speed_test.ks"
    ks_val_t float_result = KS_FLT(0.0);
#line 45 "examples/speed_test.ks"
    ks_val_t k = KS_INT(0LL);
    while (ks_v_bool(ks_bool(ks_v_cmp(k, FLOAT_ITERS) < 0))) {
        ks_val_t x = ks_v_mul(k, KS_FLT(0.001));
        float_result = ks_v_add(float_result, ks_v_mul(x, x));
        k = ks_v_add(k, KS_INT(1LL));
    }
#line 51 "examples/speed_test.ks"
    ks_val_t float_time = ks_v_sub(now_ms(), start);
#line 52 "examples/speed_test.ks"
    ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat(_ks_concat("Float:    ", ks_val_to_str(float_time)), " ms ("), ks_val_to_str(float_result)), ")"))); printf("\n");
#line 54 "examples/speed_test.ks"
    ks_val_t total = ks_v_add(ks_v_add(cpu_time, mem_time), float_time);
#line 55 "examples/speed_test.ks"
    ks_val_print(ks_str("------------------------------")); printf("\n");
#line 56 "examples/speed_test.ks"
    ks_val_print(ks_str(_ks_concat(_ks_concat("TOTAL:    ", ks_val_to_str(total)), " ms"))); printf("\n");
    return 0;
}