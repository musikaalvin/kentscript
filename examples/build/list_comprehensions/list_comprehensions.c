...
void _ks_init_globals(void) {
    ...
    for (long long x = 0, _k__lc_tmp_0 = 0; _k__lc_tmp_0 < _n__lc_tmp_0; x++, _k__lc_tmp_0++) {
        _lc_tmp_0.data[_k__lc_tmp_0] = ks_v_mul(ks_int(x), ks_int(x));
    }
...
int main(int argc, char** argv) {
    ...
    _ks_init_globals();
#line 6 "examples/list_comprehensions.ks"
    ks_val_print(ks_str("Squares: ")); printf(" "); ks_val_print(ks_arr(&(squares))); printf("\n");
#line 9 "examples/list_comprehensions.ks"
    ks_array _lc_tmp_1 = {NULL, 0};
    for (long long x = 0; x < 10; x++) {
        if (ks_v_bool(ks_bool(ks_v_cmp(ks_v_mod(ks_int(x), KS_INT(2LL)), KS_INT(0LL)) == 0))) {
        _ks_array_append(&_lc_tmp_1, ks_v_mul(ks_int(x), ks_int(x)));
        }
    }
    ks_array even_squares = _lc_tmp_1;
#line 10 "examples/list_comprehensions.ks"
    ks_val_print(ks_str("Even squares: ")); printf(" "); ks_val_print(ks_arr(&(even_squares))); printf("\n");
#line 13 "examples/list_comprehensions.ks"
    ks_array _lc_tmp_2 = {NULL, 0};
    for (long long n = 1; n < 30; n++) {
        if (ks_v_bool(ks_bool(ks_v_cmp(ks_v_mod(ks_int(n), KS_INT(3LL)), KS_INT(0LL)) == 0))) {
        _ks_array_append(&_lc_tmp_2, ks_int(n));
        }
    }
    ks_array div_by_three = _lc_tmp_2;
#line 14 "examples/list_comprehensions.ks"
    ks_val_print(ks_str("Divisible by 3: ")); printf(" "); ks_val_print(ks_arr(&(div_by_three))); printf("\n");
#line 17 "examples/list_comprehensions.ks"
    static ks_val_t _arr_words[] = {(ks_val_t){.tag=KS_T_STR,.as.s=(char*)"hello"}, (ks_val_t){.tag=KS_T_STR,.as.s=(char*)"world"}, (ks_val_t){.tag=KS_T_STR,.as.s=(char*)"kentscript"}};
    ks_array words = { .data = _arr_words, .length = 3 };
#line 18 "examples/list_comprehensions.ks"
    ks_array _lc_tmp_3 = {NULL, 0};
    for (long long _i__lc_tmp_3 = 0; _i__lc_tmp_3 < words.length; _i__lc_tmp_3++) {
        ks_val_t w = ks_array_get(words, ks_int((long long)(_i__lc_tmp_3)));
        _ks_array_append(&_lc_tmp_3, w);
    }
    ks_array upper_words = _lc_tmp_3;
#line 19 "examples/list_comprehensions.ks"
    ks_val_print(ks_str("Words: ")); printf(" "); ks_val_print(ks_arr(&(upper_words))); printf("\n");
#line 22 "examples/list_comprehensions.ks"
    long long _n__lc_tmp_4 = (8) - (0);
    ks_array _lc_tmp_4 = { (ks_val_t*)malloc(_n__lc_tmp_4 * sizeof(ks_val_t)), 0, _n__lc_tmp_4 };
    for (long long i = 0, _k__lc_tmp_4 = 0; _k__lc_tmp_4 < _n__lc_tmp_4; i++, _k__lc_tmp_4++) {
        _lc_tmp_4.data[_k__lc_tmp_4] = ks_v_pow(KS_INT(2LL), ks_int(i));
    }
    _lc_tmp_4.length = _n__lc_tmp_4;
    ks_array powers = _lc_tmp_4;
#line 23 "examples/list_comprehensions.ks"
    ks_val_print(ks_str("Powers of 2: ")); printf(" "); ks_val_print(ks_arr(&(powers))); printf("\n");
#line 25 "examples/list_comprehensions.ks"
    ks_val_print(ks_str("\n✓ List comprehensions working!")); printf("\n");
    return 0;
}