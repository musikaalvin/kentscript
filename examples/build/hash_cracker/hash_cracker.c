...
void _ks_init_globals(void) {
    parser = system_argparse_new("KentScript Hash Cracker v1.0");
}

int main(int argc, char** argv) {
    ks_argc = argc; ks_argv = argv;
    _ks_init_globals();
#line 8 "examples/hash_cracker.ks"
    system_argparse_add_argument(parser, "--hash");
#line 9 "examples/hash_cracker.ks"
    system_argparse_add_argument(parser, "--wordlist");
#line 10 "examples/hash_cracker.ks"
    system_argparse_add_argument(parser, "--type");
#line 12 "examples/hash_cracker.ks"
    _ks_dict* args = system_argparse_parse_args(parser, 0);
    if (__builtin_expect((ks_v_bool(ks_bool(ks_v_bool(ks_bool(_ks_dict_attr(args, "hash") == NULL)) || ks_v_bool(ks_bool(_ks_dict_attr(args, "wordlist") == NULL))))), 0)) {
        ks_val_print(ks_str("Usage: --hash <hash> --wordlist <wordlist.txt> [--type md5|sha1|sha256]")); printf("\n");
        ks_val_print(ks_str("")); printf("\n");
        ks_val_print(ks_str("Example: --hash 5f4dcc3b5aa765d61d8327deb882cf99 --wordlist passwords.txt --type md5")); printf("\n");
        system_os_exit(KS_INT(1LL));
    }
#line 21 "examples/hash_cracker.ks"
    char* target_hash = _ks_str_lower((char*)(uintptr_t)(_ks_dict_attr(args, "hash")));
#line 22 "examples/hash_cracker.ks"
    char* wordlist_file = _ks_dict_attr(args, "wordlist");
#line 23 "examples/hash_cracker.ks"
    char* hash_type = "md5";
    if (ks_v_bool(ks_bool(_ks_dict_attr(args, "type") != NULL))) {
        hash_type = _ks_str_lower((char*)(uintptr_t)(_ks_dict_attr(args, "type")));
    }
#line 29 "examples/hash_cracker.ks"
    ks_val_print(ks_str("[*] KentScript Hash Cracker v1.0")); printf("\n");
#line 30 "examples/hash_cracker.ks"
    ks_val_print(ks_str(_ks_concat("[*] Target Hash: ", target_hash))); printf("\n");
#line 31 "examples/hash_cracker.ks"
    ks_val_print(ks_str(_ks_concat("[*] Hash Type: ", hash_type))); printf("\n");
#line 32 "examples/hash_cracker.ks"
    ks_val_print(ks_str(_ks_concat("[*] Wordlist: ", wordlist_file))); printf("\n");
#line 33 "examples/hash_cracker.ks"
    ks_val_print(ks_str("")); printf("\n");
#line 36 "examples/hash_cracker.ks"
    char* wordlist_content = system_file_read_text(wordlist_file);
    if (__builtin_expect((ks_v_bool(ks_bool(ks_v_cmp(ks_str((wordlist_content)), ks_none()) == 0))), 0)) {
        ks_val_print(ks_str(_ks_concat("[!] Could not read wordlist: ", wordlist_file))); printf("\n");
        system_os_exit(KS_INT(1LL));
    }
#line 42 "examples/hash_cracker.ks"
    ks_array passwords = _ks_str_split(wordlist_content, "\n");
#line 43 "examples/hash_cracker.ks"
    ks_val_print(ks_str(_ks_concat(_ks_concat("[*] Loaded ", _ks_str_int(ks_array_len(passwords))), " passwords"))); printf("\n");
#line 44 "examples/hash_cracker.ks"
    ks_val_print(ks_str("[*] Starting attack...")); printf("\n");
#line 45 "examples/hash_cracker.ks"
    ks_val_print(ks_str("")); printf("\n");
#line 47 "examples/hash_cracker.ks"
    ks_val_t start_time = ks_flt(ks_time_monotonic_ms());
#line 48 "examples/hash_cracker.ks"
    ks_val_t checked = KS_INT(0LL);
    ks_array _iter__idx_password = passwords;
    for (long long _idx_password = 0; _idx_password < _iter__idx_password.length; _idx_password++) {
        ks_val_t password = ks_array_get(_iter__idx_password, ks_int((long long)(_idx_password)));
        if (ks_v_bool(ks_bool(ks_v_cmp(ks_int((long long)strlen((char*)(uintptr_t)(ks_val_to_str(password)))), KS_INT(0LL)) == 0))) {
            continue;
        }
        char* hash = "";
        if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((hash_type)), ks_str(("md5"))) == 0))) {
            hash = system_crypto_md5(ks_v_str(password));
        } else if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((hash_type)), ks_str(("sha1"))) == 0))) {
            hash = system_crypto_sha1(ks_v_str(password));
        } else if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((hash_type)), ks_str(("sha256"))) == 0))) {
            hash = system_crypto_sha256(ks_v_str(password));
        } else if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((hash_type)), ks_str(("sha512"))) == 0))) {
            hash = system_crypto_sha512(ks_v_str(password));
        } else {
            ks_val_t md5_hash = ks_str((system_crypto_md5(ks_v_str(password))));
            ks_val_t sha1_hash = ks_str((system_crypto_sha1(ks_v_str(password))));
            char* sha256_hash = system_crypto_sha256(ks_v_str(password));
            if (__builtin_expect((ks_v_bool(ks_bool(ks_v_cmp(md5_hash, ks_str((target_hash))) == 0))), 0)) {
                ks_val_print(ks_str(_ks_concat(_ks_concat("[+] FOUND! [", hash_type), "]"))); printf("\n");
                ks_val_print(ks_str(_ks_concat("[+] Password: ", ks_val_to_str(password)))); printf("\n");
                ks_val_t elapsed = ks_v_sub(ks_flt(ks_time_monotonic_ms()), start_time);
                ks_val_print(ks_str(_ks_concat(_ks_concat("[*] Time: ", ks_val_to_str(elapsed)), "s"))); printf("\n");
                system_os_exit(KS_INT(0LL));
            }
            if (__builtin_expect((ks_v_bool(ks_bool(ks_v_cmp(sha1_hash, ks_str((target_hash))) == 0))), 0)) {
                ks_val_print(ks_str("[+] FOUND! [sha1]")); printf("\n");
                ks_val_print(ks_str(_ks_concat("[+] Password: ", ks_val_to_str(password)))); printf("\n");
                ks_val_t elapsed = ks_v_sub(ks_flt(ks_time_monotonic_ms()), start_time);
                ks_val_print(ks_str(_ks_concat(_ks_concat("[*] Time: ", ks_val_to_str(elapsed)), "s"))); printf("\n");
                system_os_exit(KS_INT(0LL));
            }
            if (__builtin_expect((ks_v_bool(ks_bool(ks_v_cmp(ks_str((sha256_hash)), ks_str((target_hash))) == 0))), 0)) {
                ks_val_print(ks_str("[+] FOUND! [sha256]")); printf("\n");
                ks_val_print(ks_str(_ks_concat("[+] Password: ", ks_val_to_str(password)))); printf("\n");
                ks_val_t elapsed = ks_v_sub(ks_flt(ks_time_monotonic_ms()), start_time);
                ks_val_print(ks_str(_ks_concat(_ks_concat("[*] Time: ", ks_val_to_str(elapsed)), "s"))); printf("\n");
                system_os_exit(KS_INT(0LL));
            ...
            if (ks_v_bool(ks_bool(ks_v_cmp(ks_v_mod(checked, KS_INT(100LL)), KS_INT(0LL)) == 0))) {
                ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat(_ks_concat("[*] Progress: ", ks_val_to_str(checked)), "/"), _ks_str_int(ks_array_len(passwords))), "..."))); printf("\n");
            }
            continue;
        }
        hash = _ks_str_lower((char*)(uintptr_t)(hash));
        if (__builtin_expect((ks_v_bool(ks_bool(ks_v_cmp(ks_str((hash)), ks_str((target_hash))) == 0))), 0)) {
            ks_val_print(ks_str("")); printf("\n");
            ks_val_print(ks_str("[+] ===== PASSWORD FOUND =====")); printf("\n");
            ks_val_print(ks_str(_ks_concat("[+] Hash Type: ", hash_type))); printf("\n");
            ks_val_print(ks_str(_ks_concat("[+] Password: ", ks_val_to_str(password)))); printf("\n");
            ks_val_print(ks_str(_ks_concat("[+] Hash: ", hash))); printf("\n");
            ks_val_t elapsed = ks_v_sub(ks_flt(ks_time_monotonic_ms()), start_time);
            ks_val_print(ks_str(_ks_concat(_ks_concat("[*] Time: ", ks_val_to_str(elapsed)), "s"))); printf("\n");
            ks_val_print(ks_str(_ks_concat(_ks_concat("[*] Checked: ", ks_val_to_str(ks_v_add(checked, KS_INT(1LL)))), " passwords"))); printf("\n");
            system_os_exit(KS_INT(0LL));
        }
        checked = ks_v_add(checked, KS_INT(1LL));
        if (ks_v_bool(ks_bool(ks_v_cmp(ks_v_mod(checked, KS_INT(100LL)), KS_INT(0LL)) == 0))) {
...
#line 120 "examples/hash_cracker.ks"
    ks_val_t elapsed = ks_v_sub(ks_flt(ks_time_monotonic_ms()), start_time);
#line 121 "examples/hash_cracker.ks"
    ks_val_print(ks_str("")); printf("\n");
#line 122 "examples/hash_cracker.ks"
    ks_val_print(ks_str("[!] Password not found in wordlist")); printf("\n");
#line 123 "examples/hash_cracker.ks"
    ks_val_print(ks_str(_ks_concat(_ks_concat("[*] Checked: ", ks_val_to_str(checked)), " passwords"))); printf("\n");
#line 124 "examples/hash_cracker.ks"
    ks_val_print(ks_str(_ks_concat(_ks_concat("[*] Time: ", ks_val_to_str(elapsed)), "s"))); printf("\n");
    return 0;
}