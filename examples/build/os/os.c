...
void safe_delete(ks_val_t path);

ks_val_t files;
char* cwd;

#line 10 "examples/os.ks"
void safe_delete(ks_val_t path) {
    { /* try */
        (ks_os_remove(path), 0);
        ks_val_print(ks_str("Deleted:")); printf(" "); ks_val_print(path); printf("\n");
    } /* end try */
    if (0) { /* except */
        ks_val_print(ks_str("Cannot delete:")); printf(" "); ks_val_print(path); printf(" "); ks_val_print(ks_str("Error:")); printf(" "); ks_val_print(error); printf("\n");
    }
}

void _ks_init_globals(void) {
    files = (ks_os_listdir("/"), 0);
    cwd = ks_os_getcwd();
}
...
int main(int argc, char** argv) {
    ...
    _ks_init_globals();
#line 7 "examples/os.ks"
    (ks_os_mkdir("new_folder", 0755), 0);
    return 0;
}