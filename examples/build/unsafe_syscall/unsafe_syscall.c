...
int main(int argc, char** argv) {
    ...
    /* unsafe block */
    {
        void* buffer = ks_malloc(KS_INT(1024LL));
        write_byte((void*)buffer, ks_v_i(KS_INT(0LL)), ks_v_i(KS_INT(72LL)));
        write_byte((void*)buffer, ks_v_i(KS_INT(1LL)), ks_v_i(KS_INT(105LL)));
        write_byte((void*)buffer, ks_v_i(KS_INT(2LL)), ks_v_i(KS_INT(0LL)));
        long long c1 = read_byte((void*)buffer, ks_v_i(KS_INT(0LL)));
        long long c2 = read_byte((void*)buffer, ks_v_i(KS_INT(1LL)));
        ks_val_print(ks_str(_ks_concat(_ks_concat(_ks_concat("Characters: ", _ks_str_int(c1)), " "), _ks_str_int(c2)))); printf("\n");
        (ks_write_port(KS_INT(1016LL), KS_INT(65LL)), 0);
        ks_val_t port_val = ks_int((long long)(ks_read_port(KS_INT(1016LL))));
        ks_val_t mmio_addr = KS_INT(2147483648LL);
        write_mmio(mmio_addr, KS_INT(305419896LL));
        ks_val_t mmio_val = read_mmio(mmio_addr);
        ks_val_print(ks_str(_ks_concat("MMIO value: ", ks_val_to_str(mmio_val)))); printf("\n");
        free((void*)buffer);
    }
#line 34 "examples/unsafe_syscall.ks"
    ks_val_print(ks_str("Unsafe block complete")); printf("\n");
    return 0;
}