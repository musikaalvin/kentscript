...
void refresh_files(void);
void open_selected(void);
void show_about(void);
void show_documentation(void);
void update_status(ks_val_t message, ks_val_t msg_type);
void update_title(void);
void update_line_col(void);

char* current_file = "";
ks_val_t is_modified = {.tag = KS_T_BOOL, .as.b = 0};
char* current_directory = ".";
ks_val_t font_size = {.tag = KS_T_INT, .as.i = 14LL};
char* bg_color = "#1e1e2e";
char* fg_color = "#cdd6f4";
char* accent_color = "#89b4fa";
char* success_color = "#a6e3a1";
char* warning_color = "#f9e2af";
char* error_color = "#f38ba8";
char* sidebar_bg = "#181825";
char* toolbar_bg = "#313244";
char* editor_bg = "#11111b";
ks_val_t window;

#line 211 "examples/full_gui_demo.ks"
void new_file(void) {
    _ks_delete(gui, editor, "1.0", "end");
    current_file = "";
    is_modified = KS_BOOL(0);
    update_status(ks_str(("🆕 New file created")), ks_str(("success")));
    update_title();
}

#line 219 "examples/full_gui_demo.ks"
void open_file(void) {
    long long path = _ks_filedialog(gui, "open", "Open File");
    if (ks_v_bool(ks_bool(ks_v_cmp(ks_int(path), ks_str((""))) != 0))) {
        long long content = _ks_read(file, path);
        _ks_delete(gui, editor, "1.0", "end");
        _ks_insert(gui, editor, "1.0", content);
        current_file = path;
        is_modified = KS_BOOL(0);
        update_status(ks_str((_ks_concat("📂 Opened: ", ks_val_to_str(ks_int(path))))), ks_str(("success")));
        update_title();
    }
}

#line 232 "examples/full_gui_demo.ks"
void save_file(void) {
    if (ks_v_bool(ks_bool(ks_v_cmp(ks_str((current_file)), ks_str((""))) == 0))) {
        save_file_as();
    } else {
        long long content = _ks_get_text(gui, editor);
        _ks_write(file, current_file, content);
        is_modified = KS_BOOL(0);
        update_status(ks_str((_ks_concat("💾 Saved: ", current_file))), ks_str(("success")));
        update_title();
    }
}

#line 244 "examples/full_gui_demo.ks"
void save_file_as(void) {
    long long path = _ks_filedialog(gui, "save", "Save File");
    if (ks_v_bool(ks_bool(ks_v_cmp(ks_int(path), ks_str((""))) != 0))) {
        long long content = _ks_get_text(gui, editor);
        _ks_write(file, path, content);
        current_file = path;
        is_modified = KS_BOOL(0);
        update_status(ks_str((_ks_concat("💾 Saved: ", ks_val_to_str(ks_int(path))))), ks_str(("success")));
        update_title();
    }
}

#line 256 "examples/full_gui_demo.ks"
void exit_app(void) {
    _ks_destroy(gui, window);
}

#line 260 "examples/full_gui_demo.ks"
void pick_color(void) {
    static ks_val_t _arr_colors[] = {(ks_val_t){.tag=KS_T_STR,.as.s=(char*)"#1e1e2e"}, (ks_val_t){.tag=KS_T_STR,.as.s=(char*)"#282a36"}, (ks_val_t){.tag=KS_T_STR,.as.s=(char*)"#2d2d3f"}, (ks_val_t){.tag=KS_T_STR,.as.s=(char*)"#1a1a2e"}, (ks_val_t){.tag=KS_T_STR,.as.s=(char*)"#0f0f1a"}, (ks_val_t){.tag=KS_T_STR,.as.s=(char*)"#242436"}};
    ks_array colors = { .data = _arr_colors, .length = 6 };
    ks_val_t idx = ks_int((long long)(atoll(ks_v_mul(ks_flt(system_random_uniform(0.0, 1.0)), ks_int(ks_array_len(colors))))));
    ks_val_t color = ks_array_get(colors, idx);
    _ks_configure(gui, editor);
    update_status(ks_str((_ks_concat("🎨 Theme applied: ", ks_val_to_str(color)))), ks_str(("info")));
}

#line 268 "examples/full_gui_demo.ks"
void zoom_in(void) {
    font_size = ks_v_add(font_size, KS_INT(2LL));
    _ks_configure(gui, editor);
    update_status(ks_str((_ks_concat("🔍 Font size: ", ks_val_to_str(font_size)))), ks_str(("info")));
}

#line 274 "examples/full_gui_demo.ks"
void zoom_out(void) {
    if (ks_v_bool(ks_bool(ks_v_cmp(font_size, KS_INT(8LL)) > 0))) {
        font_size = ks_v_sub(font_size, KS_INT(2LL));
        _ks_configure(gui, editor);
        update_status(ks_str((_ks_concat("🔍 Font size: ", ks_val_to_str(font_size)))), ks_str(("info")));
    }
}

#line 282 "examples/full_gui_demo.ks"
void reset_zoom(void) {
    font_size = KS_INT(14LL);
    _ks_configure(gui, editor);
    update_status(ks_str(("🔄 Font size reset to 14")), ks_str(("info")));
}

#line 288 "examples/full_gui_demo.ks"
void refresh_files(void) {
    _ks_delete(gui, file_list, "0", "end");
    long long files = (ks_os_listdir(current_directory), 0);
    ks_val_t count = KS_INT(0LL);
    for (long long _idx_f = 0; _idx_f < 5; _idx_f++) {
        long long f = files[_idx_f];
        _ks_insert(gui, file_list, "end", f);
        count = ks_v_add(count, KS_INT(1LL));
    }
    _ks_set_text(gui, sidebar_count, _ks_concat(_ks_concat("📊 ", ks_val_to_str(count)), " files"));
    update_status(ks_str((_ks_concat(_ks_concat("🔄 Refreshed file list - ", ks_val_to_str(count)), " files"))), ks_str(("info")));
}

#line 300 "examples/full_gui_demo.ks"
void open_selected(void) {
    long long selection = _ks_curselection(gui, file_list);
    if (ks_v_bool(ks_bool(ks_v_cmp(ks_int(selection), ks_none()) != 0))) {
        ks_val_t index = _ks_dict_get_simple(selection, "0");
        long long filename = _ks_listbox_get(gui, file_list, index);
        ks_val_t filepath = _ks_concat(_ks_concat(current_directory, "/"), ks_val_to_str(ks_int(filename)));
        if (ks_v_bool(_ks_isfile(0  /* os.path */, filepath))) {
            long long content = _ks_read(file, filepath);
            _ks_delete(gui, editor, "1.0", "end");
            _ks_insert(gui, editor, "1.0", content);
            current_file = filepath;
            is_modified = KS_BOOL(0);
            update_status(ks_str((_ks_concat("📖 Opened: ", ks_val_to_str(filepath)))), ks_str(("success")));
            update_title();
        }
    } else {
        update_status(ks_str(("⚠️ No file selected")), ks_str(("warning")));
    }
}

#line 320 "examples/full_gui_demo.ks"
void show_about(void) {
    _ks_message_box(gui, "About", "🌟 KentScript Studio ✨\n\nVersion 2.0\n\nA modern, feature-rich IDE\nbuilt with KentScript\n\nMade with ❤️", "info");
}

#line 324 "examples/full_gui_demo.ks"
void show_documentation(void) {
    _ks_message_box(gui, "Documentation", "📚 KentScript Studio Documentation\n\n📁 File Menu:\n  • New - Create new file\n  • Open - Open existing file\n  • Save - Save current file\n  • Save As - Save as new file\n\n✏️ Edit Menu:\n  • Cut/Copy/Paste - Standard editing\n  • Select All - Select all text\n\n👁️ View Menu:\n  • Zoom In/Out - Adjust font size\n  • Reset Zoom - Reset to default\n\n📂 Sidebar:\n  • Shows files in current directory\n  • Double-click or use Open to edit\n\n✏️ Editor:\n  • Full-featured code editor\n  • Line numbers in status bar", "info");
}

#line 328 "examples/full_gui_demo.ks"
void update_status(ks_val_t message, ks_val_t msg_type) {
    char* icon = "🟢";
    if (ks_v_bool(ks_bool(ks_v_cmp(msg_type, ks_str(("warning"))) == 0))) {
        icon = "⚠️";
    } else if (ks_v_bool(ks_bool(ks_v_cmp(msg_type, ks_str(("error"))) == 0))) {
        icon = "❌";
    } else if (ks_v_bool(ks_bool(ks_v_cmp(msg_type, ks_str(("success"))) == 0))) {
        icon = "✅";
    } else if (ks_v_bool(ks_bool(ks_v_cmp(msg_type, ks_str(("info"))) == 0))) {
        icon = "ℹ️";
    }
    char* status_text_value = _ks_concat(_ks_concat(icon, " "), ks_val_to_str(message));
    _ks_set_text(gui, status_text, status_text_value);
}

#line 343 "examples/full_gui_demo.ks"
void update_title(void) {
    char* title = "KentScript Studio ✨";
    char* file_part = "";
    if (ks_v_bool(ks_bool(ks_v_bool(ks_bool(ks_v_cmp(ks_str((current_file)), ks_str((""))) != 0)) && ks_v_bool(ks_bool(ks_v_cmp(ks_str((current_file)), ks_none()) != 0))))) {
        file_part = _ks_concat(" - ", current_file);
    }
    if (ks_v_bool(is_modified)) {
        file_part = _ks_concat(file_part, " *");
    }
    _ks_configure(gui, window);
    char* file_label = "📝 ";
    if (ks_v_bool(ks_bool(ks_v_bool(ks_bool(ks_v_cmp(ks_str((current_file)), ks_str((""))) == 0)) || ks_v_bool(ks_bool(ks_v_cmp(ks_str((current_file)), ks_none()) == 0))))) {
        file_label = _ks_concat(file_label, "Untitled");
    } else {
        file_label = _ks_concat(file_label, current_file);
    }
    if (ks_v_bool(is_modified)) {
        file_label = _ks_concat(file_label, " [Modified]");
    }
    _ks_set_text(gui, status_file, file_label);
}

#line 366 "examples/full_gui_demo.ks"
void update_line_col(void) {
    long long cursor_pos = _ks_text_index(gui, editor, "insert");
    char* parts = _ks_str_int(cursor_pos);
    _ks_set_text(gui, line_col_label, _ks_concat(_ks_concat(_ks_concat("📊 Lines: ", _ks_str_int(cursor_pos)), " Col: "), _ks_str_int(cursor_pos)));
}

void _ks_init_globals(void) {
    window = _ks_create_window(gui, "KentScript Studio ✨", KS_INT(1000LL), KS_INT(700LL));
}

int main(int argc, char** argv) {
    ks_argc = argc; ks_argv = argv;
    _ks_init_globals();
#line 27 "examples/full_gui_demo.ks"
    _ks_geometry(gui, window, "1000x700");
#line 28 "examples/full_gui_demo.ks"
    _ks_configure(gui, window);
#line 30 "examples/full_gui_demo.ks"
    long long main_container = _ks_create_frame(gui, window);
#line 31 "examples/full_gui_demo.ks"
    _ks_pack(gui, main_container);
#line 36 "examples/full_gui_demo.ks"
    long long menubar = _ks_create_menu(gui, window);
#line 37 "examples/full_gui_demo.ks"
    _ks_config(window);
#line 39 "examples/full_gui_demo.ks"
    long long file_menu = _ks_create_menu(gui, menubar);
#line 40 "examples/full_gui_demo.ks"
    _ks_menu_add_cascade(gui, menubar, "📁 File", file_menu);
#line 42 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, file_menu, "🆕 New", &_ks_lambda_unknown, "Ctrl+N");
#line 43 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, file_menu, "📂 Open", &_ks_lambda_unknown, "Ctrl+O");
#line 44 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, file_menu, "💾 Save", &_ks_lambda_unknown, "Ctrl+S");
#line 45 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, file_menu, "💾 Save As...", &_ks_lambda_unknown, "Ctrl+Shift+S");
#line 46 "examples/full_gui_demo.ks"
    _ks_menu_add_separator(gui, file_menu);
#line 47 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, file_menu, "❌ Exit", &_ks_lambda_unknown, "Alt+F4");
#line 49 "examples/full_gui_demo.ks"
    long long edit_menu = _ks_create_menu(gui, menubar);
#line 50 "examples/full_gui_demo.ks"
    _ks_menu_add_cascade(gui, menubar, "✏️ Edit", edit_menu);
#line 52 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, edit_menu, "✂️ Cut", &_ks_lambda_unknown, "Ctrl+X");
#line 53 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, edit_menu, "📋 Copy", &_ks_lambda_unknown, "Ctrl+C");
#line 54 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, edit_menu, "📝 Paste", &_ks_lambda_unknown, "Ctrl+V");
#line 55 "examples/full_gui_demo.ks"
    _ks_menu_add_separator(gui, edit_menu);
#line 56 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, edit_menu, "🔲 Select All", &_ks_lambda_unknown, "Ctrl+A");
#line 58 "examples/full_gui_demo.ks"
    long long view_menu = _ks_create_menu(gui, menubar);
#line 59 "examples/full_gui_demo.ks"
    _ks_menu_add_cascade(gui, menubar, "👁️ View", view_menu);
#line 61 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, view_menu, "🔍 Zoom In", &_ks_lambda_unknown, "Ctrl++");
#line 62 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, view_menu, "🔍 Zoom Out", &_ks_lambda_unknown, "Ctrl+-");
#line 63 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, view_menu, "🔄 Reset Zoom", &_ks_lambda_unknown, "Ctrl+0");
#line 65 "examples/full_gui_demo.ks"
    long long help_menu = _ks_create_menu(gui, menubar);
#line 66 "examples/full_gui_demo.ks"
    _ks_menu_add_cascade(gui, menubar, "❓ Help", help_menu);
#line 68 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, help_menu, "ℹ️ About", &_ks_lambda_unknown);
#line 69 "examples/full_gui_demo.ks"
    _ks_menu_add_command(gui, help_menu, "📚 Documentation", &_ks_lambda_unknown);
#line 74 "examples/full_gui_demo.ks"
    long long toolbar = _ks_create_frame(gui, main_container);
#line 75 "examples/full_gui_demo.ks"
    _ks_configure(gui, toolbar);
#line 76 "examples/full_gui_demo.ks"
    _ks_pack(gui, toolbar);
#line 78 "examples/full_gui_demo.ks"
    long long toolbar_inner = _ks_create_frame(gui, toolbar);
#line 79 "examples/full_gui_demo.ks"
    _ks_configure(gui, toolbar_inner);
#line 80 "examples/full_gui_demo.ks"
    _ks_pack(gui, toolbar_inner);
#line 82 "examples/full_gui_demo.ks"
    long long new_btn = _ks_create_button(gui, toolbar_inner, "🆕 New", &_ks_lambda_unknown);
#line 83 "examples/full_gui_demo.ks"
    _ks_configure(gui, new_btn);
#line 84 "examples/full_gui_demo.ks"
    _ks_pack(gui, new_btn);
#line 86 "examples/full_gui_demo.ks"
    long long open_tool_btn = _ks_create_button(gui, toolbar_inner, "📂 Open", &_ks_lambda_unknown);
#line 87 "examples/full_gui_demo.ks"
    _ks_configure(gui, open_tool_btn);
#line 88 "examples/full_gui_demo.ks"
    _ks_pack(gui, open_tool_btn);
#line 90 "examples/full_gui_demo.ks"
    long long save_tool_btn = _ks_create_button(gui, toolbar_inner, "💾 Save", &_ks_lambda_unknown);
#line 91 "examples/full_gui_demo.ks"
    _ks_configure(gui, save_tool_btn);
#line 92 "examples/full_gui_demo.ks"
    _ks_pack(gui, save_tool_btn);
#line 94 "examples/full_gui_demo.ks"
    long long sep1 = _ks_create_frame(gui, toolbar_inner);
#line 95 "examples/full_gui_demo.ks"
    _ks_configure(gui, sep1);
#line 96 "examples/full_gui_demo.ks"
    _ks_pack(gui, sep1);
#line 98 "examples/full_gui_demo.ks"
    long long color_btn = _ks_create_button(gui, toolbar_inner, "🎨 Theme", &_ks_lambda_unknown);
#line 99 "examples/full_gui_demo.ks"
    _ks_configure(gui, color_btn);
#line 100 "examples/full_gui_demo.ks"
    _ks_pack(gui, color_btn);
#line 102 "examples/full_gui_demo.ks"
    long long sep2 = _ks_create_frame(gui, toolbar_inner);
#line 103 "examples/full_gui_demo.ks"
    _ks_configure(gui, sep2);
#line 104 "examples/full_gui_demo.ks"
    _ks_pack(gui, sep2);
#line 106 "examples/full_gui_demo.ks"
    long long zoom_in_btn = _ks_create_button(gui, toolbar_inner, "🔍+", &_ks_lambda_unknown);
#line 107 "examples/full_gui_demo.ks"
    _ks_configure(gui, zoom_in_btn);
#line 108 "examples/full_gui_demo.ks"
    _ks_pack(gui, zoom_in_btn);
#line 110 "examples/full_gui_demo.ks"
    long long zoom_out_btn = _ks_create_button(gui, toolbar_inner, "🔍-", &_ks_lambda_unknown);
#line 111 "examples/full_gui_demo.ks"
    _ks_configure(gui, zoom_out_btn);
#line 112 "examples/full_gui_demo.ks"
    _ks_pack(gui, zoom_out_btn);
#line 115 "examples/full_gui_demo.ks"
    long long title_label = _ks_create_label(gui, toolbar_inner, "📝 KentScript Studio ✨");
#line 116 "examples/full_gui_demo.ks"
    _ks_configure(gui, title_label);
#line 117 "examples/full_gui_demo.ks"
    _ks_pack(gui, title_label);
#line 122 "examples/full_gui_demo.ks"
    long long content_paned = _ks_create_panedwindow(gui, main_container);
#line 123 "examples/full_gui_demo.ks"
    _ks_pack(gui, content_paned);
#line 128 "examples/full_gui_demo.ks"
    long long sidebar = _ks_create_frame(gui, content_paned);
#line 129 "examples/full_gui_demo.ks"
    _ks_configure(gui, sidebar);
#line 130 "examples/full_gui_demo.ks"
    _ks_pack(gui, sidebar);
#line 132 "examples/full_gui_demo.ks"
    long long sidebar_header = _ks_create_frame(gui, sidebar);
#line 133 "examples/full_gui_demo.ks"
    _ks_configure(gui, sidebar_header);
#line 134 "examples/full_gui_demo.ks"
    _ks_pack(gui, sidebar_header);
#line 136 "examples/full_gui_demo.ks"
    long long sidebar_icon = _ks_create_label(gui, sidebar_header, "📂 Files");
#line 137 "examples/full_gui_demo.ks"
    _ks_configure(gui, sidebar_icon);
#line 138 "examples/full_gui_demo.ks"
    _ks_pack(gui, sidebar_icon);
#line 140 "examples/full_gui_demo.ks"
    long long sidebar_count = _ks_create_label(gui, sidebar_header, "📊");
#line 141 "examples/full_gui_demo.ks"
    _ks_configure(gui, sidebar_count);
#line 142 "examples/full_gui_demo.ks"
    _ks_pack(gui, sidebar_count);
#line 144 "examples/full_gui_demo.ks"
    long long file_list = _ks_create_listbox(gui, sidebar);
#line 145 "examples/full_gui_demo.ks"
    _ks_pack(gui, file_list);
#line 147 "examples/full_gui_demo.ks"
    long long file_scroll = _ks_create_scrollbar(gui, sidebar);
#line 148 "examples/full_gui_demo.ks"
    _ks_pack(gui, file_scroll);
#line 149 "examples/full_gui_demo.ks"
    _ks_configure(gui, file_list);
#line 151 "examples/full_gui_demo.ks"
    long long sidebar_buttons = _ks_create_frame(gui, sidebar);
#line 152 "examples/full_gui_demo.ks"
    _ks_configure(gui, sidebar_buttons);
#line 153 "examples/full_gui_demo.ks"
    _ks_pack(gui, sidebar_buttons);
#line 155 "examples/full_gui_demo.ks"
    long long refresh_btn = _ks_create_button(gui, sidebar_buttons, "🔄 Refresh", &_ks_lambda_unknown);
#line 156 "examples/full_gui_demo.ks"
    _ks_configure(gui, refresh_btn);
#line 157 "examples/full_gui_demo.ks"
    _ks_pack(gui, refresh_btn);
#line 159 "examples/full_gui_demo.ks"
    long long open_sel_btn = _ks_create_button(gui, sidebar_buttons, "📖 Open", &_ks_lambda_unknown);
#line 160 "examples/full_gui_demo.ks"
    _ks_configure(gui, open_sel_btn);
#line 161 "examples/full_gui_demo.ks"
    _ks_pack(gui, open_sel_btn);
#line 166 "examples/full_gui_demo.ks"
    long long editor_frame = _ks_create_frame(gui, content_paned);
#line 167 "examples/full_gui_demo.ks"
    _ks_configure(gui, editor_frame);
#line 168 "examples/full_gui_demo.ks"
    _ks_pack(gui, editor_frame);
#line 170 "examples/full_gui_demo.ks"
    long long editor_header = _ks_create_frame(gui, editor_frame);
#line 171 "examples/full_gui_demo.ks"
    _ks_configure(gui, editor_header);
#line 172 "examples/full_gui_demo.ks"
    _ks_pack(gui, editor_header);
#line 174 "examples/full_gui_demo.ks"
    long long editor_title = _ks_create_label(gui, editor_header, "✏️ Editor");
#line 175 "examples/full_gui_demo.ks"
    _ks_configure(gui, editor_title);
#line 176 "examples/full_gui_demo.ks"
    _ks_pack(gui, editor_title);
#line 178 "examples/full_gui_demo.ks"
    long long line_col_label = _ks_create_label(gui, editor_header, "📊 Lines: 0  Col: 0");
#line 179 "examples/full_gui_demo.ks"
    _ks_configure(gui, line_col_label);
#line 180 "examples/full_gui_demo.ks"
    _ks_pack(gui, line_col_label);
#line 182 "examples/full_gui_demo.ks"
    long long editor = _ks_create_text(gui, editor_frame);
#line 183 "examples/full_gui_demo.ks"
    _ks_pack(gui, editor);
#line 185 "examples/full_gui_demo.ks"
    long long editor_scroll_y = _ks_create_scrollbar(gui, editor_frame);
#line 186 "examples/full_gui_demo.ks"
    _ks_pack(gui, editor_scroll_y);
#line 187 "examples/full_gui_demo.ks"
    _ks_configure(gui, editor);
#line 189 "examples/full_gui_demo.ks"
    long long editor_scroll_x = _ks_create_scrollbar(gui, editor);
#line 190 "examples/full_gui_demo.ks"
    _ks_configure(gui, editor);
#line 195 "examples/full_gui_demo.ks"
    long long status_bar = _ks_create_frame(gui, window);
#line 196 "examples/full_gui_demo.ks"
    _ks_configure(gui, status_bar);
#line 197 "examples/full_gui_demo.ks"
    _ks_pack(gui, status_bar);
#line 199 "examples/full_gui_demo.ks"
    long long status_text = _ks_create_label(gui, status_bar, "🟢 Ready");
#line 200 "examples/full_gui_demo.ks"
    _ks_configure(gui, status_text);
#line 201 "examples/full_gui_demo.ks"
    _ks_pack(gui, status_text);
#line 203 "examples/full_gui_demo.ks"
    long long status_file = _ks_create_label(gui, status_bar, "📝 Untitled");
#line 204 "examples/full_gui_demo.ks"
    _ks_configure(gui, status_file);
#line 205 "examples/full_gui_demo.ks"
    _ks_pack(gui, status_file);
#line 373 "examples/full_gui_demo.ks"
    system_socket_bind(gui, editor, ks_str((char*)("<KeyRelease>")));
#line 374 "examples/full_gui_demo.ks"
    system_socket_bind(gui, editor, ks_str((char*)("<ButtonRelease-1>")));
#line 377 "examples/full_gui_demo.ks"
    refresh_files();
#line 378 "examples/full_gui_demo.ks"
    update_status(ks_str(("🟢 Welcome to KentScript Studio!")), ks_str(("success")));
#line 379 "examples/full_gui_demo.ks"
    update_title();
#line 381 "examples/full_gui_demo.ks"
    _ks_mainloop(gui, window);
    return 0;
}