...
class Substitution:
    ...
    def bind(self, let, type_):
        """Add a binding from type variable to type"""
        if isinstance(let, TypeVariable):
            self.bindings[let.name] = type_
    
    def lookup(self, let):
        """Look up a type variable"""
        if isinstance(let, TypeVariable):
            return self.bindings.get(let.name, let)
        return let
...
class HindleyMilnerInferencer:
    ...
    def generate_report(self):
        ...
        for let, type_ in self.type_env.items():
            report += f"  {let}: {type_}\n"
        return report
...
class CompileTimeBorrowChecker:
    ...
    def __init__(self):
        self.ownership = {}      # let -> scope_id (who owns it)
        self.borrows = {}        # let -> [(scope_id, is_mutable)]
        self.moved_vars = {}     # let -> line_moved
    ...
    def exit_scope(self, scope_id):
        ...
        for let, owner_id in list(self.ownership.items()):
            if owner_id == scope_id:
                del self.ownership[let]
...
class BorrowChecker:
    ...
    def __init__(self):
        self.owners = {}          # let -> scope_id
        self.borrows = {}         # let -> list of (scope_id, mutable)
        self.moved = set()        # let that were moved
        self.lifetimes = {}       # let -> creation_scope
    ...
    def exit_scope(self):
        ...
        for let in list(self.borrows.keys()):
            self.borrows[let] = [(s, m) for s, m in self.borrows[let] if s != scope_id]
            if not self.borrows[let]:
                del self.borrows[let]
        
        # Clean up moved vars that are out of scope
        self.moved = {v for v in self.moved if v in self.owners}
    
    def declare_ownership(self, let, scope_id):
        """Declare that a scope owns a variable"""
        # Skip builtins completely
        if let in self.builtins or (let.startswith('__') and let.endswith('__')):
            return
        
        if let in self.moved:
            raise BorrowError(f"Cannot own '{let}' - value was moved")
        self.owners[let] = scope_id
        self.lifetimes[let] = scope_id
        
    def move_ownership(self, let, from_scope, to_scope):
        """Move ownership from one scope to another"""
        # Skip builtins
        if let in self.builtins or (let.startswith('__') and let.endswith('__')):
            return
        
        if let not in self.owners:
            raise BorrowError(f"Cannot move '{let}' - not owned")
        if self.owners[let] != from_scope:
            raise BorrowError(f"Cannot move '{let}' - not owned by this scope")
        if let in self.borrows and self.borrows[let]:
            raise BorrowError(f"Cannot move '{let}' - has {len(self.borrows[let])} active borrows")
        
        self.owners[let] = to_scope
        self.moved.add(let)
        
    def borrow(self, let, scope_id, mutable=False):
        ...
        if let in self.moved:
            raise BorrowError(f"Cannot borrow '{let}' - value was moved")
        
        # Check for conflicts
        if let in self.borrows:
            for _, is_mut in self.borrows[let]:
                if mutable or is_mut:
                    suffix = " mutably" if is_mut else ""
                    raise BorrowError(f"Cannot borrow '{let}' - already borrowed{suffix}")
        
        # Register borrow
        if let not in self.borrows:
            self.borrows[let] = []
        self.borrows[let].append((scope_id, mutable))
        
    def release(self, let, scope_id):
        """Release a borrow"""
        # Skip builtins
        if let in self.builtins or (let.startswith('__') and let.endswith('__')):
            return
    ...
    def check_access(self, let, mutable=False):
        ...
        if let in self.moved:
            raise BorrowError(f"Cannot access '{let}' - value was moved")
        ...
        if let in self.borrows:
            for _, is_mut in self.borrows[let]:
                ...
                if not mutable:
                    return
            if mutable:
                raise BorrowError(f"Cannot mutably access '{let}' - {len(self.borrows[let])} active borrows")
                
    def get_borrow_count(self, let):
        """Get number of active borrows"""
        return len(self.borrows.get(let, []))
...
class Lexer:
    ...
    def read_identifier(self) -> Token:
        ...
        # ENFORCE: Reject 'let' keyword - use 'let' instead
        if ident == 'let':
            raise SyntaxError(f"Line {line}, Col {col}: "
                            f"'let' is not allowed in KentScript. Use 'let' instead. "
                            f"Example: let x = 42;")
...
class Parser:
    ...
    def parse_for(self) -> ForStmt:
        self.advance()
        let = self.expect(TokenType.IDENTIFIER).value
        ...
        return ForStmt(let, iterable, body, else_block)
    ...
    def parse_borrow(self) -> BorrowStmt:
        ...
        if self.current().type == TokenType.MULTIPLY:
            ...
            self.advance()
        let = self.expect(TokenType.IDENTIFIER).value
        return BorrowStmt(let, mutable)
    
    def parse_release(self) -> ReleaseStmt:
        self.advance()
        let = self.expect(TokenType.IDENTIFIER).value
        return ReleaseStmt(let)
    ...
    def parse_unary(self) -> ASTNode:
        ...
        if self.current().type == TokenType.MOVE:
            ...
            if self.current().type == TokenType.IDENTIFIER and self.current().value == 'to':
                ...
                target = let
            return UnaryOp('move', Identifier(let))
        ...
        if self.current().type == TokenType.BORROW:
            ...
            if self.current().type == TokenType.MULTIPLY:
                ...
                mutable = True
            let = self.expect(TokenType.IDENTIFIER).value
            return UnaryOp('borrow' if not mutable else 'borrow_mut', Identifier(let))
        ...
        if token.type == TokenType.LBRACKET:
            ...
            if self.current().type != TokenType.RBRACKET:
                ...
                if self.current().type == TokenType.FOR:
                    ...
                    self.expect(TokenType.RBRACKET)
                    return ListComprehension(first_expr, let, iterable, condition)
        ...
        if token.type == TokenType.LBRACE:
            ...
            if self.current().type != TokenType.RBRACE:
                ...
                if self.current().type == TokenType.FOR:
                    ...
                    self.expect(TokenType.RBRACE)
                    return DictComprehension(key, value, let, iterable, condition)
...
class BytecodeCompiler:
    ...
    def compile_for(self, for_stmt):
        ...
        name_idx = self.add_name(for_stmt.let)
        self.emit('STORE_NAME', name_idx)
...
class Interpreter:
    ...
    def setup_builtins(self):
        ...
        builtins = {
            ...
            'borrow': lambda let, mutable=False: g_borrow_checker.borrow(let, mutable),
            'release': lambda let: g_borrow_checker.release(let),
            
            # Event Loop and Promises (JavaScript-style non-blocking I/O)
            'get_event_loop': get_event_loop,
            'Promise': Promise,
            
            # JIT Compiler (native code generation)
    ...
    def eval(self, node: ASTNode, env: Environment) -> Any:
        ...
        # ---------- FOR LOOP ----------
            ...
            try:
                for item in iterable:
                    ...
                    self.borrow_checker.enter_scope(id(local_env))
                    local_env.define(node.let, item)
        ...
        # ---------- BORROW ----------
            ...
            scope_id = id(env)
            self.borrow_checker.borrow(node.let, scope_id, node.mutable)
            return env.get(node.let)
        
        # ---------- RELEASE ----------
        elif isinstance(node, ReleaseStmt):
            scope_id = id(env)
            self.borrow_checker.release(node.let, scope_id)
            return None
        ...
        # ---------- MOVE ----------
            ...
            from_scope = id(env)
            to_scope = id(target_env)
            self.borrow_checker.move_ownership(node.let, from_scope, to_scope)
            value = env.get(node.let)
            target_env.define(node.let, value)
            return value
    ...
    def import_module(self, module_name: str, alias: Optional[str], env: Environment, names: List[str] = None):
        ...
        # Built-in modules
            ...
            module_attrs = {
                'getpid': __import__('os').getpid,
                'getcwd': __import__('os').getcwd,
                'chdir': __import__('os').chdir,
                'open': lambda p, f, m=438: __import__('os').open(p, f, m),
                'close': __import__('os').close,
                'read': __import__('os').read,
                'write': lambda fd, data: __import__('os').write(fd, data.encode() if isinstance(data, str) else data),
                'stat': lambda p: {'st_size': __import__('os').stat(p).st_size, 'st_mode': __import__('os').stat(p).st_mode},
                'lseek': __import__('os').lseek,
                'exit': __import__('os')._exit,
            }
            ...
            module_attrs = {
                'borrow_immutable': lambda let: let,
                'borrow_mutable': lambda let: let,
    ...
    if prompt_toolkit_available:
        try:
            kscript_completer = WordCompleter([
                'let', 'const', 'mut',
                'print', 'if', 'else', 'elif', 'while', 'for', 'func',
                'class', 'struct', 'enum', 'interface', 'trait',
                'import', 'from', 'as', 'return', 'True', 'False', 'None',
                'and', 'or', 'not', 'in', 'is', 'break', 'continue',
                'try', 'except', 'finally', 'raise', 'throw',
                'match', 'case', 'default', 'assert', 'yield', 'async', 'await',
                'decorator', 'type', 'unsafe', 'safe',
                'super', 'self', 'new', 'delete',
                'sizeof', 'typeof', 'with',
                'thread', 'spawn', 'Lock', 'RLock',
                'Event', 'Semaphore', 'ThreadPool',
                'pub', 'priv', 'static', 'inline', 'extern',
                'move', 'borrow', 'release',
                'defer', 'where', 'impl', 'extends', 'implements',
                'export',
            ])
