...
class KSError:
    ...
    def set_context(cls, filename=None, source=None):
        """Set current file context for error reporting (flat, no stacking)."""
        cls._current_file = filename
        cls._current_source = source

    @classmethod
    def get_context(cls):
        """Return a snapshot of the current file context (for save/restore)."""
        return (cls._current_file, cls._current_source)

    @classmethod
    def restore_context(cls, saved):
        """Restore a context snapshot returned by get_context()."""
        if saved is None:
            return
        cls._current_file, cls._current_source = saved
