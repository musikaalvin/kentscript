...
KEYWORDS = {
    ...
    "none": TokenType.NONE,
    # Flow control (parser has implementations)
...
class Lexer:
    ...
    def tokenize(self) -> List[Token]:
        while self.pos < len(self.source):
            ...
            # Detect # — not a comment in KentScript
            if self._peek() == "#":
                hash_line, hash_col = self.line, self.column
                self._advance()  # consume #
                while self._peek() != "\n" and self._peek() != "\0":
                    self._advance()
                self.tokens.append(Token(TokenType.ERROR, "#", hash_line, hash_col))
