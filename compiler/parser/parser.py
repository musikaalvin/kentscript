...
class Parser:
    ...
    def parse_statement(self) -> Optional[ASTNode]:
        ...
        if token.type == TokenType.IDENTIFIER:
            ...
            if token.value == "function":
                KSError.syntax_error(
                    f"Invalid keyword 'function'. KentScript uses 'func' for function declarations",
                    line=token.line,
                    col=token.column,
    ...
    def parse_class(self) -> ClassDef:
        ...
        self.expect(TokenType.RBRACE)
        node = ClassDef(name, methods, parent, [], implements)
        node.line = _start_line
        return node
    ...
    def parse_import(self):
        ...
        _kw_as_name = {TokenType.ASYNC, TokenType.MATCH, TokenType.FUNC, TokenType.TYPE}
        modules = []
        while True:
            if self.current().type == TokenType.STRING_LIT:
