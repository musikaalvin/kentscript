...
Architecture:
  ...
  - x86-64 JIT (mmap RWX + ctypes, hotspot detection)
...
try:
    ...
    from compiler_cache import (
        ...
        HotFunctionJIT as _KSHotFunctionJIT,
...
# These types are used by the KentScript → C transpiler and LLVM IR emitter.
...
class LLVMBackendOptimized:
    ...
    def compile_with_llvmlite(self, ast_nodes) -> bytes:
        ...
        tm = target.create_target_machine(codemodel="default", reloc="pic", opt=2)
            ...
            if cls == "FunctionCall":
                ...
                if hasattr(fn, "name") and fn.name == "print" and node.args:
                    ...
                    puts_ty = ir.FunctionType(
                        ir.IntType(32), [ir.PointerType(ir.IntType(8))]
                    )
                    puts_fn = (
                        ir.Function(builder.module, puts_ty, name="puts")
                        if "puts" not in builder.module.globals
                        else builder.module.globals["puts"]
                    )
                    arg = node.args[0]
                    if arg.__class__.__name__ == "Literal" and isinstance(
                        arg.value, str
                    ):
                        s = arg.value + "\0"
                        ...
                        g = ir.GlobalVariable(
                            builder.module, const_str.type, name=".str"
                        )
    ...
    def generate(self, ast=None) -> str:
        ...
        lines = [
            ...
            "",
            "; KentScript v3.1.0 — [KS-REF-003] LLVM IR output",
    ...
    def jit_run(self, ast_nodes) -> int:
        ...
        try:
            r = subprocess.run(
                ["clang", obj_path, "-o", out_path], capture_output=True, text=True
            )
            if r.returncode != 0:
...
class LLVMBackend:
    ...
    def declare_function(self, func: Function):
        ...
        param_types = (
            [self.llvm_type(p.param_type) for p in func.parameters]
            if func.parameters
            else []
        )
    ...
    def process_expression(self, expr: ASTNode) -> str:
        ...
        if isinstance(expr, IntLiteral):
            ...
            args = ", ".join(
                f"i32 {self.process_expression(arg)}" for arg in expr.arguments
            )
            temp = self.get_temp_var()
    ...
    def write_mmio(addr, value, size=4):
        ...
        try:
            ...
            raise PermissionError(
                "MMIO access requires root privileges (kernel Ring 0)"
            )
        except FileNotFoundError:
            raise PermissionError(
                "/dev/mem not available - direct hardware access disabled"
            )
        ...
        if isinstance(ast_node, dict):
            if ast_node.get("type") == "BinOp":
                ...
                return self.jit_execute(
                    self.generate_x86_64_return_constant(ast_node.get("value", 0))
                )
...
class LLVMJITEngine:
    ...
    def _generate_llvm_ir(self, ast, func_name: str) -> str:
        """Generate real LLVM IR from a KentScript AST node.

        Walks the AST to produce well-typed LLVM IR instead of the
        previous stub that always returned `arg0 + 1`.

        Supports: arithmetic (+,-,*,/,%), comparisons, if/else, while,
                  local variables, function calls, and return statements.
        """
        lines = []
        self._ir_tmp = 0  # SSA temp counter
        self._ir_vars: dict = {}  # varname → LLVM SSA name
        self._ir_label = 0

        def fresh() -> str:
            self._ir_tmp += 1
            return f"%t{self._ir_tmp}"

        def fresh_label() -> str:
            self._ir_label += 1
            return f"L{self._ir_label}"

        def emit(s: str):
            lines.append(s)

        def llvm_type(ks_type: str) -> str:
            mapping = {
                "i32": "i32",
                "i64": "i64",
                "int": "i64",
            ...
            return mapping.get(str(ks_type).lower(), "i64")

        def gen_expr(node) -> str:
            """Return SSA value name for an expression node."""
            # IntLiteral / float
            if (
                hasattr(node, "value")
                and not hasattr(node, "op")
                and not hasattr(node, "left")
            ):
                v = node.value
                ...
                if isinstance(v, float):
                    t = fresh()
                    emit(f"  {t} = fadd double 0.0, {v:.17g}")
                    return t
                if isinstance(v, str):
                    return f'c"{v}\00"'
                return "0"
            ...
            # Identifier
            if hasattr(node, "name") and not hasattr(node, "args"):
                vname = node.name
                slot = self._ir_vars.get(vname)
                if slot:
                    t = fresh()
                    emit(f"  {t} = load i64, i64* {slot}")
                    return t
                return f"%{vname}"

            # BinaryOp
            if hasattr(node, "op") and hasattr(node, "left") and hasattr(node, "right"):
                lv = gen_expr(node.left)
                rv = gen_expr(node.right)
                t = fresh()
                op_map = {
                    "+": "add",
                    "-": "sub",
                ...
                llop = op_map.get(getattr(node, "op", "+"), "add")
                # Comparison → i1, needs zext for arithmetic context
                if llop.startswith("icmp"):
                    emit(f"  {t} = {llop} i64 {lv}, {rv}")
                    t2 = fresh()
                    emit(f"  {t2} = zext i1 {t} to i64")
                    return t2
                emit(f"  {t} = {llop} i64 {lv}, {rv}")
                return t
            ...
            if hasattr(node, "op") and hasattr(node, "operand"):
                v = gen_expr(node.operand)
                t = fresh()
                if node.op == "-":
                    emit(f"  {t} = sub i64 0, {v}")
                elif node.op == "!":
                    emit(f"  {t} = xor i64 {v}, 1")
                else:
                    return v
                return t

            # FunctionCall
            if hasattr(node, "args") and (
                hasattr(node, "name") or hasattr(node, "func")
            ):
                callee = getattr(node, "name", None) or getattr(
                    node.func, "name", "unknown"
                )
                a_vals = [gen_expr(a) for a in (node.args or [])]
                args_str = ", ".join(f"i64 {a}" for a in a_vals)
                t = fresh()
                emit(f"  {t} = call i64 @{callee}({args_str})")
                return t
        ...
        def gen_stmt(node):
            ...
            if node_type in ("Assignment", "VarDecl", "LetStmt", "Variable"):
                vname = getattr(node, "name", None) or getattr(
                    getattr(node, "target", None), "name", "tmp"
                )
                if vname not in self._ir_vars:
                    slot = f"%slot_{vname}"
                    emit(f"  {slot} = alloca i64")
                    self._ir_vars[vname] = slot
                val = gen_expr(
                    getattr(node, "value", None)
                    or getattr(node, "init", None)
                    or type("Z", (), {"value": 0})()
                )
                emit(f"  store i64 {val}, i64* {self._ir_vars[vname]}")

            elif node_type == "ReturnStmt":
                v = gen_expr(node.value) if node.value else "0"
                emit(f"  ret i64 {v}")

            elif node_type in ("IfStmt", "If"):
                cond_v = gen_expr(node.condition)
                cond_b = fresh()
                emit(f"  {cond_b} = trunc i64 {cond_v} to i1")
                then_l, else_l, end_l = fresh_label(), fresh_label(), fresh_label()
                emit(f"  br i1 {cond_b}, label %{then_l}, label %{else_l}")
                emit(f"{then_l}:")
                for s in getattr(node.then_block, "statements", None) or [
                    node.then_block
                ]:
                    gen_stmt(s)
                emit(f"  br label %{end_l}")
                emit(f"{else_l}:")
                if node.else_block:
                    for s in getattr(node.else_block, "statements", None) or [
                        node.else_block
                    ]:
                        gen_stmt(s)
                emit(f"  br label %{end_l}")
                ...
                test_l, body_l, end_l = fresh_label(), fresh_label(), fresh_label()
                emit(f"  br label %{test_l}")
                emit(f"{test_l}:")
                cond_v = gen_expr(node.condition)
                cond_b = fresh()
                emit(f"  {cond_b} = trunc i64 {cond_v} to i1")
                emit(f"  br i1 {cond_b}, label %{body_l}, label %{end_l}")
                emit(f"{body_l}:")
                for s in getattr(node.body, "statements", None) or [node.body]:
                    gen_stmt(s)
                emit(f"  br label %{test_l}")
                emit(f"{end_l}:")

            elif node_type in ("ExprStmt", "ExpressionStatement"):
                gen_expr(node.expression if hasattr(node, "expression") else node.expr)
        ...
        # Accept ast as either a FunctionDef node or a raw body list/node
        if hasattr(ast, "params"):
            params = ast.params or []
            body_stmts = getattr(ast.body, "statements", None) or [ast.body]
            ret_type = llvm_type(getattr(ast, "return_type", "i64") or "i64")
        elif isinstance(ast, list):
            params = []
            body_stmts = ast
            ret_type = "i64"
            ...
            params = []
            body_stmts = getattr(ast, "statements", None) or [ast]
            ret_type = "i64"

        param_strs = [f"i64 %{p}" for p in params]
        emit(f"define {ret_type} @{func_name}({', '.join(param_strs)}) {{")
        emit("entry:")

        # Pre-allocate parameter slots so assignments inside work
        for p in params:
            slot = f"%slot_{p}"
            emit(f"  {slot} = alloca i64")
            emit(f"  store i64 %{p}, i64* {slot}")
            self._ir_vars[p] = slot

        # Generate body
        for stmt in body_stmts:
            gen_stmt(stmt)

        # Ensure every exit path has a ret
        if not lines or not lines[-1].strip().startswith("ret"):
            emit(f"  ret {ret_type} {'0' if ret_type != 'void' else ''}")
...
class RealLLVMIRGenerator:
    ...
    def emit_div(
        self, left: str, right: str, llvm_type: str = "i64", signed: bool = True
    ) -> str:
        result = self.new_var()
    ...
    def emit_icmp(
        self, pred: str, left: str, right: str, llvm_type: str = "i64"
    ) -> str:
        result = self.new_var()
    ...
    def emit_getelementptr(self, base_type: str, ptr: str, indices: list) -> str:
        ...
        self.emit_line(
            f"  {result} = getelementptr {base_type}, {base_type}* {ptr}, {idx_str}"
        )
        return result
    ...
    def emit_atomic_load(self, ptr: str, llvm_type: str) -> str:
        result = self.new_var()
        self.emit_line(
            f"  {result} = load atomic {llvm_type}, {llvm_type}* {ptr} seq_cst, align 8"
        )
        return result

    def emit_atomic_store(self, value: str, ptr: str, llvm_type: str):
        self.emit_line(
            f"  store atomic {llvm_type} {value}, {llvm_type}* {ptr} seq_cst, align 8"
        )

    def emit_atomicrmw(self, op: str, ptr: str, value: str, llvm_type: str) -> str:
        result = self.new_var()
        self.emit_line(
            f"  {result} = atomicrmw {op} {llvm_type}* {ptr}, {llvm_type} {value} seq_cst"
        )
        return result
...
class RealLLVMCompiler:
    ...
    def compile_ir_to_asm(self, ir_code: str, output_asm: str) -> bool:
        ...
        try:
            result = subprocess.run(
                ["llc", ll_file, "-o", output_asm], capture_output=True, timeout=30
            )
            return result.returncode == 0
        except:
            return False
...
LLVM_SYSTEMS = {
    ...
    "CompilerArchitecture": CompilerArchitecture,
}
import sys
import ctypes
...
class LLVMBackendCompleteFixed:
    ...
    def compile_ir_direct_to_binary(self, ir_code: str, output_binary: str) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["gcc", obj_file, "-o", output_binary], capture_output=True, timeout=30
            )
...
class ARM64CodeEmitter:
    ...
    def execute(code: bytes, a: int, b: int) -> int:
        try:
            ...
            # Fallback: use Python arithmetic when JIT unavailable
            return a + b
...
try:
    ...
    _interp_inject(
        ...
        _global_jit_compiler=_global_jit_compiler,
